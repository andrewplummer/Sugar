...
function Db(a,b,c,d){var e=/^(.+?)(\[.*\])$/,g,f,j;if(d!==n&&(f=b.match(e))){j=f[1];b=f[2].replace(/^\[|\]$/g,"").split("][");b.forEach(function(i){g=!i||i.match(/^\d+$/);if(!j&&da(a))j=a.length;a[j]||(a[j]=g?[]:{});a=a[j];j=i});if(!j&&g)j=a.length.toString();Db(a,j,c)}else a[b]=c.match(/^[\d.]+$/)?parseFloat(c):c==="true"?k:c==="false"?n:c}E(p,n,k,{watch:function(a,b,c){if(ca){var d=a[b];p.defineProperty(a,b,{enumerable:k,configurable:k,get:function(){return d},set:function(e){d=c.call(a,b,d,e)}})}}});
