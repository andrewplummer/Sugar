...
function dc(a,b,c,d){Vb||ec();var e=E(b).join(""),g=Vb[d];e=e.replace(/all/,"").replace(/(\w)lphabet|umbers?|atakana|paces?|unctuation/g,"$1");return a.replace(c,function(f){return g[f]&&(!e||e.has(g[f].type))?g[f].to:f})}
function ec(){var a;Vb={zenkaku:{},hankaku:{}};Ub.forEach(function(b){N(b.start,b.end,function(c){Z(b.type,t.fromCharCode(c),t.fromCharCode(c+b.shift))})});cc.each(function(b,c){a=bc.charAt(c);Z("k",a,b);b.match($b)&&Z("k",a+"\uff9e",b.shift(1));b.match(ac)&&Z("k",a+"\uff9f",b.shift(2))});Zb.each(function(b,c){Z("p",Yb.charAt(c),b)});Z("k","\uff73\uff9e","\u30f4");Z("k","\uff66\uff9e","\u30fa");Z("s"," ","\u3000")}function Z(a,b,c){Vb.zenkaku[b]={type:a,to:c};Vb.hankaku[c]={type:a,to:b}}
function fc(){Rb={};G(Tb,function(a,b){b.split("").forEach(function(c){Rb[c]=a});Sb+=b});Sb=r("["+Sb+"]","g")}
