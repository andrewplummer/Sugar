...
{ba:"hh?|hours|12hr",format:function(a){a=W(a,"Hours");return a===0?12:a-P(a/13)*12}},{ba:"HH?|24hr",format:function(a){return W(a,"Hours")}},{ba:"dd?|date|day",format:function(a){return W(a,"Date")}},{ba:"dow|weekday",oa:k,format:function(a,b,c){a=W(a,"Day");return b.weekdays[a+(c-1)*7]}},{ba:"MM?",format:function(a){return W(a,"Month")+1}},{ba:"mon|month",oa:k,format:function(a,b,c){a=W(a,"Month");return b.months[a+(c-1)*12]}},{ba:"y{2,4}|year",format:function(a){return W(a,"FullYear")}},{ba:"[Tt]{1,2}",
format:function(a,b,c,d){a=W(a,"Hours");b=b.ampm[P(a/12)];if(d.length===1)b=b.slice(0,1);if(d.slice(0,1)==="T")b=b.toUpperCase();return b}},{ba:"z{1,4}|tz|timezone",text:k,format:function(a,b,c,d){a=a.getUTCOffset();if(d=="z"||d=="zz")a=a.replace(/(\d{2})(\d{2})/,function(e,f){return R(f,d.length)});return a}},{ba:"iso(tz|timezone)",format:function(a){return a.getUTCOffset(k)}},{ba:"ord",format:function(a){a=W(a,"Date");return a+pa(a)}}],cb=[{$:"year",method:"FullYear",ka:k,da:function(a){return(365+
(a?a.isLeapYear()?1:0:0.25))*24*60*60*1E3}},{$:"month",method:"Month",ka:k,da:function(a,b){var c=30.4375,d;if(a){d=a.daysInMonth();if(b<=d.days())c=d}return c*24*60*60*1E3}},{$:"week",method:"Week",da:aa(6048E5)},{$:"day",method:"Date",ka:k,da:aa(864E5)},{$:"hour",method:"Hours",da:aa(36E5)},{$:"minute",method:"Minutes",da:aa(6E4)},{$:"second",method:"Seconds",da:aa(1E3)},{$:"millisecond",method:"Milliseconds",da:aa(1)}],db={};function eb(a){na(this,a);this.ha=ab.concat()}
eb.prototype={getMonth:function(a){return B(a)?a-1:this.months.indexOf(a)%12},la:function(a){return this.weekdays.indexOf(a)%7},ra:function(a){var b;return B(a)?a:a&&(b=this.numbers.indexOf(a))!==-1?(b+1)%10:1},xa:function(a){var b=this;return a.replace(q(this.num,"g"),function(c){return b.ra(c)||""})},va:function(a){return V.units[this.units.indexOf(a)%8]},Ea:function(a){return this.qa(a,a[2]>0?"future":"past")},ua:function(a){return this.qa(fb(a),"duration")},ya:function(a){a=a||this.code;return a===
"en"||a==="en-US"?k:this.variant},Ba:function(a){return a===this.ampm[0]},Ca:function(a){return a===this.ampm[1]},qa:function(a,b){var c=a[0],d=a[1],e=a[2],f,g,j;if(this.code=="ru"){j=c.toString().slice(-1);switch(k){case j==1:j=1;break;case j>=2&&j<=4:j=2;break;default:j=3}}else j=this.plural&&c>1?1:0;g=this.units[j*8+d]||this.units[d];if(this.capitalizeUnit)g=gb(g);f=this.modifiers.filter(function(i){return i.name=="sign"&&i.value==(e>0?1:-1)})[0];return this[b].replace(/\{(.*?)\}/g,function(i,
h){switch(h){case "num":return c;case "unit":return g;case "sign":return f.src}})},wa:function(){return this.pa?[this.pa].concat(this.ha):this.ha},addFormat:function(a,b,c,d,e){var f=c||[],g=this,j;a=a.replace(/\s+/g,"[-,. ]*");a=a.replace(/\{([^,]+?)\}/g,function(i,h){var n=h.match(/\?$/),r=h.match(/(\d)(?:-(\d))?/),y=h.match(/^\d+$/),z=h.replace(/[^a-z]+$/,""),C,ca;if(y)C=g.optionals[y[0]];else if(g[z])C=g[z];else if(g[z+"s"]){C=g[z+"s"];if(r){ca=[];C.forEach(function(da,Da){var Q=Da%(g.units?8:
C.length);if(Q>=r[1]&&Q<=(r[2]||r[1]))ca.push(da)});C=ca}C=hb(C)}if(y)return"(?:"+C+")?";else{c||f.push(z);return"("+C+")"+(n?"?":"")}});if(b){b=ib(Wa,g,e);e=["t","[\\s\\u3000]"].concat(g.timeMarker);j=a.match(/\\d\{\d,\d\}\)+\??$/);jb(g,"(?:"+b+")[,\\s\\u3000]+?"+a,Va.concat(f),d);jb(g,a+"(?:[,\\s]*(?:"+e.join("|")+(j?"+":"*")+")"+b+")?",f.concat(Va),d)}else jb(g,a,f,d)}};function kb(a,b){var c;D(a)||(a="");c=db[a]||db[a.slice(0,2)];if(b===m&&!c)throw Error("Invalid locale.");return c||Ua}
...
j;g=new eb(b);c("modifiers");"months,weekdays,units,numbers,articles,optionals,timeMarker,ampm,timeSuffixes,dateParse,timeParse".split(",").forEach(c);j=!g.monthSuffix;e("months",j,12);e("weekdays",j,7);e("units",m,8);e("numbers",m,10);g.code=a;g.date=f(1,2,g.digitDate);g.year=f(4,4);g.num=function(){var i=["\\d+"].concat(g.articles);if(g.numbers)i=i.concat(g.numbers);return hb(i)}();(function(){var i=[];g.ia={};g.modifiers.forEach(function(h){var n=h.name;d(h.src,function(r){var y=g[n];g.ia[r]=h;
...
g}function jb(a,b,c,d){a.ha.unshift({Ga:d,Aa:a,Da:q("^"+b+"$","i"),to:c})}function gb(a){return a.slice(0,1).toUpperCase()+a.slice(1)}function hb(a){return a.filter(function(b){return!!b}).join("|")}function mb(a,b){var c;if(L(a[0]))return a;else if(B(a[0])&&!B(a[1]))return[a[0]];else if(D(a[0])&&b)return[nb(a[0]),a[1]];c={};Za.forEach(function(d,e){c[d.$]=a[e]});return[c]}
function nb(a,b){var c={};if(match=a.match(/^(\d+)?\s?(\w+?)s?$/i)){if(K(b))b=parseInt(match[1])||1;c[match[2].toLowerCase()]=b}return c}function ob(a,b){var c={},d,e;b.forEach(function(f,g){d=a[g+1];if(!(K(d)||d==="")){if(f==="year")c.Ha=d;e=parseFloat(d.replace(/,/,"."));c[f]=!isNaN(e)?e:d.toLowerCase()}});return c}function pb(a){a=a.trim().replace(/^(just )?now|\.+$/i,"");return qb(a)}
...
function rb(a,b,c,d){var e=new s,f=m,g,j,i,h,n,r,y,z,C;d&&e.na();if(ha(a))e=a.clone();else if(B(a))e=new s(a);else if(L(a)){e.set(a,k);h=a}else if(D(a)){g=kb(b);a=pb(a);g&&H(g.wa(),function(ca,da){var Da=a.match(da.Da);if(Da){i=da;j=i.Aa;h=ob(Da,i.to,j);h.utc&&e.na();j.pa=i;if(h.timestamp){h=h.timestamp;return m}if(i.Ga&&!D(h.month)&&(D(h.date)||g.ya(b))){z=h.month;h.month=h.date;h.date=z}if(h.year&&h.Ha.length===2)h.year=O(W(new s,"FullYear")/100)*100-O(h.year/100)*100+h.year;if(h.month){h.month=
j.getMonth(h.month);if(h.shift&&!h.unit)h.unit=j.units[7]}if(h.weekday&&h.date)delete h.weekday;else if(h.weekday){h.weekday=j.la(h.weekday);if(h.shift&&!h.unit)h.unit=j.units[5]}if(h.day&&(z=j.ia[h.day])){h.day=z.value;e.reset();f=k}else if(h.day&&(r=j.la(h.day))>-1){delete h.day;if(h.num&&h.month){C=function(){var Q=e.la();e.Fa(7*(h.num-1)+(Q>r?r+7:r))};h.day=1}else h.weekday=r}if(h.date&&!B(h.date))h.date=j.xa(h.date);if(j.Ca(h.ampm)&&h.hour<12)h.hour+=12;else if(j.Ba(h.ampm)&&h.hour===12)h.hour=
0;if("offset_hours"in h||"offset_minutes"in h){e.na();h.offset_minutes=h.offset_minutes||0;h.offset_minutes+=h.offset_hours*60;if(h.offset_sign==="-")h.offset_minutes*=-1;h.minute-=h.offset_minutes}if(h.unit){f=k;y=j.ra(h.num);n=j.va(h.unit);if(h.shift||h.edge){y*=(z=j.ia[h.shift])?z.value:0;if(n==="month"&&J(h.date)){e.set({day:h.date},k);delete h.date}if(n==="year"&&J(h.month)){e.set({month:h.month,day:h.date},k);delete h.month;delete h.date}}if(h.sign&&(z=j.ia[h.sign]))y*=z.value;if(J(h.weekday)){e.set({weekday:h.weekday},
k);delete h.weekday}h[n]=(h[n]||0)+y}if(h.year_sign==="-")h.year*=-1;$a.slice(1,4).forEach(function(Q,Ub){var zb=h[Q.$],Ab=zb%1;if(Ab){h[$a[Ub].$]=O(Ab*(Q.$==="second"?1E3:60));h[Q.$]=P(zb)}});return m}});if(i)if(f)e.advance(h);else{e.ea&&e.reset();sb(e,h,k,m,c)}else e=a?new s(a):new s;if(h&&h.edge){z=j.ia[h.edge];H($a.slice(4),function(ca,da){if(J(h[da.$])){n=da.$;return m}});if(n==="year")h.ga="month";else if(n==="month"||n==="week")h.ga="day";e[(z.value<0?"endOf":"beginningOf")+gb(n)]();z.value===
-2&&e.reset()}C&&C()}return{fa:e,set:h}}function fb(a){var b,c=v.abs(a),d=c,e=0;$a.slice(1).forEach(function(f,g){b=P(O(c/f.da()*10)/10);if(b>=1){d=b;e=g+1}});return[d,e,a]}
function tb(a,b,c,d){var e,f=kb(d),g=q(/^[A-Z]/);if(a.isValid())if(Date[b])b=Date[b];else{if(A(b)){e=fb(a.millisecondsFromNow());b=b.apply(a,e.concat(f))}}else return"Invalid Date";if(!b&&c){e=e||fb(a.millisecondsFromNow());if(e[1]===0){e[1]=1;e[0]=1}return f.Ea(e)}b=b||"long";b=f[b]||b;bb.forEach(function(j){b=b.replace(q("\\{("+j.ba+")(\\d)?\\}",j.oa?"i":""),function(i,h,n){i=j.format(a,f,n||1,h);n=h.length;var r=h.match(/^(.)\1+$/);if(j.oa){if(n===3)i=i.slice(0,3);if(r||h.match(g))i=gb(i)}else if(r&&
!j.text)i=(B(i)?R(i,n):i.toString()).slice(-n);return i})});return b}
function ub(a,b,c,d){var e=rb(b,l,l,d),f=0;d=b=0;var g;if(c>0){b=d=c;g=k}if(!e.fa.isValid())return m;if(e.set&&e.set.ga){cb.forEach(function(i){if(i.$===e.set.ga)f=i.da(e.fa,a-e.fa)-1});c=gb(e.set.ga);if(e.set.edge||e.set.shift)e.fa["beginningOf"+c]();if(e.set.ga==="month")j=e.fa.clone()["endOf"+c]().getTime();if(!g&&e.set.sign&&e.set.ga!="millisecond"){b=50;d=-50}}g=a.getTime();c=e.fa.getTime();var j=j||c+f;return g>=c-b&&g<=j+d}
function sb(a,b,c,d,e){function f(i){return J(b[i])?b[i]:b[i+"s"]}var g,j;if(B(b)&&d)b={milliseconds:b};else if(B(b)){a.setTime(b);return a}if(b.date)b.day=b.date;H($a,function(i,h){var n=h.$==="day";if(J(f(h.$))||n&&J(f("weekday"))){b.ga=h.$;j=+i;return m}else if(c&&h.$!=="week"&&(!n||!J(f("week"))))X(a,h.method,n?1:0)});cb.forEach(function(i){var h=i.$;i=i.method;var n;n=f(h);if(!K(n)){if(d){if(h==="week"){n=(b.day||0)+n*7;i="Date"}n=n*d+W(a,i)}else h==="month"&&J(f("day"))&&X(a,"Date",15);X(a,
i,n);if(d&&h==="month"){h=n;if(h<0)h+=12;h%12!=W(a,"Month")&&X(a,"Date",0)}}});if(!d&&!J(f("day"))&&J(f("weekday"))){g=f("weekday");a.Fa(g)}(function(){var i=new s;return e===-1&&a>i||e===1&&a<i})()&&H($a.slice(j+1),function(i,h){if((h.ka||h.$==="week"&&J(f("weekday")))&&!J(f(h.$))){a[h.ja](e);return m}});return a}function W(a,b){return a["get"+(a.ea?"UTC":"")+b]()}function X(a,b,c){return a["set"+(a.ea?"UTC":"")+b](c)}
function ib(a,b,c){var d={h:0,m:1,s:2},e;b=b||V;return a.replace(/{([a-z])}/g,function(f,g){var j=[],i=g==="h",h=i&&!c;if(g==="t")return b.ampm.join("|");else{i&&j.push(":");if(e=b.timeSuffixes[d[g]])j.push(e+"\\s*");return j.length===0?"":"(?:"+j.join("|")+")"+(h?"":"?")}})}function vb(a,b){var c,d,e;if(B(a[1]))c=mb(a)[0];else{c=a[0];d=a[1];e=a[2]}return rb(c,d,b,e).fa}
...
return R(O(-b/60),2,k)+c+R(b%60,2)},utc:function(a){this.ea=a===k||arguments.length===0;return this},isUTC:function(){return!!this.ea||this.getTimezoneOffset()===0},advance:function(){var a=mb(arguments,k);return sb(this,a[0],a[1],1)},rewind:function(){var a=mb(arguments,k);return sb(this,a[0],a[1],-1)},isValid:function(){return!isNaN(this.getTime())},isAfter:function(a,b){return this.getTime()>s.create(a).getTime()-(b||0)},isBefore:function(a,b){return this.getTime()<s.create(a).getTime()+(b||0)},
...
e=this.clone().na(c);switch(k){case a==="future":return this.getTime()>(new s).getTime();case a==="past":return this.getTime()<(new s).getTime();case a==="weekday":return W(e,"Day")>0&&W(e,"Day")<6;case a==="weekend":return W(e,"Day")===0||W(e,"Day")===6;case (d=V.weekdays.indexOf(a)%7)>-1:return W(e,"Day")===d;case (d=V.months.indexOf(a)%12)>-1:return W(e,"Month")===d}}return ub(this,a,b,c)}},reset:function(a){var b={},c;a=a||"hours";if(a==="date")a="days";c=cb.some(function(d){return a===d.$||a===
...
function wb(a,b){function c(){return O(this*b)}function d(){return vb(arguments)[a.ja](this)}function e(){return vb(arguments)[a.ja](-this)}var f=a.$,g={};g[f]=c;g[f+"s"]=c;g[f+"Before"]=e;g[f+"sBefore"]=e;g[f+"Ago"]=e;g[f+"sAgo"]=e;g[f+"After"]=d;g[f+"sAfter"]=d;g[f+"FromNow"]=d;g[f+"sFromNow"]=d;u.extend(g)}u.extend({duration:function(a){return kb(a).ua(this)}});
...
Za.splice(2,1);
I(s,k,m,cb,function(a,b,c){var d=b.$,e=gb(d),f=b.da(),g,j;b.ja="add"+e+"s";g=function(i,h){return O((this.getTime()-s.create(i,h).getTime())/f)};j=function(i,h){return O((s.create(i,h).getTime()-this.getTime())/f)};a[d+"sAgo"]=j;a[d+"sUntil"]=j;a[d+"sSince"]=g;a[d+"sFromNow"]=g;a[b.ja]=function(i,h){var n={};n[d]=i;return this.advance(n,h)};wb(b,f);c<3&&["Last","This","Next"].forEach(function(i){a["is"+i+e]=function(){return this.is(i+" "+d)}});if(c<4){a["beginningOf"+e]=function(){var i={};switch(d){case "year":i.year=
...
V.addFormat("(\\d{1,2})[-.\\/]{full_month}(?:[-.\\/](\\d{2,4}))?",k,["date","month","year"],k);V.addFormat("{full_month}[-.](\\d{4,4})",m,["month","year"]);V.addFormat("\\/Date\\((\\d+(?:\\+\\d{4,4})?)\\)\\/",m,["timestamp"]);V.addFormat(ib(Wa,V),m,Va);ab=V.ha.slice(0,7).reverse();V.ha=V.ha.slice(7).concat(ab);I(s,k,m,"short,long,full",function(a,b){a[b]=function(c){return tb(this,b,m,c)}});
