...
function za(a,b,c,d){var e=j;if(a===b)return j;else if(F(b)&&E(a))return s(b).test(a);else if(z(b))return b.apply(c,d);else if(la(b)&&ka(a)){I(b,function(g){za(a[g],b[g],c,[a[g],a])||(e=m)});return e}else return S(a)===S(b)}function T(a,b,c,d){return L(b)?a:z(b)?b.apply(c,d||[]):z(a[b])?a[b].call(a):a[b]}function U(a,b,c,d){var e,g;if(c<0)c=a.length+c;g=isNaN(c)?0:c;for(c=d===j?a.length+g:a.length;g<c;){e=g%a.length;if(e in a){if(b.call(a,a[e],e,a)===m)break}else return Aa(a,b,g,d);g++}}
function Aa(a,b,c){var d=[],e;for(e in a)e in a&&e>>>0==e&&e!=4294967295&&e>=c&&d.push(parseInt(e));d.sort().each(function(g){return b.call(a,a[g],g,a)});return a}function Ba(a,b,c,d,e){var g,f;U(a,function(i,h,l){if(za(i,b,l,[i,h,l])){g=i;f=h;return m}},c,d);return e?f:g}function Ca(a,b){var c=[],d={},e,g;U(a,function(f,i){g=b?T(f,b,a,[f,i,a]):f;e=S(g);if(!(e in d&&(typeof f!=="function"||f===d[e]))){d[e]=g;c.push(f)}});return c}
function Fa(a,b,c){var d=[],e={};b.each(function(g){e[S(g)]=g});a.each(function(g){var f=S(g);if((f in e&&(typeof g!=="function"||g===e[f]))!=c){delete e[f];d.push(g)}});return d}function Ga(a,b,c){b=b||Infinity;c=c||0;var d=[];U(a,function(e){if(da(e)&&c<b)d=d.concat(Ga(e,b,c+1));else d.push(e)});return d}function Ha(a){var b=[];H(a,function(c){b=b.concat(c)});return b}
...
var Pa="AlphanumericSortOrder",La="AlphanumericSortIgnore",Ka="AlphanumericSortIgnoreCase",Na="AlphanumericSortEquivalents";G(r,m,m,{create:function(){var a=[];H(arguments,function(b){if(ka(b))a=a.concat(r.prototype.slice.call(b));else a.push(b)});return a}});
...
"-")h.year*=-1;Ya.slice(1,4).forEach(function(pa,Ea){var wb=h[pa.$],xb=wb%1;if(xb){h[Ya[Ea].$]=O(xb*(pa.$==="second"?1E3:60));h[pa.$]=P(wb)}});return m}});if(i)if(e)d.advance(h);else{h.utc&&d.reset();pb(d,h,j,h.utc,m,c)}else d=a?new t(a):new t;if(h&&h.edge){q=f.ha[h.edge];I(Ya.slice(4),function(C,A){if(K(h[A.$])){l=A.$;return m}});if(l==="year")h.fa="month";else if(l==="month"||l==="week")h.fa="day";d[(q.value<0?"endOf":"beginningOf")+db(l)]();q.value===-2&&d.reset()}B&&B()}return{ea:d,set:h}}
...
t.extend({set:function(){var a=jb(arguments);return pb(this,a[0],a[1])},setUTC:function(){var a=jb(arguments);return pb(this,a[0],a[1],j)},setWeekday:function(a){if(!L(a))return this.setDate(this.getDate()+a-this.getDay())},setUTCWeekday:function(a){if(!L(a))return this.setDate(this.getUTCDate()+a-this.getDay())},setWeek:function(a){if(!L(a)){this.setMonth(0);this.setDate(a*7+1)}},setUTCWeek:function(a){if(!L(a)){this.setMonth(0);this.setUTCDate(a*7+1)}},getWeek:function(){return qb(this)},getUTCWeek:function(){return qb(this.toUTC())},
...
var Eb=[],Fb=[],Y=[],Gb=[],Hb={},Ib,Z;function Jb(a,b){var c=a.indexOf(b);c>-1&&a.splice(c,1)}
function Kb(a,b,c){E(b)&&Jb(Y,b);Jb(Y,c);a.unshift({ja:b,ra:c})}function Lb(a,b){return a==b||a=="all"||!a}function Mb(a){return Y.some(function(b){return(new s("\\b"+b+"$","i")).test(a)})}function Nb(a,b){a=E(a)?a.toString():"";return a.isBlank()||Mb(a)?a:Ob(a,b?Eb:Fb)}function Ob(a,b){I(b,function(c,d){if(a.match(d.ja)){a=a.replace(d.ja,d.ra);return m}});return a}function Pb(a){return a.replace(/^\W*[a-z]/,function(b){return b.toUpperCase()})}
Z={acronym:function(a){Hb[a.toLowerCase()]=a;a=p.keys(Hb).map(function(b){return Hb[b]});Z.acronymRegExp=s(a.join("|"),"g")},plural:function(a,b){Kb(Eb,a,b)},singular:function(a,b){Kb(Fb,a,b)},irregular:function(a,b){var c=a.first(),d=a.from(1),e=b.first(),g=b.from(1),f=e.toUpperCase(),i=e.toLowerCase(),h=c.toUpperCase(),l=c.toLowerCase();Jb(Y,a);Jb(Y,b);if(h==f){Z.plural(new s("({1}){2}$".assign(c,d),"i"),"$1"+g);Z.plural(new s("({1}){2}$".assign(e,g),"i"),"$1"+g);Z.singular(new s("({1}){2}$".assign(e,
g),"i"),"$1"+d)}else{Z.plural(new s("{1}{2}$".assign(h,d)),f+g);Z.plural(new s("{1}{2}$".assign(l,d)),i+g);Z.plural(new s("{1}{2}$".assign(f,g)),f+g);Z.plural(new s("{1}{2}$".assign(i,g)),i+g);Z.singular(new s("{1}{2}$".assign(f,g)),h+d);Z.singular(new s("{1}{2}$".assign(i,g)),l+d)}},uncountable:function(a){var b=r.isArray(a)?a:H(arguments);Y=Y.concat(b)},human:function(a,b){Gb.unshift({ja:a,ra:b})},clear:function(a){if(Lb(a,"singulars"))Fb=[];if(Lb(a,"plurals"))Eb=[];if(Lb(a,"uncountables"))Y=[];
if(Lb(a,"humans"))Gb=[];if(Lb(a,"acronyms"))Hb={}}};Ib=["and","or","nor","a","an","the","so","but","to","of","at","by","from","into","on","onto","off","out","in","over","with","for"];Z.plural(/$/,"s");Z.plural(/s$/gi,"s");Z.plural(/(ax|test)is$/gi,"$1es");Z.plural(/(octop|vir|fung|foc|radi|alumn)(i|us)$/gi,"$1i");Z.plural(/(census|alias|status)$/gi,"$1es");Z.plural(/(bu)s$/gi,"$1ses");Z.plural(/(buffal|tomat)o$/gi,"$1oes");Z.plural(/([ti])um$/gi,"$1a");Z.plural(/([ti])a$/gi,"$1a");
...
G(u,j,m,{pluralize:function(){return Nb(this,j)},singularize:function(){return Nb(this,m)},humanize:function(){var a=Ob(this,Gb);a=a.replace(/_id$/g,"");a=a.replace(/(_)?([a-z\d]*)/gi,function(b,c,d){return(c?" ":"")+(Hb[d]||d.toLowerCase())});return Pb(a)},titleize:function(){var a=/[.:;!]$/,b,c,d;return this.spacify().humanize().words(function(e,g,f){b=a.test(e);d=g==0||g==f.length-1||b||c;c=b;return d||Ib.indexOf(e)===-1?Pb(e):e}).join(" ")},parameterize:function(a){var b=this;if(a===undefined)a=
"-";if(b.normalize)b=b.normalize();b=b.replace(/[^a-z0-9\-_]+/gi,a);if(a)b=b.replace(new s("^{sep}+|{sep}+$|({sep}){sep}+".assign({sep:R(a)}),"g"),"$1");return encodeURI(b.toLowerCase())}});u.Inflector=Z;u.Inflector.acronyms=Hb;
