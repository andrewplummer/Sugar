...
(y=i.ha[g.sign]))v*=y.value;if(I(g.weekday)){e.set({weekday:g.weekday},k);delete g.weekday}g[m]=(g[m]||0)+v}if(g.year_sign==="-")g.year*=-1;ab.slice(1,4).forEach(function(S,Ub){var zb=g[S.$],Ab=zb%1;if(Ab){g[ab[Ub].$]=N(Ab*(S.$==="second"?1E3:60));g[S.$]=pa(zb)}});return n}});if(j)if(f)e.advance(g);else{e._utc&&e.reset();tb(e,g,k,n,c)}else e=a?new s(a):new s;if(g&&g.edge){y=i.ha[g.edge];G(ab.slice(4),function(L,P){if(I(g[P.$])){m=P.$;return n}});if(m==="year")g.fa="month";else if(m==="month"||m===
"week")g.fa="day";e[(y.value<0?"endOf":"beginningOf")+hb(m)]();y.value===-2&&e.reset()}J&&J();e.utc(n)}return{ea:e,set:g}}function gb(a){var b,c=w.abs(a),d=c,e=0;ab.slice(1).forEach(function(f,h){b=pa(N(c/f.da()*10)/10);if(b>=1){d=b;e=h+1}});return[d,e,a]}
