...
function ga(a){if(!a.SugarMethods){ha(a,"SugarMethods",{});E(a,n,n,{restore:function(){var b=arguments.length===0,c=F(arguments);G(a.SugarMethods,function(d,e){if(b||c.indexOf(d)>-1)ha(e.wa?a.prototype:a,d,e.method)})},extend:function(b,c,d){E(a,d!==n,c,b)}})}}function E(a,b,c,d){var e=b?a.prototype:a,g;ga(a);G(d,function(h,i){g=e[h];if(typeof c==="function")i=ia(e[h],i,c);if(c!==n||!e[h])ha(e,h,i);a.SugarMethods[h]={wa:b,method:i,Da:g}})}
function H(a,b,c,d,e){var g={};d=C(d)?d.split(","):d;d.forEach(function(h,i){e(g,h,i)});E(a,b,c,g)}function ia(a,b,c){return function(){return a&&(c===k||!c.apply(this,arguments))?a.apply(this,arguments):b.apply(this,arguments)}}function ha(a,b,c){if(ca)p.defineProperty(a,b,{value:c,configurable:k,enumerable:n,writable:k});else a[b]=c}function F(a,b){var c=[],d;for(d=0;d<a.length;d++){c.push(a[d]);b&&b.call(a,a[d],d)}return c}
...
function Q(a){C(a)||(a=t(a));return a.replace(/([\\/'*+?|()\[\]{}.^$])/g,"\\$1")}function ua(a,b){var c=typeof a,d,e,g,h,i,j;if(c==="string")return a;g=p.prototype.toString.call(a);d=M(a);e=g==="[object Array]";if(a!=l&&d||e){b||(b=[]);if(b.length>1)for(j=b.length;j--;)if(b[j]===a)return"CYC";b.push(a);d=t(a.constructor);h=e?a:p.keys(a).sort();for(j=0;j<h.length;j++){i=e?j:h[j];d+=i+ua(a[i],b)}b.pop()}else d=1/a===-Infinity?"-0":t(a&&a.valueOf?a.valueOf():a);return c+g+d}
function va(a){var b=p.prototype.toString.call(a);return b==="[object Date]"||b==="[object Array]"||b==="[object String]"||b==="[object Number]"||b==="[object RegExp]"||b==="[object Boolean]"||b==="[object Arguments]"||M(a)}function wa(a,b,c){var d=[],e=a.length,g=b[b.length-1]!==n,h;F(b,function(i){if(ea(i))return n;if(g){i%=e;if(i<0)i=e+i}h=c?a.charAt(i)||"":a[i];d.push(h)});return d.length<2?d[0]:d}
...
function Ba(a,b,c,d){var e=k;if(a===b)return k;else if(D(b)&&C(a))return r(b).test(a);else if(A(b))return b.apply(c,d);else if(M(b)&&ka(a)){G(b,function(g){Ba(a[g],b[g],c,[a[g],a])||(e=n)});return e}else return va(a)&&va(b)?ua(a)===ua(b):a===b}function R(a,b,c,d){return K(b)?a:A(b)?b.apply(c,d||[]):A(a[b])?a[b].call(a):a[b]}
function T(a,b,c,d){var e,g;if(c<0)c=a.length+c;g=isNaN(c)?0:c;for(c=d===k?a.length+g:a.length;g<c;){e=g%a.length;if(e in a){if(b.call(a,a[e],e,a)===n)break}else return Ca(a,b,g,d);g++}}function Ca(a,b,c){var d=[],e;for(e in a)e in a&&e>>>0==e&&e!=4294967295&&e>=c&&d.push(parseInt(e));d.sort().each(function(g){return b.call(a,a[g],g,a)});return a}function Ea(a,b,c,d,e){var g,h;T(a,function(i,j,f){if(Ba(i,b,f,[i,j,f])){g=i;h=j;return n}},c,d);return e?h:g}
function Fa(a,b){var c=[],d={},e;T(a,function(g,h){e=b?R(g,b,a,[g,h,a]):g;Ga(d,e)||c.push(g)});return c}function Ha(a,b,c){var d=[],e={};b.each(function(g){Ga(e,g)});a.each(function(g){var h=ua(g),i=!va(g);if(Ia(e,h,g,i)!=c){var j=0;if(i)for(h=e[h];j<h.length;)if(h[j]===g)h.splice(j,1);else j+=1;else delete e[h];d.push(g)}});return d}function Ja(a,b,c){b=b||Infinity;c=c||0;var d=[];T(a,function(e){if(da(e)&&c<b)d=d.concat(Ja(e,b,c+1));else d.push(e)});return d}
...
function La(a,b,c,d){var e,g=[],h=c==="max",i=c==="min",j=Array.isArray(a);G(a,function(f){var m=a[f];f=R(m,b,a,j?[m,parseInt(f),a]:[]);if(K(f))throw new TypeError("Cannot compare with undefined");if(f===e)g.push(m);else if(K(e)||h&&f>e||i&&f<e){g=[m];e=f}});j||(g=Ja(g,1));return d?g:g[0]}function Ma(a){if(q[Na])a=a.toLowerCase();return a.replace(q[Oa],"")}function Pa(a,b){var c=a.charAt(b);return(q[Qa]||{})[c]||c}function Ra(a){var b=q[Sa];return a?b.indexOf(a):l}
var Sa="AlphanumericSortOrder",Oa="AlphanumericSortIgnore",Na="AlphanumericSortIgnoreCase",Qa="AlphanumericSortEquivalents";E(q,n,n,{create:function(){var a=[],b;F(arguments,function(c){if(ka(c))try{b=q.prototype.slice.call(c,0);if(b.length>0)c=b}catch(d){}a=a.concat(c)});return a}});
E(q,k,n,{find:function(a,b,c){return Ea(this,a,b,c)},findAll:function(a,b,c){var d=[];T(this,function(e,g,h){Ba(e,a,h,[e,g,h])&&d.push(e)},b,c);return d},findIndex:function(a,b,c){a=Ea(this,a,b,c,k);return K(a)?-1:a},count:function(a){if(K(a))return this.length;return this.findAll(a).length},removeAt:function(a,b){if(K(a))return this;if(K(b))b=a;for(var c=0;c<=b-a;c++)this.splice(a,1);return this},include:function(a,b){return this.clone().add(a,b)},exclude:function(){return q.prototype.remove.apply(this.clone(),
...
a.sum()/a.length:0},inGroups:function(a,b){var c=arguments.length>1,d=this,e=[],g=N(this.length/a,void 0,"ceil");oa(0,a-1,function(h){h=h*g;var i=d.slice(h,h+g);c&&i.length<g&&oa(1,g-i.length,function(){i=i.add(b)});e.push(i)});return e},inGroupsOf:function(a,b){var c=[],d=this.length,e=this,g;if(d===0||a===0)return e;if(K(a))a=1;if(K(b))b=l;oa(0,N(d/a,void 0,"ceil")-1,function(h){for(g=e.slice(a*h,a*h+a);g.length<a;)g.push(b);c.push(g)});return c},isEmpty:function(){return this.compact().length==
0},sortBy:function(a,b){var c=this.clone();c.sort(function(d,e){var g,h;g=R(d,a,c,[d]);h=R(e,a,c,[e]);if(C(g)&&C(h)){g=g;h=h;var i,j,f,m,o=0,v=0;g=Ma(g);h=Ma(h);do{f=Pa(g,o);m=Pa(h,o);i=Ra(f);j=Ra(m);if(i===-1||j===-1){i=g.charCodeAt(o)||l;j=h.charCodeAt(o)||l}f=f!==g.charAt(o);m=m!==h.charAt(o);if(f!==m&&v===0)v=f-m;o+=1}while(i!=l&&j!=l&&i===j);g=i===j?v:i<j?-1:1}else g=g<h?-1:g>h?1:0;return g*(b?-1:1)});return c},randomize:function(){for(var a=this.concat(),b,c,d=a.length;d;b=parseInt(w.random()*
...
c,b,[b[a],a,b]))b.splice(a,1);else a++});return b},compact:function(a){var b=[];T(this,function(c){if(da(c))b.push(c.compact());else if(a&&c)b.push(c);else!a&&c!=l&&c.valueOf()===c.valueOf()&&b.push(c)});return b},groupBy:function(a,b){var c=this,d={},e;T(c,function(g,h){e=R(g,a,c,[g,h,c]);d[e]||(d[e]=[]);d[e].push(g)});b&&G(d,b);return d},none:function(){return!this.any.apply(this,arguments)}});E(q,k,n,{all:q.prototype.every,any:q.prototype.some,insert:q.prototype.add});
function Ta(a){if(a&&a.valueOf)a=a.valueOf();return p.keys(a)}function Ua(a,b){H(p,n,n,a,function(c,d){c[d]=function(e,g,h){var i=Ta(e);h=q.prototype[d].call(i,function(j){return b?R(e[j],g,e,[j,e[j],e]):Ba(e[j],g,e,[j,e[j],e])},h);if(da(h))h=h.reduce(function(j,f){j[f]=e[f];return j},{});return h}});xa(a,na)}
...
return a==="en"||a==="en-US"?k:this.variant},ya:function(a){return a===this.ampm[0]},za:function(a){return a&&a===this.ampm[1]},na:function(a,b){var c,d,e=a[0],g=a[1],h=a[2],i=this[b]||this.relative;if(A(i))return i.call(this,e,g,h,b);d=this.units[(this.plural&&e>1?1:0)*8+g]||this.units[g];if(this.capitalizeUnit)d=hb(d);c=this.modifiers.filter(function(j){return j.name=="sign"&&j.value==(h>0?1:-1)})[0];return i.replace(/\{(.*?)\}/g,function(j,f){switch(f){case "num":return e;case "unit":return d;
case "sign":return c.src}})},sa:function(){return this.ma?[this.ma].concat(this.ga):this.ga},addFormat:function(a,b,c,d,e){var g=c||[],h=this,i;a=a.replace(/\s+/g,"[-,. ]*");a=a.replace(/\{([^,]+?)\}/g,function(j,f){var m,o,v,z=f.match(/\?$/);v=f.match(/^(\d+)\??$/);var J=f.match(/(\d)(?:-(\d))?/),L=f.replace(/[^a-z]+$/,"");if(v)m=h.tokens[v[1]];else if(h[L])m=h[L];else if(h[L+"s"]){m=h[L+"s"];if(J){o=[];m.forEach(function(P,Da){var S=Da%(h.units?8:m.length);if(S>=J[1]&&S<=(J[2]||J[1]))o.push(P)});
m=o}m=ib(m)}if(v)v="(?:"+m+")";else{c||g.push(L);v="("+m+")"}if(z)v+="?";return v});if(b){b=jb(Xa,h,e);e=["t","[\\s\\u3000]"].concat(h.timeMarker);i=a.match(/\\d\{\d,\d\}\)+\??$/);kb(h,"(?:"+b+")[,\\s\\u3000]+?"+a,Wa.concat(g),d);kb(h,a+"(?:[,\\s]*(?:"+e.join("|")+(i?"+":"*")+")"+b+")?",g.concat(Wa),d)}else kb(h,a,g,d)}};function lb(a,b){var c;C(a)||(a="");c=eb[a]||eb[a.slice(0,2)];if(b===n&&!c)throw Error("Invalid locale.");return c||Va}
function mb(a,b){function c(f){var m=i[f];if(C(m))i[f]=m.split(",");else m||(i[f]=[])}function d(f,m){f=f.split("+").map(function(o){return o.replace(/(.+):(.+)$/,function(v,z,J){return J.split("|").map(function(L){return z+L}).join("|")})}).join("|");return f.split("|").forEach(m)}function e(f,m,o){var v=[];i[f].forEach(function(z,J){if(m)z+="+"+z.slice(0,3);d(z,function(L,P){v[P*o+J]=L.toLowerCase()})});i[f]=v}function g(f,m,o){f="\\d{"+f+","+m+"}";if(o)f+="|(?:"+ib(i.numbers)+")+";return f}function h(f,
m){i[f]=i[f]||m}var i,j;i=new fb(b);c("modifiers");"months,weekdays,units,numbers,articles,tokens,timeMarker,ampm,timeSuffixes,dateParse,timeParse".split(",").forEach(c);j=!i.monthSuffix;e("months",j,12);e("weekdays",j,7);e("units",n,8);e("numbers",n,10);h("code",a);h("date",g(1,2,i.digitDate));h("year","'\\d{2}|"+g(4,4));h("num",function(){var f=["\\d+"].concat(i.articles);if(i.numbers)f=f.concat(i.numbers);return ib(f)}());(function(){var f=[];i.ha={};i.modifiers.forEach(function(m){var o=m.name;
d(m.src,function(v){var z=i[o];i.ha[v]=m;f.push({name:o,src:v,value:m.value});i[o]=z?z+"|"+v:v})});i.day+="|"+ib(i.weekdays);i.modifiers=f})();if(i.monthSuffix){i.month=g(1,2);i.months=oa(1,12).map(function(f){return f+i.monthSuffix})}i.full_month=g(1,2)+"|"+ib(i.months);i.timeSuffixes.length>0&&i.addFormat(jb(Xa,i),n,Wa);i.addFormat("{day}",k);i.addFormat("{month}"+(i.monthSuffix||""));i.addFormat("{year}"+(i.yearSuffix||""));i.timeParse.forEach(function(f){i.addFormat(f,k)});i.dateParse.forEach(function(f){i.addFormat(f)});
...
function ob(a,b){var c={};if(match=a.match(/^(\d+)?\s?(\w+?)s?$/i)){if(K(b))b=parseInt(match[1])||1;c[match[2].toLowerCase()]=b}return c}function pb(a,b){var c={},d,e;b.forEach(function(g,h){d=a[h+1];if(!(K(d)||d==="")){if(g==="year")c.Ca=d.replace(/'/,"");e=parseFloat(d.replace(/'/,"").replace(/,/,"."));c[g]=!isNaN(e)?e:d.toLowerCase()}});return c}function qb(a){a=a.trim().replace(/^(just )?now|\.+$/i,"");return rb(a)}
function rb(a){return a.replace(Za,function(b,c,d){var e=0,g=1,h,i;if(c)return b;d.split("").reverse().forEach(function(j){j=Ya[j];var f=j>9;if(f){if(h)e+=g;g*=j/(i||1);i=j}else{if(h===n)g*=10;e+=g*j}h=f});if(h)e+=g;return e})}
function sb(a,b,c,d){var e=new s,g=n,h,i,j,f,m,o,v,z,J;e.utc(d);if(fa(a))e.utc(a.isUTC()).setTime(a.getTime());else if(B(a))e.setTime(a);else if(M(a)){e.set(a,k);f=a}else if(C(a)){h=lb(b);a=qb(a);h&&G(h.sa(),function(L,P){var Da=a.match(P.Aa);if(Da){j=P;i=j.xa;f=pb(Da,j.to,i);f.utc&&e.utc();i.ma=j;if(f.timestamp){f=f.timestamp;return n}if(j.Ba&&!C(f.month)&&(C(f.date)||h.va(b))){z=f.month;f.month=f.date;f.date=z}if(f.year&&f.Ca.length===2)f.year=N(V(new s,"FullYear")/100)*100-N(f.year/100)*100+f.year;
if(f.month){f.month=i.getMonth(f.month);if(f.shift&&!f.unit)f.unit=i.units[7]}if(f.weekday&&f.date)delete f.weekday;else if(f.weekday){f.weekday=i.getWeekday(f.weekday);if(f.shift&&!f.unit)f.unit=i.units[5]}if(f.day&&(z=i.ha[f.day])){f.day=z.value;e.reset();g=k}else if(f.day&&(o=i.getWeekday(f.day))>-1){delete f.day;if(f.num&&f.month){J=function(){var S=e.getWeekday();e.setWeekday(7*(f.num-1)+(S>o?o+7:o))};f.day=1}else f.weekday=o}if(f.date&&!B(f.date))f.date=i.ta(f.date);if(i.za(f.ampm)&&f.hour<
12)f.hour+=12;else if(i.ya(f.ampm)&&f.hour===12)f.hour=0;if("offset_hours"in f||"offset_minutes"in f){e.utc();f.offset_minutes=f.offset_minutes||0;f.offset_minutes+=f.offset_hours*60;if(f.offset_sign==="-")f.offset_minutes*=-1;f.minute-=f.offset_minutes}if(f.unit){g=k;v=i.oa(f.num);m=i.ra(f.unit);if(f.shift||f.edge){v*=(z=i.ha[f.shift])?z.value:0;if(m==="month"&&I(f.date)){e.set({day:f.date},k);delete f.date}if(m==="year"&&I(f.month)){e.set({month:f.month,day:f.date},k);delete f.month;delete f.date}}if(f.sign&&
(z=i.ha[f.sign]))v*=z.value;if(I(f.weekday)){e.set({weekday:f.weekday},k);delete f.weekday}f[m]=(f[m]||0)+v}if(f.year_sign==="-")f.year*=-1;ab.slice(1,4).forEach(function(S,Ub){var zb=f[S.$],Ab=zb%1;if(Ab){f[ab[Ub].$]=N(Ab*(S.$==="second"?1E3:60));f[S.$]=pa(zb)}});return n}});if(j)if(g)e.advance(f);else{e._utc&&e.reset();tb(e,f,k,n,c)}else{e=a?new s(a):new s;d&&e.addMinutes(e.getTimezoneOffset())}if(f&&f.edge){z=i.ha[f.edge];G(ab.slice(4),function(L,P){if(I(f[P.$])){m=P.$;return n}});if(m==="year")f.fa=
"month";else if(m==="month"||m==="week")f.fa="day";e[(z.value<0?"endOf":"beginningOf")+hb(m)]();z.value===-2&&e.reset()}J&&J();e.utc(n)}return{ea:e,set:f}}function gb(a){var b,c=w.abs(a),d=c,e=0;ab.slice(1).forEach(function(g,h){b=pa(N(c/g.da()*10)/10);if(b>=1){d=b;e=h+1}});return[d,e,a]}
function ub(a,b,c,d){var e,g=lb(d),h=r(/^[A-Z]/);if(a.isValid())if(Date[b])b=Date[b];else{if(A(b)){e=gb(a.millisecondsFromNow());b=b.apply(a,e.concat(g))}}else return"Invalid Date";if(!b&&c){e=e||gb(a.millisecondsFromNow());if(e[1]===0){e[1]=1;e[0]=1}return g.ua(e)}b=b||"long";b=g[b]||b;cb.forEach(function(i){b=b.replace(r("\\{("+i.ba+")(\\d)?\\}",i.la?"i":""),function(j,f,m){j=i.format(a,g,m||1,f);m=f.length;var o=f.match(/^(.)\1+$/);if(i.la){if(m===3)j=j.slice(0,3);if(o||f.match(h))j=hb(j)}else if(o&&
!i.text)j=(B(j)?O(j,m):j.toString()).slice(-m);return j})});return b}
function vb(a,b,c,d){var e,g,h,i=0,j=0,f=0;e=sb(b,l,l,d);if(c>0){j=f=c;g=k}if(!e.ea.isValid())return n;if(e.set&&e.set.fa){db.forEach(function(m){if(m.$===e.set.fa)i=m.da(e.ea,a-e.ea)-1});b=hb(e.set.fa);if(e.set.edge||e.set.shift)e.ea["beginningOf"+b]();if(e.set.fa==="month")h=e.ea.clone()["endOf"+b]().getTime();if(!g&&e.set.sign&&e.set.fa!="millisecond"){j=50;f=-50}}g=a.getTime();b=e.ea.getTime();h=h||b+i;h=wb(a,b,h);return g>=b-j&&g<=h+f}
...
function tb(a,b,c,d,e){function g(f){return I(b[f])?b[f]:b[f+"s"]}function h(f){return I(g(f))}var i,j;if(B(b)&&d)b={milliseconds:b};else if(B(b)){a.setTime(b);return a}if(I(b.date))b.day=b.date;G(ab,function(f,m){var o=m.$==="day";if(h(m.$)||o&&h("weekday")){b.fa=m.$;j=+f;return n}else if(c&&m.$!=="week"&&(!o||!h("week")))W(a,m.method,o?1:0)});db.forEach(function(f){var m=f.$;f=f.method;var o;o=g(m);if(!K(o)){if(d){if(m==="week"){o=(b.day||0)+o*7;f="Date"}o=o*d+V(a,f)}else m==="month"&&h("day")&&
W(a,"Date",15);W(a,f,o);if(d&&m==="month"){m=o;if(m<0)m=m%12+12;m%12!=V(a,"Month")&&W(a,"Date",0)}}});if(!d&&!h("day")&&h("weekday")){i=g("weekday");a.setWeekday(i)}(function(){var f=new s;return e===-1&&a>f||e===1&&a<f})()&&G(ab.slice(j+1),function(f,m){if((m.ja||m.$==="week"&&h("weekday"))&&!(h(m.$)||m.$==="day"&&h("weekday"))){a[m.ia](e);return n}});return a}function V(a,b){return a["get"+(a._utc?"UTC":"")+b]()}function W(a,b,c){return a["set"+(a._utc?"UTC":"")+b](c)}
function jb(a,b,c){var d={h:0,m:1,s:2},e;b=b||U;return a.replace(/{([a-z])}/g,function(g,h){var i=[],j=h==="h",f=j&&!c;if(h==="t")return b.ampm.join("|");else{j&&i.push(":");if(e=b.timeSuffixes[d[h]])i.push(e+"\\s*");return i.length===0?"":"(?:"+i.join("|")+")"+(f?"":"?")}})}function X(a,b,c){var d,e;if(B(a[1]))d=nb(a)[0];else{d=a[0];e=a[1]}return sb(d,e,b,c).ea}
...
return O(N(-b/60),2,k)+c+O(b%60,2)},utc:function(a){ha(this,"_utc",a===k||arguments.length===0);return this},isUTC:function(){return!!this._utc||this.getTimezoneOffset()===0},advance:function(){var a=nb(arguments,k);return tb(this,a[0],a[1],1)},rewind:function(){var a=nb(arguments,k);return tb(this,a[0],a[1],-1)},isValid:function(){return!isNaN(this.getTime())},isAfter:function(a,b){return this.getTime()>s.create(a).getTime()-(b||0)},isBefore:function(a,b){return this.getTime()<s.create(a).getTime()+
(b||0)},isBetween:function(a,b,c){var d=this.getTime();a=s.create(a).getTime();var e=s.create(b).getTime();b=w.min(a,e);a=w.max(a,e);c=c||0;return b-c<d&&a+c>d},isLeapYear:function(){var a=V(this,"FullYear");return a%4===0&&a%100!==0||a%400===0},daysInMonth:function(){return 32-V(new s(V(this,"FullYear"),V(this,"Month"),32),"Date")},format:function(a,b){return ub(this,a,n,b)},relative:function(a,b){if(C(a)){b=a;a=l}return ub(this,a,k,b)},is:function(a,b,c){var d,e;if(this.isValid()){if(C(a)){a=a.trim().toLowerCase();
...
function xb(a,b){function c(){return N(this*b)}function d(){return X(arguments)[a.ia](this)}function e(){return X(arguments)[a.ia](-this)}var g=a.$,h={};h[g]=c;h[g+"s"]=c;h[g+"Before"]=e;h[g+"sBefore"]=e;h[g+"Ago"]=e;h[g+"sAgo"]=e;h[g+"After"]=d;h[g+"sAfter"]=d;h[g+"FromNow"]=d;h[g+"sFromNow"]=d;u.extend(h)}u.extend({duration:function(a){return lb(a).qa(this)}});
...
H(s,k,n,db,function(a,b,c){function d(f){f=f/h;var m=f%1,o=b.error||0.999;if(m&&w.abs(m%1)>o)f=N(f);return parseInt(f)}var e=b.$,g=hb(e),h=b.da(),i,j;b.ia="add"+g+"s";i=function(f,m){return d(this.getTime()-s.create(f,m).getTime())};j=function(f,m){return d(s.create(f,m).getTime()-this.getTime())};a[e+"sAgo"]=j;a[e+"sUntil"]=j;a[e+"sSince"]=i;a[e+"sFromNow"]=i;a[b.ia]=function(f,m){var o={};o[e]=f;return this.advance(o,m)};xb(b,h);c<3&&["Last","This","Next"].forEach(function(f){a["is"+f+g]=function(){return this.is(f+
" "+e)}});if(c<4){a["beginningOf"+g]=function(){var f={};switch(e){case "year":f.year=V(this,"FullYear");break;case "month":f.month=V(this,"Month");break;case "day":f.day=V(this,"Date");break;case "week":f.weekday=0}return this.set(f,k)};a["endOf"+g]=function(){var f={hours:23,minutes:59,seconds:59,milliseconds:999};switch(e){case "year":f.month=11;f.day=31;break;case "month":f.day=this.daysInMonth();break;case "week":f.weekday=6}return this.set(f,k)}}});
U.addFormat("([+-])?(\\d{4,4})[-.]?{full_month}[-.]?(\\d{1,2})?",k,["year_sign","year","month","date"],n,k);U.addFormat("(\\d{1,2})[-.\\/]{full_month}(?:[-.\\/](\\d{2,4}))?",k,["date","month","year"],k);U.addFormat("{full_month}[-.](\\d{4,4})",n,["month","year"]);U.addFormat("\\/Date\\((\\d+(?:\\+\\d{4,4})?)\\)\\/",n,["timestamp"]);U.addFormat(jb(Xa,U),n,Wa);bb=U.ga.slice(0,7).reverse();U.ga=U.ga.slice(7).concat(bb);H(s,k,n,"short,long,full",function(a,b){a[b]=function(c){return ub(this,b,n,c)}});
...
E(DateRange,k,n,{isValid:function(){return this.start<this.end},duration:function(){return this.isValid()?this.end.getTime()-this.start.getTime():NaN},contains:function(a){var b=this;return(a.start&&a.end?[a.start,a.end]:[a]).every(function(c){return c>=b.start&&c<=b.end})},every:function(a,b){var c=this.start.clone(),d=[],e=0,g,h;if(C(a)){c.advance(ob(a,0),k);g=ob(a);h=a.toLowerCase()==="day"}else g={milliseconds:a};for(;c<=this.end;){d.push(c);b&&b(c,e);if(h&&V(c,"Hours")===23){c=c.clone();W(c,
"Hours",48)}else c=c.clone().advance(g,k);e++}return d},union:function(a){return new DateRange(this.start<a.start?this.start:a.start,this.end>a.end?this.end:a.end)},intersect:function(a){return new DateRange(this.start>a.start?this.start:a.start,this.end<a.end?this.end:a.end)},clone:function(){return new DateRange(this.start,this.end)}});H(DateRange,k,n,"Millisecond,Second,Minute,Hour,Day,Week,Month,Year",function(a,b){a["each"+b]=function(c){return this.every(b,c)}});
E(s,n,n,{range:function(a,b){return new DateRange(a,b)}});
function yb(a,b,c,d,e){var g;if(!a.timers)a.timers=[];B(b)||(b=0);a.timers.push(setTimeout(function(){a.timers.splice(g,1);c.apply(d,e||[])},b));g=a.timers.length}
E(Function,k,n,{lazy:function(a,b){function c(){if(!(g&&e.length>b-2)){e.push([this,arguments]);h()}}var d=this,e=[],g=n,h,i,j;a=a||1;b=b||Infinity;i=N(a,void 0,"ceil");j=N(i/a);h=function(){if(!(g||e.length==0)){for(var f=w.max(e.length-j,0);e.length>f;)Function.prototype.apply.apply(d,e.shift());yb(c,i,function(){g=n;h()});g=k}};return c},delay:function(a){var b=F(arguments).slice(1);yb(this,a,this,this,b);return this},throttle:function(a){return this.lazy(a,1)},debounce:function(a){function b(){b.cancel();
...
function Eb(a,b,c,d){var e=/^(.+?)(\[.*\])$/,g,h,i;if(d!==n&&(h=b.match(e))){i=h[1];b=h[2].replace(/^\[|\]$/g,"").split("][");b.forEach(function(j){g=!j||j.match(/^\d+$/);if(!i&&da(a))i=a.length;a[i]||(a[i]=g?[]:{});a=a[i];i=j});if(!i&&g)i=a.length.toString();Eb(a,i,c)}else a[b]=c.match(/^[+-]?\d+(\.\d+)?$/)?parseFloat(c):c==="true"?k:c==="false"?n:c}
...
E(p,n,n,{isObject:function(a){return M(a)},isNaN:function(a){return B(a)&&a.valueOf()!==a.valueOf()},equal:function(a,b){return va(a)&&va(b)?ua(a)===ua(b):a===b},extended:function(a){return new na(a)},merge:function(a,b,c,d){var e,g;if(a&&typeof b!="string")for(e in b)if(la(b,e)&&a){g=b[e];if(I(a[e])){if(d===n)continue;if(A(d))g=d.call(b,e,a[e],b[e])}if(c===k&&g&&ka(g))if(fa(g))g=new s(g.getTime());else if(D(g))g=new r(g.source,ta(g));else{a[e]||(a[e]=q.isArray(g)?[]:{});p.merge(a[e],b[e],c,d);continue}a[e]=
g}return a},values:function(a,b){var c=[];G(a,function(d,e){c.push(e);b&&b.call(a,e)});return c},clone:function(a,b){var c;if(!ka(a))return a;c=a instanceof na?new na:new a.constructor;return p.merge(c,a,b)},fromQueryString:function(a,b){var c=p.extended();a=a&&a.toString?a.toString():"";a.replace(/^.*?\?/,"").split("&").forEach(function(d){d=d.split("=");d.length===2&&Eb(c,d[0],decodeURIComponent(d[1]),b)});return c},tap:function(a,b){var c=b;A(b)||(c=function(){b&&a[b]()});c.call(a,a);return a},
has:function(a,b){return la(a,b)}});H(p,n,n,x,function(a,b){var c="is"+b;Cb.push(c);a[c]=function(d){return p.prototype.toString.call(d)==="[object "+b+"]"}});(function(){E(p,n,function(){return arguments.length===0},{extend:function(){xa(Cb.concat(Db),p)}})})();xa(Db,na);
...
(function(a){if(this.btoa){Fb=this.btoa;Gb=this.atob}else{var b=/[^A-Za-z0-9\+\/\=]/g;Fb=function(c){var d="",e,g,h,i,j,f,m=0;do{e=c.charCodeAt(m++);g=c.charCodeAt(m++);h=c.charCodeAt(m++);i=e>>2;e=(e&3)<<4|g>>4;j=(g&15)<<2|h>>6;f=h&63;if(isNaN(g))j=f=64;else if(isNaN(h))f=64;d=d+a.charAt(i)+a.charAt(e)+a.charAt(j)+a.charAt(f)}while(m<c.length);return d};Gb=function(c){var d="",e,g,h,i,j,f=0;if(c.match(b))throw Error("String contains invalid base64 characters");c=c.replace(/[^A-Za-z0-9\+\/\=]/g,"");
do{e=a.indexOf(c.charAt(f++));g=a.indexOf(c.charAt(f++));i=a.indexOf(c.charAt(f++));j=a.indexOf(c.charAt(f++));e=e<<2|g>>4;g=(g&15)<<4|i>>2;h=(i&3)<<6|j;d+=t.fromCharCode(e);if(i!=64)d+=t.fromCharCode(g);if(j!=64)d+=t.fromCharCode(h)}while(f<c.length);return d}}})("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=");
...
Z={acronym:function(a){Kb[a.toLowerCase()]=a;a=p.keys(Kb).map(function(b){return Kb[b]});Z.acronymRegExp=r(a.join("|"),"g")},plural:function(a,b){Nb(Hb,a,b)},singular:function(a,b){Nb(Ib,a,b)},irregular:function(a,b){var c=a.first(),d=a.from(1),e=b.first(),g=b.from(1),h=e.toUpperCase(),i=e.toLowerCase(),j=c.toUpperCase(),f=c.toLowerCase();Mb(Y,a);Mb(Y,b);if(j==h){Z.plural(new r("({1}){2}$".assign(c,d),"i"),"$1"+g);Z.plural(new r("({1}){2}$".assign(e,g),"i"),"$1"+g);Z.singular(new r("({1}){2}$".assign(e,
g),"i"),"$1"+d)}else{Z.plural(new r("{1}{2}$".assign(j,d)),h+g);Z.plural(new r("{1}{2}$".assign(f,d)),i+g);Z.plural(new r("{1}{2}$".assign(h,g)),h+g);Z.plural(new r("{1}{2}$".assign(i,g)),i+g);Z.singular(new r("{1}{2}$".assign(h,g)),j+d);Z.singular(new r("{1}{2}$".assign(i,g)),f+d)}},uncountable:function(a){var b=q.isArray(a)?a:F(arguments);Y=Y.concat(b)},human:function(a,b){Jb.unshift({ka:a,pa:b})},clear:function(a){if(Ob(a,"singulars"))Ib=[];if(Ob(a,"plurals"))Hb=[];if(Ob(a,"uncountables"))Y=[];
...
function gc(a,b,c,d){Yb||hc();var e=F(b).join(""),g=Yb[d];e=e.replace(/all/,"").replace(/(\w)lphabet|umbers?|atakana|paces?|unctuation/g,"$1");return a.replace(c,function(h){return g[h]&&(!e||e.has(g[h].type))?g[h].to:h})}
