...
function J(a,b){function d(g,h){g=g.split("+").map(function(c){return c.replace(/(.+):(.+)$/,function(i,m,l){return l.split("|").map(function(n){return m+n}).join("|")})}).join("|");return g.split("|").each(h)}function e(g,h,c){var i=[];if(b[g]){b[g].each(function(m,l){d(m,function(n,r){i[r*c+l]=n.toLowerCase()})});if(h)i=i.concat(b[g].map(function(m){return m.slice(0,3).toLowerCase()}));return b[g]=i}}function f(g,h){var c="[0-9\uff10-\uff19]"+(g?"{"+g+","+h+"}":"+");if(b.digits)c+="|["+b.digits+
...
m=h.match(/^\d+$/),l=h.replace(/[^a-z]+$/,""),n,r;if(l==="time"){e=e.concat(v);return c?"\\s*(?:(?:t|at |\\s+)(\\d{1,2}):?(\\d{2})?:?(\\d{2})?(?:\\.(\\d{1,6}))?(am|pm)?(?:(Z)|(?:([+-])(\\d{2})(?::?(\\d{2}))?)?)?)?":"(\\d{1,2}):?(\\d{2})?:?(\\d{2})?(?:\\.(\\d{1,6}))?(am|pm)?(?:(Z)|(?:([+-])(\\d{2})(?::?(\\d{2}))?)?)?"}if(m)n=f.optionals[m[0]];else if(f[l])n=f[l];else if(f[l+"s"]){n=f[l+"s"];if(i){r=[];n.each(function(V,W){var K=W%(f.units?8:n.length);if(K>=i[1]&&K<=(i[2]||i[1]))r.push(V)});n=r}n=n.compact().join("|")}if(m)return"(?:"+
n+")?";else{e.push(l);return"("+n+")"+(c?"?":"")}});s.addFormat(a,e,b)}});function O(a){var b;if(q.isObject(a[0]))return a;else if(a.length==1&&q.isNumber(a[0]))return[a[0]];b={};A.each(function(d,e){b[d.a]=a[e]});return[b]}function P(a,b){if(b!="date"&&b!="month"&&b!="year")return a;return a.replace(y,function(d){return w.indexOf(d)+1||""})}
