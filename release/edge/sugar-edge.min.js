...
function sb(a,b,c,d){var e=new s,f=n,h,i,j,g,m,o,v,y,J;e.utc(d);if(fa(a))e.utc(a.isUTC()).setTime(a.getTime());else if(B(a))e.setTime(a);else if(M(a)){e.set(a,k);g=a}else if(C(a)){h=lb(b);a=qb(a);h&&G(h.sa(),function(L,P){var Da=a.match(P.Aa);if(Da){j=P;i=j.xa;g=pb(Da,j.to,i);g.utc&&e.utc();i.ma=j;if(g.timestamp){g=g.timestamp;return n}if(j.Ba&&!C(g.month)&&(C(g.date)||h.va(b))){y=g.month;g.month=g.date;g.date=y}if(g.year&&g.Ca.length===2)g.year=N(V(new s,"FullYear")/100)*100-N(g.year/100)*100+g.year;
if(g.month){g.month=i.getMonth(g.month);if(g.shift&&!g.unit)g.unit=i.units[7]}if(g.weekday&&g.date)delete g.weekday;else if(g.weekday){g.weekday=i.getWeekday(g.weekday);if(g.shift&&!g.unit)g.unit=i.units[5]}if(g.day&&(y=i.ha[g.day])){g.day=y.value;e.reset();f=k}else if(g.day&&(o=i.getWeekday(g.day))>-1){delete g.day;if(g.num&&g.month){J=function(){var S=e.getWeekday();e.setWeekday(7*(g.num-1)+(S>o?o+7:o))};g.day=1}else g.weekday=o}if(g.date&&!B(g.date))g.date=i.ta(g.date);if(i.za(g.ampm)&&g.hour<
12)g.hour+=12;else if(i.ya(g.ampm)&&g.hour===12)g.hour=0;if("offset_hours"in g||"offset_minutes"in g){e.utc();g.offset_minutes=g.offset_minutes||0;g.offset_minutes+=g.offset_hours*60;if(g.offset_sign==="-")g.offset_minutes*=-1;g.minute-=g.offset_minutes}if(g.unit){f=k;v=i.oa(g.num);m=i.ra(g.unit);if(g.shift||g.edge){v*=(y=i.ha[g.shift])?y.value:0;if(m==="month"&&I(g.date)){e.set({day:g.date},k);delete g.date}if(m==="year"&&I(g.month)){e.set({month:g.month,day:g.date},k);delete g.month;delete g.date}}if(g.sign&&
(y=i.ha[g.sign]))v*=y.value;if(I(g.weekday)){e.set({weekday:g.weekday},k);delete g.weekday}g[m]=(g[m]||0)+v}if(g.year_sign==="-")g.year*=-1;ab.slice(1,4).forEach(function(S,Ub){var zb=g[S.$],Ab=zb%1;if(Ab){g[ab[Ub].$]=N(Ab*(S.$==="second"?1E3:60));g[S.$]=pa(zb)}});return n}});if(j)if(f)e.advance(g);else{e._utc&&e.reset();tb(e,g,k,n,c)}else{e=a?new s(a):new s;d&&e.addMinutes(e.getTimezoneOffset())}if(g&&g.edge){y=i.ha[g.edge];G(ab.slice(4),function(L,P){if(I(g[P.$])){m=P.$;return n}});if(m==="year")g.fa=
"month";else if(m==="month"||m==="week")g.fa="day";e[(y.value<0?"endOf":"beginningOf")+hb(m)]();y.value===-2&&e.reset()}J&&J();e.utc(n)}return{ea:e,set:g}}function gb(a){var b,c=w.abs(a),d=c,e=0;ab.slice(1).forEach(function(f,h){b=pa(N(c/f.da()*10)/10);if(b>=1){d=b;e=h+1}});return[d,e,a]}
...
f}return a},values:function(a,b){var c=[];G(a,function(d,e){c.push(e);b&&b.call(a,e)});return c},clone:function(a,b){var c;if(!ka(a))return a;c=a instanceof na?new na:new a.constructor;return p.merge(c,a,b)},fromQueryString:function(a,b){var c=p.extended();a=a&&a.toString?a.toString():"";a.replace(/^.*?\?/,"").split("&").forEach(function(d){d=d.split("=");d.length===2&&Eb(c,d[0],decodeURIComponent(d[1]),b)});return c},tap:function(a,b){var c=b;A(b)||(c=function(){b&&a[b]()});c.call(a,a);return a},
