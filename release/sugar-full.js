...
(function() {
  ...
  Inflector = {
    ...
    'irregular': function(singular, plural) {
      ...
      if (singularFirstUpper == pluralFirstUpper) {
        Inflector.plural(new regexp(withCapture(singularFirst, singularRest), 'i'), '$1' + pluralRest);
        Inflector.plural(new regexp(withCapture(pluralFirst, pluralRest), 'i'), '$1' + pluralRest);
        Inflector.singular(new regexp(withCapture(pluralFirst, pluralRest), 'i'), '$1' + singularRest);
      } else {
        Inflector.plural(new regexp(noCapture(singularFirstUpper, singularRest)), pluralFirstUpper + pluralRest);
        Inflector.plural(new regexp(noCapture(singularFirstLower, singularRest)), pluralFirstLower + pluralRest);
        Inflector.plural(new regexp(noCapture(pluralFirstUpper, pluralRest)), pluralFirstUpper + pluralRest);
        Inflector.plural(new regexp(noCapture(pluralFirstLower, pluralRest)), pluralFirstLower + pluralRest);
        Inflector.singular(new regexp(noCapture(pluralFirstUpper, pluralRest)), singularFirstUpper + singularRest);
        Inflector.singular(new regexp(noCapture(pluralFirstLower, pluralRest)), singularFirstLower + singularRest);
      }
    },
  ...
  defineInstance(sugarString, {
    ...
    'parameterize': function(str, separator) {
      ...
      if (separator) {
        str = str.replace(new regexp('^{s}+|{s}+$|({s}){s}+'.split('{s}').join(escapeRegExp(separator)), 'g'), '$1');
      }
  ...
  function buildWidthConversionTables() {
    ...
    for (var i = 0; i < zenkakuKatakana.length; i++) {
      var c = zenkakuKatakana.charAt(i);
    ...
    for (var i = 0; i < zenkakuPunctuation.length; i++) {
      setWidthConversion('p', hankakuPunctuation.charAt(i), zenkakuPunctuation.charAt(i));
    }
