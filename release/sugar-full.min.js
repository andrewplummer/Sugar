...
function Dd(a,b,c,d){if(Ga(b))return a;Cd(b,function(e){var f,g=Ad(b,e);f=Ad(a,e);var h=g&&g.value,m=f&&f.value,p=L(h),p=c&&p;if(N(f)&&null!=f.value){if(!p&&!1===d)return;K(d)&&(f=d.call(b,e,m,h),N(f)&&(g.value=f,p=!1))}p&&(Ba(h)?h=new y(h.getTime()):Ca(h)?h=new x(h.source,ab(h)):(L(m)||(m=J(h)?[]:{}),h=Dd(m,h,c,d)),g.value=h);yd(a,e,g)});return a}function Ed(){var a=n.prototype;qd.concat(rd).forEach(function(b){a[b]&&delete a[b]})}
