  ...
  var defineProperty = Object.defineProperties && Object.defineProperty || definePropertyShim;
  ...
  function setupNamespace(i, name) {
    ...
    setProperty(namespace, 'alias', function(fromName, toName) {
      setProperty(namespace, toName, namespace[fromName], true);
    });

    setProperty(namespace, 'defineStatic', function(methods, flags) {
      defineMethods(namespace, methods, flags);
    });

    setProperty(namespace, 'defineStaticWithArguments', function(methods, flags) {
      defineMethods(namespace, methods, flags, false, true);
    });

    setProperty(namespace, 'defineInstance', function(methods, flags) {
      defineMethods(namespace, methods, flags, true);
    });

    setProperty(namespace, 'defineInstanceWithArguments', function(methods, flags) {
      defineMethods(namespace, methods, flags, true, true);
    });

    setProperty(namespace, 'defineStaticPolyfill', function(methods) {
      extendNative(nativeClass, methods, true);
    });

    setProperty(namespace, 'defineInstancePolyfill', function(methods) {
      extendNative(nativeClass.prototype, methods, true);
    });
  ...
  function defineMethods(sugarNamespace, methods, flags, instance, args) {
