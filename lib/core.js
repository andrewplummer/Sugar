...
function disambiguateMethod(methodName) {
  var fn = function() {
    ...
    if (!sugarNamespace) {
      // If there is no namespace, throw an error.
      throwDisambiguationError(raw);
    }

    var fn = new sugarNamespace(raw)[methodName];

    if (fn.disambiguate) {
      // Alternately, if the method about to be called on
      // this chainable is itself a disambiguation method
      // then throw an error to prevent infinite recursion.
      throwDisambiguationError(raw);
    }
...
function throwDisambiguationError(raw) {
  throw new TypeError('Cannot determine namespace for ' + raw);
}
