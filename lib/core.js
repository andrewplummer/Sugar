...
function setupGlobal() {
  ...
  Sugar = function(arg) {
    iterateOverObject(Sugar, function(name, namespace) {
      // Although only the only enumerable properties on the global
      // object are Sugar namespaces, environments that can't set
      // non-enumerable properties will step through the utility methods
      // as well here, so use this check to only allow true namespaces.
      if (hasOwn(namespacesByName, name)) {
        namespace.extend(arg);
      }
    });
  };
 ...
 *        new Sugar.Namespace() or Sugar.Namespace(). Lastly, the global method
 *        `Sugar.extend()` allows end users to extend native prototypes with
 *        defined methods.
 ***/
function createNamespace(name) {

  // The namespace is also a chainable class: Sugar.Array, etc.
  var namespace = getNewChainableClass(name);
  ...
  setProperty(namespace, 'defineInstancePolyfill', function(arg1, arg2, arg3) {
    ...
    iterateOverObject(opts.methods, function(methodName, fn) {
      setMethodOnChainable(namespace, methodName, fn);
    });
  });
  ...
  namespacesByClassName['[object ' + name + ']'] = namespace;
...
function setMethodOnChainable(sugarNamespace, methodName, fn) {
  ...
  // If the method already exists on the default chainable and is not native,
  // then a collision exists, so set the method to a disambiguation function
  // that will lazily evaluate the object and find it's associatd chainable.
...
function disambiguateMethod(methodName) {
  return function() {
    var raw = this.raw, ns;
    if (raw != null) {
      ns = namespacesByClassName[className(raw)];
    }
    if (!ns) {
