...
(function() {
  ...
  function arrayUnique(arr, map) {
    return arr.reduce(function(result, el, index) {
      var prop = transformArgument(el, map, arr, [el, index, arr]), none;
      none = result.none(function(el, i, arr) {
        return object.equal(transformArgument(el, map, arr, [el]), prop);
      });
      if(none) {
        result.push(el);
      }
