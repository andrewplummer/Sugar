...
(function() {
  ...
  function getCollationValue(str, index) {
    var char = str.charAt(index), eq = array[AlphanumericSortEquivalents] || {};
    if(!char) {
      return null;
    } else {
      char = eq[char] || char;
      return array[AlphanumericSortOrder].indexOf(char);
    }
  }
  ...
  function buildArray() {
    var order = 'AÁÀÂÃĄBCĆČÇDĎÐEÉÈĚÊËĘFGĞHIÍÌİÎÏJKLŁMNŃŇÑOÓÒÔPQRŘSŚŠŞTŤUÚÙŮÛÜVWXYÝZŹŻŽÞÆŒØÕÅÄÖ';
    var equiv = 'AÁÀÂÃÅÄĄ,CĆČÇ,DĎÐ,EÉÈĚÊËĘ,GĞ,IÍÌİÎÏ,LŁ,NŃŇÑ,OÓÒÔØÕÖ,RŘ,SŚŠßŞ,TŤ,UÚÙŮÛÜ,YÝ,ZŹŻŽ'
