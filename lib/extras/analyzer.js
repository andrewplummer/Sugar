...
var SugarPrototypeMethods = [
  ...
  methods: [
    ...
    },
    {
      ...
      sugar_compatibility: 1,
      sugar_notes: 'ES6 Array.from natively creates arrays from array-like collections. Sugar provides a polyfill for this method.',
      original_code: '$A(arguments)',
      sugar_code: 'Array.from(arguments)',
      ref: 'Array/from'
    },
    {
      name: '$H',
...
},
{
  ...
  },
  {
    ...
    sugar_compatibility: 3,
    sugar_notes: 'String#interpolate exists in Sugar as String#format, with a slightly different syntax.',
    original_code: "'i like #{fruit}'.interpolate({ fruit: 'peaches' })",
    sugar_code: "'i like {fruit}'.format({ fruit: 'peaches' })",
    ref: 'String/format'
  },
  {
    name: 'isJSON',
...
var SugarUnderscoreMethods = [
  ...
  },
  {
    ...
    sugar_compatibility: 2,
    sugar_notes: '_.isEqual#abs exists in Sugar and is identical.',
    original_code: '_.isEqual(obj1, obj2)',
    sugar_code: 'Object.isEqual(obj1, obj2)',
    ref: 'Object/isEqual'
  },
  {
    name: 'isElement',
  ...
  },
  {
    ...
    sugar_compatibility: 2,
    sugar_notes: '_.isArguments does not exist in Sugar. A simple check of the "callee" parameter may be enough to simulate this (and is also cross-browser). Note that Sugar does have Array.from(), which will convert an arguments object into a standard array.',
  ...
  },
  {
    ...
    sugar_compatibility: 1,
    sugar_notes: '_.template exists in Sugar as String#format with slightly different syntax. Although it does not have the complex "eval" functionality of Underscore, it can be useful for quickly assigning a value inside a string. String#format will also accept numbers for arguments passed.',
    original_code: "_.template('hello: <%= name %>')({ name: 'joe' })",
    sugar_code: "'hello: {name}'.format({ name: 'joe' })",
    ref: 'String/format'
  }
  ]
}
  ...
  var warn = function(message, stackLevel, skipMeta, docs, logLevel) {
    ...
    if (stack) {
      ...
      if (!isConsole && (!file || file.match(RegExp('(' + baseExcludePackages.concat(SUGAR_ANALYZER_EXCLUDES).join('|') + ')[^\/]*\.js')))) {
        return;
      }
      warned[message] = true;
      if (!skipMeta) {
