...
//- Level: Moderate
  ...
  var METHODS = [
    ...
    },
    {
      ...
      message: 'Object.select now, when passed an object as a matcher, only checks for key existence, not whether values also match or not. To do the "intersect" operation that these methods previously performed, use Object.intersect instead.',
...
//- Level: Major
  ...
  function initMethod(method, id) {
    ...
    function wrapMethod(target, name) {
      ...
      function withSugarNotification() {
        ...
        if (!isNotifying && canNotify(this, arguments, id)) {
          ...
          if (fn.name !== 'withSugarNotification') {
    ...
    if (method.sName) {
      wrapMethod(ns, method.sName);
    }
    if (method.iName) {
      wrapMethod(ns.prototype, method.iName);
    }
