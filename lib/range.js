...
Range.prototype = {
   ...
   *   Date.range('1 milliseconds ago', 'now').toArray() -> [1ms ago, now]
   *
   ***/
  'toArray': function() {
    return this.every();
...
defineStatic(sugarNumber, {
   ...
   * @returns Range
   * @static
   * @short Creates a new range between [start] and [end]. See `range methods` for more.
   *
   * @example
   *
   *   Number.range(5, 10)
...
defineStatic(sugarDate,   {
   ...
   *        current date. See `range methods` for more.
   *
   * @example
   *
   *   Date.range()
   *   Date.range('last Monday', undefined)
   *   Date.range(undefined, 'next Monday')
