  ...
  function extend(klass, methods, instance, polyfill, override) {
    ...
    iterateOverObject(methods, function(name, extendedFn) {
      ...
      if(canDefineOnNative(klass, polyfill, nativeFn, override)) {
  ...
  function canDefineOnNative(klass, polyfill, nativeFn, override) {
    if(override) {
      return true;
  ...
  function defineProxy(klass, name, instance, nativeFn, extendedFn, existed) {
    ...
    defineProperty(getProxy(klass), name, fn, true);
