  ...
  function extend3(globalNamespace, source, instance, polyfill) {
    var extendee = instance ? globalNamespace.prototype : globalNamespace;
  ...
  function namespaceExtend(name, methods) {
    var sugarNamespace = Sugar[name], staticMethods = {}, instanceMethods = {};
    iterateOverObject(methods || sugarNamespace, function(name, method) {
      if (typeof method === 'string') {
        method = sugarNamespace[name];
      }
      if (method.instance) {
        instanceMethods[name] = method.instance;
      } else {
        staticMethods[name] = method;
      }
    });
    extend3(globalContext[name], staticMethods);
    extend3(globalContext[name], instanceMethods, true);
  }
  ...
  function defineStaticProperties(sugarNamespace, properties) {
    // For now, as long as the "instance" and "args" flags are false,
    // calling "defineMethods" will simply set the properties.
    defineMethods(sugarNamespace, properties);
  }
