  ...
  function getExtendedDate(f, localeCode, prefer) {
    ...
    if(isDate(f)) {
      ...
      if(baseLocalization) {
        baseLocalization.getFormats().forEach(function(dif) {
      ...
      if(set && set['edge']) {
        ...
        DateUnitsReversed.slice(4).forEach(function(u) {
  ...
  function updateDate(d, params, reset, utc, advance, prefer) {
    ...
    DateUnitsReversed.forEach(function(u) {
  ...
  function buildDateOutputShortcuts() {
    var methods = {};
    ['short','long','full'].forEach(function(name) {
  ...
  function buildRelativeAliases() {
    ...
    ['today','yesterday','tomorrow','weekday','weekend','future','past'].concat(weekdays).concat(months).forEach(function(s) {
      methods['is'+ simpleCapitalize(s)] = function() {
        return this.is(s);
      };
    });
    date.extend(methods);
