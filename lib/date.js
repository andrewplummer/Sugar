  ...
  Locale.prototype = {
    ...
    getNumeralValue: function(str) {
      ...
      for (var i = arr.length - 1; numeral = arr[i]; i--) {
        digit = this.numeralMap[numeral];
        if (isUndefined(digit)) {
          digit = fullWidthNumberMap[numeral];
        }
    ...
    addFormat: function(src, to) {
      ...
      function getTokenSrc(str) {
        ...
        if (nc) {
          ...
          val = loc.parseTokens[key] || loc[key];
          ...
          if (!val) {
            val = loc.parseTokens[key] || loc[key + 's'];
          }
    ...
    init: function(def) {
      ...
      function buildTimeFormat() {
        var src;
        if (loc.ampmFront) {
          // The AMPM first format also presumes time suffixes exist,
          // so that it can make mm:ss optional without a more complex regex.
      ...
      function buildModifiers() {
        forEach(loc.modifiers, function(modifier) {
          ...
          forEachAlternate(modifier.src, function(alt, j) {
            var token = loc.parseTokens[name], val = modifier.value;
            map[alt] = val;
      ...
      function getTimeAfter() {
        ...
        localized = arrayToRegAlternates(loc.timeMarker);
        if (localized) {
          markers += '| (?:' + localized + ') ';
        }
