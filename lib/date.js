   ...
   * @extra [d] will accept a date object, timestamp, or text format. If not specified, [d] is assumed to be now. [locale] can be passed to specify the locale that the date is in. `[unit]Ago` is provided as an alias to make this more readable when [d] is assumed to be the current date.
   *
   * @set
   *   millisecondsSince
   *   secondsSince
   ...
   * @extra [d] will accept a date object, timestamp, or text format. If not specified, [d] is assumed to be now. [locale] can be passed to specify the locale that the date is in. `[unit]FromNow` is provided as an alias to make this more readable when [d] is assumed to be the current date.
   *
   * @set
   *   millisecondsUntil
   *   secondsUntil
  ...
  extend(Date, {
     ...
     * @extra If no argument is given, date is assumed to be now. `Date.create` additionally can accept enumerated parameters as with the standard date constructor. [locale] can be passed to specify the locale that the date is in. When unspecified, the current locale (default is English) is assumed. UTC-based dates can be created through the `utc` object.
     * @set
     *   Date.utc.create
     *
     * @example
     ...
     * @extra For example `"Sunday"` can be either "the Sunday coming up" or "the Sunday last" depending on context. Note that dates explicitly in the future ("next Sunday") will remain in the future. This method simply provides a hint when ambiguity exists. UTC-based dates can be created through the `utc` object.
     * @set
     *   Date.utc.past
     *
     * @example
     ...
     * @extra For example `"Sunday"` can be either "the Sunday coming up" or "the Sunday last" depending on context. Note that dates explicitly in the past ("last Sunday") will remain in the past. This method simply provides a hint when ambiguity exists. UTC-based dates can be created through the `utc` object.
     * @set
     *   Date.utc.future
     *
     * @example
     ...
     * @extra Sugar has support for 13 locales that are available through the "Date Locales" package. In addition you can define a new locale with `Date.addLocale`.
     *
     ***/
     ...
     * @extra This method allows fine grained control for alternate formats. <format> is a string that can have regex tokens inside. <match> is an array of the tokens that each regex capturing group will map to, for example `year`, `date`, etc.
     *
     **/
  ...
  extend(Date, {
     ...
     * @extra This method can accept multiple formats including an object, a string in the format `3 days`, a single number as milliseconds, or enumerated parameters (as with the Date constructor). If [reset] is `true`, any units more specific than those passed will be reset.
     * @example
     *
     ...
     * @extra [margin] is to allow extra margin of error (in ms). <d> will accept a date object, timestamp, or text format. If not specified, <d> is assumed to be now.
     * @example
     *
     ...
     * @extra [margin] is to allow extra margin of error (in ms). <d1> and <d2> will accept a date object, timestamp, or text format. If not specified, they are assumed to be now.
     * @example
     *
     ...
     * @extra <format> can be a number of pre-determined formats or a string of tokens. Locale-specific formats are `short`, `long`, and `full` which have their own aliases and can be called with `date.short()`, etc. If <format> is not specified the `long` format is assumed. [locale] specifies a locale code to use (if not specified the current locale is used).
     *
     * @set
     *   short
     *   long
     ...
     * @extra [fn] can be passed to provide for more granular control over the resulting string. [fn] is passed 4 arguments: the adjusted value, unit, offset in milliseconds, and a localization object. As an alternate syntax, [locale] can also be passed as the first (and only) parameter.
     * @example
     *
     ...
     * @extra <f> will accept a date object, timestamp, or text format. `is` additionally understands more generalized expressions like month/weekday names, 'today', etc, and compares to the precision implied in <f>. [margin] allows an extra margin of error in milliseconds. [utc] will treat the compared date as UTC.
     * @example
     *
   ...
   * @extra [d] will accept a date object, timestamp, or text format. Note that "months" is ambiguous as a unit of time. If the target date falls on a day that does not exist (ie. August 31 -> February 31), the date will be shifted to the last day of the month. Be careful using `monthsBefore` if you need exact precision.
   *
   * @set
   *   millisecondBefore
   *   millisecondsBefore
   ...
   * @extra [d] will accept a date object, timestamp, or text format. Note that "months" is ambiguous as a unit of time. If the target date falls on a day that does not exist (ie. August 31 -> February 31), the date will be shifted to the last day of the month. Be careful using `monthsAfter` if you need exact precision.
   *
   * @set
   *   millisecondAfter
   *   millisecondsAfter
