  ...
  function getAdjustedUnit(ms, fn) {
    ...
    iterateOverDateUnitsReverse(function(unit, i) {
      value = abs(fn(unit));
      if (value >= 1) {
        unitIndex = 7 - i;
        return false;
  ...
  function updateDate(d, params, reset, advance, prefer, weekdayForward) {
    ...
    iterateOverDateUnits(function(name, unit, i) {
      ...
      if (uniqueParamExists(name, isDay)) {
        ...
        callDateSet(d, unit.method, unit.resetValue || 0);
      }
    });

    // Now actually set or advance the date in order, higher units first.
    iterateOverDateUnitsReverse(function(u, i) {
