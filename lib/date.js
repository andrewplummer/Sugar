  ...
  var formatTokensBase = [
    ...
    },
    {
      ...
      ldmlPaddedToken: 'DDD',
      get: function(d) {
        var s = resetDateUnit(cloneDate(d), 'month');
  ...
  function resetDateUnit(d, resetUnit) {
    resetUnit = resetUnit || 'hours';
    if (isValidDateUnit(resetUnit)) {
      iterateOverDateUnits(function(name, u) {
        if (name !== 'week') {
          callDateSet(d, u.method, u.resetValue || 0);
        }
        return !matchUnitName(resetUnit, name);
      });
    }
    return d;
  }

  function isValidDateUnit(unit) {
    return some(dateUnits, function(u) {
      return matchUnitName(unit, u.name);
    });
  }

  function resetDateUnitWithWeek(d, unit) {
    if (matchUnitName(unit, 'week')) {
      setISOWeekNumber(d, 1);
      return resetHours(d);
    }
    return resetDateUnit(d, unit);
  }
  ...
  defineInstance(sugarDate, {
    ...
    'reset': function(d, unit) {
      return resetDateUnitWithWeek(d, unit);
    },
