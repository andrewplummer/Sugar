...
var CoreParsingFormats = [
  ...
  {
    // ISO-8601
    src: '{yearSign?}{yyyy}(?:-?{MM}(?:-?{dd}(?:T{isoTimeWithZone})?)?)?'
  }
];
...
var FormatTokensBase = [
  ...
  },
  {
    ...
    strf: 'p',
    get: function(d, localeCode) {
      return getMeridianToken(d, localeCode);
    }
  },
  {
    ldml: 'tt',
    strf: 'P',
    get: function(d, localeCode) {
      return getMeridianToken(d, localeCode).toLowerCase();
    }
  },
  {
    ldml: 'T',
    lowerToken: 't',
    get: function(d, localeCode) {
      return getMeridianToken(d, localeCode).charAt(0);
    }
  },
  {
    ldml: 's,seconds',
...
function moveToEndOfUnit(d, unitIndex, localeCode) {
  if (unitIndex === WEEK_INDEX) {
    moveToEndOfWeek(d, localeManager.get(localeCode).getFirstDayOfWeek());
  }
  return setUnitAndLowerToEdge(d, getLowerUnitIndex(unitIndex), true);
}

function setUnitAndLowerToEdge(d, startIndex, end) {
  walkUnitDown(startIndex, function(unit) {
...
function getMeridianToken(d, localeCode) {
...
function compareDate(date, d, margin, localeCode, options) {
  var loMargin = 0, hiMargin = 0, compareEdges, override, min, max, p, t;
  ...
  min = p.date.getTime();
  max = max || min;
  var timezoneShift = getTimezoneShift();
  if (timezoneShift) {
    min -= timezoneShift;
    max -= timezoneShift;
  }
...
function getExtendedDate(contextDate, d, opt) {
  ...
  function getFormatParams(match, dif) {
    ...
    forEach(dif.to, function(field, i) {
      ...
      if (field === 'yy' || field === 'y') {
        field = 'year';
        val = getYearFromAbbreviation(str, date, options.prefer);
  ...
  function handleSpecialTokens(params) {
  ...
  function handleRelativeUnit(unitIndex) {
    ...
    if (isDefined(set.shift)) {
      ...
      num *= set.shift;
    } else if (isDefined(set.sign)) {
      // Unit and sign, ie "months ago", "weeks from now", etc.
      num *= set.sign;
    }

    if (unitIndex === DAY_INDEX && isDefined(set.day)) {
      num += set.day;
      delete set.day;
    }
  ...
  function handleEdge(edge, params) {
    ...
    afterDateSet(function() {
      ...
      if (edge < 0) {
        moveToBeginningOfUnit(date, edgeIndex, options.locale);
      } else if (edge > 0) {
        moveToEndOfUnit(date, edgeIndex, options.locale);
        if (edge === 1) {
          resetTime(date);
        }
      }
  ...
  if (isDate(d)) {
    ...
    for (var i = 0, dif, match; dif = loc.compiledFormats[i]; i++) {
      ...
      if (match) {
        ...
        handleSpecialTokens(getFormatParams(match, dif));
        break;
      }
    }
  ...
  Locale.prototype = {
    ...
    init: function(def) {
      ...
      function buildTimeFormats() {
        loc.parseAliases['time'] = getTimeFormat();
        loc.parseAliases['timeWithZone'] = getTimeWithZoneFormat();
        loc.parseAliases['isoTimeWithZone'] = getTimeWithZoneFormat(true);
      }
      ...
      function getTimeWithZoneFormat(iso) {
        var i = iso ? 'i' : '';
        var time = '{'+i+'hh}(?::?{'+i+'mm}(?::?{ss})?)?';
        var zone = '(?:{Z}|{GMT?}(?:{offsetSign}{offsetHour}(?::?{offsetMinute}(?: \\([\\w\\s]+\\))?)?)?)?';
        return [time, zone].join(' ');
      }

      function buildParsingTokens() {
        forEachProperty(LocalizedParsingTokens, function(name, token) {
          var src, arr;
...
function buildDateUnitMethods() {
  ...
  defineInstanceSimilar(sugarDate, DateUnits, function(methods, unit, index) {
    ...
    var since = function(date, d, options) {
      return getTimeDistanceForUnit(date, createDateWithContext(date, d, options), unit);
    };
    var until = function(date, d, options) {
      return getTimeDistanceForUnit(createDateWithContext(date, d, options), date, unit);
    };
...
defineInstance(sugarDate, {
  ...
  'endOfISOWeek': function(d) {
    ...
    return moveToEndOfUnit(d, DAY_INDEX);
  },
   ...
   *
   ***/
  'reset': function(d, unit, localeCode) {
    var unitIndex = unit ? getUnitIndexForParamName(unit) : DAY_INDEX;
    moveToBeginningOfUnit(d, unitIndex, localeCode);
    return d;
  },
...
function buildNumberUnitMethods() {
  defineInstanceSimilar(sugarNumber, DateUnits, function(methods, unit) {
    ...
    after = function(n, d, options) {
      return advanceDate(createDate(d, options), name, n);
    };
