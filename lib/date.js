  ...
  function setLocalization(localeCode, set) {
    ...
    loc.parse.forEach(function(src) {
      loc.addFormat(src);
    });
  ...
  function fullCompareDate(d, f, margin) {
    ...
    if (isString(f)) {
      ...
      switch(true) {
        ...
        case f === 'today':     return compareDay(d);
        case f === 'tomorrow':  return compareDay(d,  1);
        case f === 'yesterday': return compareDay(d, -1);
        case f === 'weekday':   return getWeekday(d) > 0 && getWeekday(d) < 6;
        case f === 'weekend':   return getWeekday(d) === 0 || getWeekday(d) === 6;
        case (tmp = English.weekdays.indexOf(f) % 7) > -1: return getWeekday(d) === tmp;
        case (tmp = English.months.indexOf(f) % 12) > -1:  return getMonth(d) === tmp;
      }
    }
