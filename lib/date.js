  ...
  function buildDateUnitMethods() {
    ...
    defineInstanceSimilarWithArguments(sugarDate, dateUnits, function(methods, u, i) {
      ...
      function getDistance(d1, d2) {
        ...
        // If the unit is "days" or higher, then there is potential ambiguity.
        // The numeric calculation serves as a good starting point, then iterate
        // until we pass the compared date 
        if (i < 4) {
          sugarDate[u.addMethod](d1, n);
          while (d1 < d2) {
            sugarDate[u.addMethod](d1, 1);
            if (d1 > d2) {
              break;
            }
            n += 1;
