...
function getExtendedDate(contextDate, d, opt) {
  ...
  function handleRelativeUnit(loc) {
    ...
    if (set.half) {
      if (unitIndex === YEAR_INDEX) {
        num = 6;
        unitIndex = MONTH_INDEX;
        unitName = 'months';
      } else if (unitIndex > DAY_INDEX) {
        invalidateDate(date);
      } else {
        num = .5;
      }
 ...
 *
 ***
 * @method add[Unit]s(<n>, [reset] = false)
 * @returns Date
 ...
 *        that doesn't exist (ie. February 30). In this case the date will be
...
defineStatic(sugarDate, {
   ...
   * @extra The locale object has various properties that can be manipulated to
   *        provide more control over dates, however it differs from the locale
   *        definition object in that it has various "compiled" properties. If
   *        possible, it is better to modify the locale definition object instead,
   *        as this is more straight forward. For more, see `date locales`.
   *
   * @example
   *
...
defineInstanceWithArguments(sugarDate, {
   ...
   *        If a month is set to a date that does not exist, it will rewind to
   ...
   * @extra This method accepts multiple formats including an object, a string
...
defineInstance(sugarDate, {
   ...
   *        also be passed in place of [fn]. For more, see `date output formats`.
   *
   * @callback fn
   *
   ...
   *   jan.relative(function(val, unit, ms, loc) {
...
function getLocaleManager(loc) {
  ...
  LocaleManager.prototype = {
    ...
    add: function(code, def) {
      ...
      var loc = def.compiledFormats ? def : getLocale(def);
...
var EnglishLocaleBaseDefinition = {
  ...
  'articles':   'a,an,the',
  'tokens':     'the,st|nd|rd|th,of|in,a|an',
  'time':       '{H}:{mm}',
  'past':       '{num} {unit} {sign}',
...
var English = getLocale(AmericanEnglishDefinition);
var localeManager = getLocaleManager(English);
