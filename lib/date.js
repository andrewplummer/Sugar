  ...
  function getExtendedDate(contextDate, d, opt) {
    ...
    var date, relative, baseLocalization, afterCallbacks, loc, set, unit, unitIndex, weekday, num, tmp, weekdayForward;
    ...
    if (isDate(d)) {
      ...
      baseLocalization = getLocale(options.locale);
  ...
  function fullCompareDate(date, d, margin) {
    ...
    if (isString(d)) {
      ...
      switch(true) {
        ...
        case (tmp = English.weekdays.indexOf(d) % 7) > -1: return getWeekday(date) === tmp;
        case (tmp = English.months.indexOf(d) % 12) > -1:  return getMonth(date) === tmp;
      }
    }
