  ...
  // Date units without weeks.
  var dateUnitsNoWeek;

  // Date units in reverse order, milliseconds to years.
  var dateUnitsReverse;

  // Numeric and base formats that are basic to all locales.
  var coreDateFormats = [];
  ...
  function collectParamsFromArguments(args) {
    ...
    while (isNumber(args[0])) {
      obj[dateUnitsNoWeek[i++].name] = args[0];
      args.splice(0, 1);
    }
    args.unshift(obj);
    return args;
  ...
  // Years -> Milliseconds
  function iterateOverDateUnits(fn, arr, from, to) {
    var i, unit;
    arr = arr || dateUnits;
    if (isUndefined(to)) to = arr.length;
    for(i = from || 0; i < to; i++) {
      unit = arr[i];
      if (fn(unit.name, unit, +i, arr) === false) {
        break;
      }
    }
  }
  ...
  // Millseconds -> Years
  function iterateOverDateUnitsReverse(fn, from) {
    iterateOverDateUnits(fn, dateUnitsReverse, from);
  }

  // Days -> Years
  function iterateOverHigherDateUnits(fn) {
    iterateOverDateUnits(fn, dateUnitsReverse, 4);
  }

  // Seconds -> Hours
  function iterateOverLowerDateUnits(fn) {
    iterateOverDateUnits(fn, dateUnitsReverse, 1, 4);
  }

  // Years -> Milliseconds (No Weeks)
  function iterateOverDateUnitsNoWeek(fn, from) {
    iterateOverDateUnits(fn, dateUnitsNoWeek, from);
  }
  ...
  function resetUnitAndLower(d, unit) {
    return setUnitAndLowerToEdge(d, unit);
  }
  ...
  // TODO create hash for 's' to quickly access units by name??
  function setUnitAndLowerToEdge(d, unit, end) {
    var specificityIndex;

    function getValue(u) {
      var val = end ? u.end : u.start;
      return isFunction(val) ? val(d) : val;
    }

    unit = unit || 'hours';

    iterateOverDateUnitsNoWeek(function(name, u, i) {
      if (matchUnitName(unit, name)) {
        specificityIndex = i;
        return false;
      }
    });

    if (isDefined(specificityIndex)) {
      iterateOverDateUnitsNoWeek(function(name, u) {
        callDateSet(d, u.method, getValue(u));
      }, specificityIndex);
    }
    return d;
  }

  function matchUnitName(input, u) {
    return input === u || input === u + 's' || (u === 'day' && input === 'date');
  }

  function getLowerUnitName(higherUnit) {
    var match;
    iterateOverDateUnitsNoWeek(function(name, u, i, arr) {
      if (matchUnitName(higherUnit, name)) {
        match = arr[i + 1].name;
      }
      return !match;
    });
  ...
  function moveToEdgeOfUnit(d, unit, localeCode, edgeOfWeek, end) {
    var lowerUnit;
    if (matchUnitName(unit, 'week')) {
      edgeOfWeek(d, localeManager.get(localeCode).getFirstDayOfWeek());
      lowerUnit = 'hours';
    } else {
      lowerUnit = getLowerUnitName(unit);
    }
    return setUnitAndLowerToEdge(d, lowerUnit, end);
  }
  ...
  function daysSince(d1, d2) {
    // TODO: some better ways of getting the "day" unit
    return getTimeDistanceForUnit(d1, d2, dateUnits[3]);
  }

  // Higher order units have inherent ambiguity (months have different numbers
  // of days, years have leap years, etc), so for operations such as counting
  // time distance, traversal needs to be used instead of absolute time. This
  // however opens up an edge case where a date can jump ahead an hour during
  // DST fallback, ie. there are two "2am"s during a DST fallback and using
  // setHours(2) chooses the second when the first was intended. Detect for
  // this here and remove the offset using absolute time if necessary.
  function safeAdvanceDate(d, unit, num) {
    var hours = getHours(d);
    advanceDate(d, unit, num);
    if (getHours(d) !== hours) {
  ...
  function getTimeDistanceForUnit(d1, d2, u) {
    ...
    if (u.higher) {
      d1 = cloneDate(d1);
      safeAdvanceDate(d1, u.name, num);
      while (d1 < d2) {
        safeAdvanceDate(d1, u.name, 1);
        if (d1 > d2) {
          break;
        }
        num += 1;
  ...
  function getExtendedDate(contextDate, d, opt) {
    ...
    function setUnitEdge() {
      var modifier = loc.modifiersByName[set.edge], localeCode;
        ...
        iterateOverObject(baseLocale.getFormats(), function(i, dif) {
          ...
          if (match) {
            ...
            if (set.day && (tmp = loc.modifiersByName[set.day])) {
              ...
              set.day = tmp.value;
            } else if ((weekday = loc.getWeekdayValue(set.weekday || set.day)) > -1) {
            ...
            if (set.unit) {
              ...
              unit      = English.unitsLower[unitIndex];
            ...
            iterateOverLowerDateUnits(function(name, u, i, arr) {
              var value = set[name] || 0, fraction = value % 1;
              if (fraction) {
                set[arr[i - 1].name] = round(fraction * (name === 'second' ? 1000 : 60));
  ...
  function getYearFromAbbreviation(d, year) {
    return round(callDateGet(d, 'FullYear') / 100) * 100 - round(year / 100) * 100 + year;
  }
  ...
  function getWeekNumber(d, allowPrevious, firstDayOfWeek, firstDayOfWeekYear) {
    ...
    if (isUndefined(firstDayOfWeekYear)) {
      firstDayOfWeekYear = DEFAULT_FIRST_DAY_OF_WEEK_YEAR;
    }
    // Moving to the end of the week allows for forward year traversal, ie
    // Dec 29 2014 is actually week 01 of 2015.
    yearStart = moveToEndOfWeek(cloneDate(d), firstDayOfWeek);
    moveToFirstDayOfWeekYear(yearStart, firstDayOfWeek, firstDayOfWeekYear);
    if (allowPrevious && d < yearStart) {
      // If the date is still before the start of the year, then it should be
      // the last week of the previous year, ie Jan 1 2016 is actually week 53
      // of 2015, so move to the beginning of the week to traverse the year.
      yearStart = moveToBeginningOfWeek(cloneDate(d), firstDayOfWeek);
      moveToFirstDayOfWeekYear(yearStart, firstDayOfWeek, firstDayOfWeekYear);
    }
    return 1 + floor(daysSince(d, yearStart) / 7);
  }
  ...
  function updateDate(d, params, reset, advance, prefer, weekdayForward) {
    ...
    iterateOverDateUnits(function(name, u, i) {
      var method = u.method, value, checkMonth;
      value = getParam(name)
      ...
      // TODO: can this be handled just by using safeAdvanceDate instead???
      if (advance && i > 3) {
  ...
  function buildDateUnitVariants() {
    dateUnitsReverse = dateUnits.concat().reverse();
    dateUnitsNoWeek = dateUnits.concat();
    dateUnitsNoWeek.splice(2, 1);
  }
  ...
  // First day of week for most locales. Also ISO8601 definition.
  var DEFAULT_FIRST_DAY_OF_WEEK = 1;

  // The week of Jan 4th is always the first week of the year. ISO8601 definition.
  var DEFAULT_FIRST_DAY_OF_WEEK_YEAR = 4;
  ...
  Locale.prototype = {
    ...
    getFirstDayOfWeek: function() {
      ...
      return isDefined(val) ? val : DEFAULT_FIRST_DAY_OF_WEEK;
    },

    getFirstDayOfWeekYear: function() {
      return this.firstDayOfWeekYear || DEFAULT_FIRST_DAY_OF_WEEK_YEAR;
    },
