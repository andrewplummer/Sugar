  ...
  function toUTC(d, force) {
    defineProperty(d, '_utc', force !== false);
    return d;
  }
  ...
  function getExtendedDate(f, localeCode, prefer, forceUTC) {
    ...
    if(isDate(f)) {
      ...
      toUTC(d, isUTC(f)).setTime(f.getTime());
  ...
  function compensateForTimezoneTraversal(d, min, max) {
    ...
    dMin = new date(min);
    dMax = toUTC(new date(max), isUTC(d));
  ...
  extend(date, {
    ...
    'setISOWeek': function(week) {
      ...
      dateSet(this, [{ 'month': 0, 'date': 4 }]);
      dateSet(this, [{ 'weekday': 1 }]);
      if(week > 1) {
        this.addWeeks(week - 1);
      }
      if(weekday !== 1) {
        dateAdvance(this, [{ 'days': weekday - 1 }]);
      }
      return this.getTime();
    },
    ...
    'reset': function(unit) {
      ...
      return recognized ? dateSet(this, [params, true]) : this;
    },
  ...
  English = CurrentLocalization = Sugar.Date.addLocale('en', {
    'plural':     true,
    'timeMarker': 'at',
    'ampm':       'am,pm',
