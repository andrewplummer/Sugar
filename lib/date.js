    ...
    if (isDate(f)) {
      ...
      if (!set) {
        ...
        if (options.fromUTC) {
          ...
          d.setTime(d.getTime() + (tzOffset(-d) * 60 * 1000));
        }
      } else if (relative) {
        advanceDate(d, [set]);
      } else {
  ...
  function getTimezoneShift(d, p) {
    ...
    return (tzOffset(p.date) - tzOffset(d)) * 60 * 1000;
  }
  ...
  function buildDateUnitMethods() {
    ...
    defineInstanceSimilarWithArguments(sugarDate, dateUnits, function(methods, u, i) {
      ...
      function addUnit(d, n, dsc) {
        ...
        // days. If February is our target month, then this is arguably expected.
        // However if we are simply passing by February, for example calling
        // "2 months ago" on March 30th, then this shift in date needs to be
        // compensated for, or else the final date will end up "28".
