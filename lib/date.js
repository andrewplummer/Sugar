  ...
  function setUTC(d, force) {
    setProperty(d, 'utc', !!force);
    return d;
  }

  function isUTC(d) {
    return !!d.utc || tzOffset(d) === 0;
  }

  function getUTCOffset(d, iso) {
    var offset = d.utc ? 0 : tzOffset(d);
    var colon  = iso === true ? ':' : '';
  ...
  function getExtendedDate(contextDate, d, opt) {
    ...
    function cloneDateByFlag(d) {
      ...
      if (d.utc && !isDefined(options.fromUTC)) {
        options.fromUTC = true;
      }
      if (d.utc && !isDefined(options.setUTC)) {
        options.setUTC = true;
      }
      return clone;
    }
  ...
  function buildEnglish() {
    English = currentLocale = sugarDate.addLocale('en', AmericanEnglish);
