  ...
  // Date units without weeks for enumerated arguments.
  var dateArgumentUnits;

  // Date units in reverse order, milliseconds to years.
  var dateUnitsReversed;

  // A reference for the fully compiled English locale.
  var English;
  ...
  var formatTokensBase = [
    {
      ...
      get: function(d, localeCode) {
        return getLocale(localeCode).getAbbreviatedWeekdayName(getWeekday(d));
      }
    },
    {
      ldml: 'Weekday',
      ...
      lowerToken: 'weekday',
      get: function(d, localeCode) {
        return getLocale(localeCode).getWeekdayName(getWeekday(d));
      }
    },
    {
      ldml: 'Mon',
      ...
      lowerToken: 'mon',
      get: function(d, localeCode) {
        return getLocale(localeCode).getAbbreviatedMonthName(getMonth(d));
      }
    },
    {
      ldml: 'Month',
      ...
      get: function(d, localeCode, alternate) {
        return getLocale(localeCode).getMonthName(getMonth(d), alternate);
      }
    },
    {
      strf: 'C',
    ...
    },
    {
      ...
      ldmlPaddedToken: 'DDD',
      get: function(d) {
        var s = resetDate(cloneDate(d), 'month');
        return sugarDate.daysSince(d, s) + 1;
      }
    },
    {
      ldml: 'M',
  ...
  function resetDate(d, unit) {
    ...
    params[unit] = unit.match(/^days?|weeks?/) ? 1 : 0;
  ...
  function moveToBeginningOfUnit(d, unit, localeCode) {
    ...
    switch(unit) {
      case 'year':
        ...
        break;
      case 'month':
        set.month = getMonth(d);
        break;
      case 'day':
        set.day = getDate(d);
        break;
      case 'week':
        moveToBeginningOfWeek(d, getLocale(localeCode).getFirstDayOfWeek());
        resetDate(d);
        return d;
    }
  ...
  function moveToEndOfUnit(d, unit, localeCode) {
    ...
    switch(unit) {
      case 'year':
        ...
        break;
      case 'month':
        set.day = daysInMonth(d);
        break;
      case 'week':
        moveToEndOfWeek(d, getLocale(localeCode).getFirstDayOfWeek());
        break;
    }
    return updateDate(d, set, true);
  }
  ...
  function getWeekNumber(d, allowPrevious, firstDayOfWeek, firstDayOfWeekYear) {
    ...
    return 1 + floor(sugarDate.daysSince(d, yearStart) / 7);
  }
  ...
  function buildFormatTokens() {
    ...
    function buildOrdinal(get) {
      return function(d, localeCode) {
        ...
        return n + getLocale(localeCode).getOrdinal(n);
      }
    }
  ...
  function buildDateProperties() {
    defineStaticProperties(sugarDate, {
      'RFC1123': '{Dow}, {dd} {Mon} {yyyy} {HH}:{mm}:{ss} {tz}',
      'RFC1036': '{Weekday}, {dd}-{Mon}-{yy} {HH}:{mm}:{ss} {tz}',
      'ISO8601_DATE': '{yyyy}-{MM}-{dd}',
      'ISO8601_DATETIME': '{yyyy}-{MM}-{dd}T{HH}:{mm}:{ss}.{SSS}{Z}'
    });
  }
  ...
  defineStatic(sugarDate, {
    ...
    'addFormat': function(format, match, localeCode) {
      getLocale(localeCode).addRawFormat(format, match);
    }
  ...
  // Methods for manipulating locales, held in a closure.
  var getLocale, getAllLocales, setLocale, addLocale, removeLocale;

  function buildDateCore() {

    // Sorry about this guys
    English = addLocale('en', AmericanEnglishBase);
  ...
  function getEnglishVariant(v) {
    return simpleMerge(simpleClone(EnglishLocaleBase), v);
  }
  ...
  function buildLocaleManagement() {
    ...
    getLocale = function(code, noFallback) {
      var loc = locales[code];
      if (!loc && LazyLoadedLocales[code]) {
        loc = addLocale(code, LazyLoadedLocales[code]);
      } else if (!loc && code) {
        loc = locales[code.slice(0, 2)];
      }
      return loc || noFallback ? loc : currentLocale;
    }
    ...
    setLocale = function(code) {
      var loc = getLocale(code, true);
      if (!loc) {
        throw new TypeError('Invalid locale.');
      }
      return currentLocale = loc;
    }

    addLocale = function(arg1, arg2) {
      var loc, code, set;
    ...
    removeLocale = function(code) {
      ...
      return delete locales[code];
    }

  }

  var EnglishLocaleBase = {
    'plural':     true,
    'timeMarker': 'at',
    'ampm':       'AM,PM,a,p',
  ...
  var AmericanEnglishBase = getEnglishVariant({
    'mdy': true,
    'firstDayOfWeek': 0,
  ...
  var BritishEnglishBase = getEnglishVariant({
    'short':  '{dd}/{MM}/{yyyy}',
    'medium': '{d} {Month} {yyyy}',
  ...
  var CanadianEnglishBase = getEnglishVariant({
    'short':  '{yyyy}-{MM}-{dd}',
    'medium': '{d} {Month}, {yyyy}',
    'long':   '{d} {Month}, {yyyy} {H}:{mm}',
    'full':   '{Weekday}, {d} {Month}, {yyyy} {time}',
  ...
  var LazyLoadedLocales = {
    'en-US': AmericanEnglishBase,
    'en-GB': BritishEnglishBase,
    'en-AU': BritishEnglishBase,
    'en-CA': CanadianEnglishBase
  }

  buildLocaleManagement();
  buildDateCore();
  buildCJKDigits();
  buildFormatTokens();
  // TODO: REMOVE!
