...
/***
 * @module ES5
 * @description Shim methods that provide ES5 compatible functionality. This
 *              module is excluded from default builds, and can be included if
 *              you need legacy browser support (IE8 and below).
 *
 ***/

function buildNativeToChainableFallback() {
  if (!Object.getOwnPropertyNames) {
    defineFallbackNativeOnChainable();
  }
}

function defineFallbackNativeOnChainable() {
  ...
  iterateOverObject(nativeTokens, function(name, str) {
    ...
    Sugar.mapNativeToChainable(name, tokens);
  });
...
var getKeys = function(obj) {
  assertNonNull(obj);
  var keys = [];
  iterateOverObject(coercePrimitiveToObject(obj), function(key) {
    keys.push(key);
  });
  return keys;
};
...
defineInstancePolyfill(sugarArray, {
   ...
   * @extra [fromIndex] is the index fromIndex which to begin the search. This
   *        method performs a simple strict equality comparison on <search>. It
   *        does not support enhanced functionality such as searching the
   *        contents against a regex, callback, or deep comparison of objects.
   ...
   *   val  The current element for this iteration.
...
defineInstancePolyfill(sugarString, {
   ...
   *        The standard `trim` method is only added when not fully supported
   *        natively.
   *
   * @example
   *
...
defineInstancePolyfill(sugarFunction, {
   ...
   *        they will be added will be added to the end of the curried parameters.
   ...
   *
   +   (function() {
   *     return this;
   *   }).bind('woof')(); -> returns 'woof'; function is bound with 'woof' as
   *                         the this object.
   *
   *   (function(a) {
   *     return a;
   *   }).bind(1, 2)();   -> returns 2; function is bound with 1 as the this
   *                         object and 2 curried as the first parameter
   *
   *   (function(a, b) {
   *     return a + b;
   *   }).bind(1, 2)(3);  -> returns 5; function is bound with 1 as the this
   *                         object, 2 curied as the first parameter and 3
   *                         passed as the second when calling the function
   *
   ***/
...
buildNativeToChainableFallback();
