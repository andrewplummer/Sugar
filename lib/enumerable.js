 ...
 * @package Enumerable
 * @dependency core
 * @description Counting, mapping, and matching methods on both arrays and objects.
 *
 ***/
...
function getLeastOrMost(obj, map, all, most, k) {
  ...
  result = getMinOrMax(groupedByValue, 'length', all, most);
  if (all) {
    // Flatten result
...
// TODO: this should not iterate over non-array indexes!
...
defineInstance(sugarArray, {
   ...
   * @extra If <fn> returns %false% at any time it will break out of the loop. Once %each% finishes, it will return the array. If [index] is passed, <fn> will begin at that index and work its way to the end. If [loop] is true, it will then start over from the beginning of the array and continue until it reaches [index] - 1.
   * @callback
   *
   *   el  The element of the current iteration.
   *   i   The index of the current iteration.
...
function buildHashEnumerable() {
  ...
  var methods = [
    'map', 'some', 'any', 'every', 'all', 'find', 'filter',
    'count', 'sum', 'average', 'median',
    'isEmpty', 'each', 'size', 'none',
    'min', 'max', 'most', 'least'
  ];
