...
defineInstance(sugarArray, {
   ...
   * @returns Mixed
   * @polyfill es6
   * @short Returns the first element in the array for which <search> is true.
   * @extra In native methods such as this, <search> must be a function whose
   *        return value should be true or false to determine a match. Here,
   *        when <search> is a function it is simply handed off to the native
   *        implementation. However, Sugar further enhances these methods by
   *        accepting other values for <search>, allowing enhanced matching:
   *
   *        Passing a string or number will match using strict equality `===`.
   *        Regular expressions will match against an array of strings. Dates
   *        will call `getTime` internally to match by value. Arrays will
   *        perform deep comparison, calling `isEqual` internally.
   *
   *        Passing an object will perform "fuzzy matching", which will match
   *        objects by specific properties. Object matchers can be as deep as
   *        necessary, and allow the same values as <search>, which makes complex
   *        deep matching possible. For example, if `{ name: /^[a-f]/i }` is
   *        passed, it will match any objects with a `name` property that
   *        starts with `a-f`.
   *
   *        Note that only plain objects will trigger fuzzy matching, so things
   *        like DOM elements can still be matched by strict equality.
   *
   * @callback search
   *
   ...
   * @returns Array
   * @polyfill es5
   * @short Returns any elements in the array for which <search> is true.
   * @extra [context] is the `this` object. This method implements `enhanced
   *        matching`.
   *
   * @callback search
   ...
   * @returns Number
   * @polyfill es6
   * @short Returns the index of the first element in the array for which <search> is
   *        true, or -1 if none.
   * @extra [context] is the `this` object. This method implements `enhanced
   *        matching`.
   *
...
defineInstanceAndStatic(sugarObject, {
   ...
   * @returns Boolean
   * @short Returns the first key in the object for which <search> is true.
   ...
   * @returns Array
   * @short Returns an object with all properties in <obj> for which <search> is true.
   * @extra This method implements `enhanced matching`.
   *
   * @callback search
   *
