...
function buildFromIndexMethods() {
  ...
  forEachProperty(methods, function(key, opts) {
    commaSplit(key).forEach(function(baseName) {
  ...
  // which they are being called, so the actual array must be sliced. Note
  // that this is not the case for some methods like forEach and map, which
  // either do not use their return values or use them in a way that simply
  // getting the element at a shifted index will not affect the final return
  // value. However, these methods will still fail on sparse arrays, so opting
  // to always slice them here. For example, if "forEachFromIndex" were to be
  // called on an array such as [1,,2] from index 1, although the actual index
  // 1 would itself would be skipped, when the array loops back to index 0,
  // shifting it by adding 1 would result in the element for that iteration
  // being undefined. For shifting to work in this case, all gaps in the array
...
function wrapNativeCallback(methodName, wrapper) {
  var nativeFn = Array.prototype[methodName];
  return fixArgumentLength(function(arr, f, context, argsLen) {
    var args = [];
...
function buildEnhancedMapping(name) {
  return wrapNativeCallback(name, enhancedMapping);
}


function buildEnhancedMatching(name) {
  return wrapNativeCallback(name, enhancedMatching);
}
