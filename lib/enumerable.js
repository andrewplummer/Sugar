...
function buildHashEnumerable() {
  wrapHashMethods('each', HASH_RETURNS_SELF);
  wrapHashMethods('map,filter', HASH_WRAPS_RESULT);
  wrapHashMethods('some,any,every,all,none,reduce,find');
  wrapHashMethods('count,sum,average,median,min,max,most,least');
}
