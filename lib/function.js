  ...
  function setDelay(fn, ms, after, scope, args) {
    ...
    if (!fn.timers) {
      setProperty(fn, 'timers', []);
    }
    // This is a workaround for <= IE8, which apparently has the
    // ability to call timeouts in the queue on the same tick (ms?)
    // even if functionally they have already been cleared.
    setProperty(fn, 'canceled', false);
  ...
  defineInstance(sugarFunction, {
    ...
    'lock': function(fn, n) {
      var lockedFn;
      if (fn.partial) {
        setProperty(fn, 'lock', isNumber(n) ? n : null);
        return fn;
      }
      lockedFn = function() {
        arguments.length = min(lockedFn.lock, arguments.length);
        return fn.apply(this, arguments);
      }
      setProperty(lockedFn, 'lock', isNumber(n) ? n : fn.length);
      return lockedFn;
    }
  ...
  defineInstanceWithArguments(sugarFunction, {
    ...
    'partial': function(fn, curriedArgs) {
      ...
      var partialFn = function() {
        var argIndex = 0, applyArgs = [], self = this, lock = partialFn.lock, result;
      ...
      setProperty(partialFn, 'partial', true);
      return partialFn;
    },
