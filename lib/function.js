  ...
  extend(Function, {
     ...
     * @extra If [immediate] is `true`, first execution will happen immediately, then lock. If [limit] is a fininte number, calls past [limit] will be ignored while execution is locked. Compare this to `throttle`, which will execute only once per [ms] milliseconds. Note that [ms] can also be a fraction. Calling `cancel` on a lazy function will clear the entire queue.
     * @example
     *
     *   (function() {
     ...
     * @extra This is functionally equivalent to calling `lazy` with a [limit] of `1` and [immediate] as `true`. `throttle` is appropriate when you want to make sure a function is only executed at most once for a given duration.
     * @example
     *
     *   (3).times(function() {
