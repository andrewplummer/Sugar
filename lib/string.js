  ...
  function stringAssign(str, args) {
    var obj = {};
    if(Object.size(args) === 1 && Array.isArray(args[0])){
      args[0].forEach(function(a, i){
  ...
  extend(string, {
     ...
     * @extra If an object is passed, it's properties can be assigned using the object's keys (i.e. {name}). If a non-object (string, number, etc.) is passed it can be accessed by the argument number beginning with {1} (as with regex tokens). Multiple objects can be passed and will be merged together (original objects are unaffected). If an array of objects is passed it's properties can be assigned using the object's index + key.
     * @example
     *
     *   'Welcome, Mr. {name}.'.assign({ name: 'Franklin' })   -> 'Welcome, Mr. Franklin.'
     *   'You are {1} years old today.'.assign(14)             -> 'You are 14 years old today.'
     *   '{n} and {r}'.assign({ n: 'Cheech' }, { r: 'Chong' }) -> 'Cheech and Chong'
     *   '{0:name} and {1:name} are not the same name!'.assign([{name: 'Ana'}, {name: 'Susana'}]) -> 'Ana and Susana are not the same name!'
     *
     ***/
    'assign': function() {
