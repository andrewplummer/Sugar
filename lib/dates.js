...
(function() {
  ...
  var DateRange = function(start, end) {
    this.start = object.isDate(start) ? start.clone() : new Date();
    this.end   = object.isDate(end)   ? end.clone()   : new Date();
  };
    ...
    DateRange.prototype = {
      ...
      'every': function(increment, fn) {
        ...
        if(object.isString(increment)) {
          match = increment.match(/^(\d+)?\s?(\w+?)s?$/);
          increment = parseInt(match[1] || 1);
          method = 'add' + match[2].capitalize() + 's';
