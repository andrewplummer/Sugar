...
(function() {
  ...
  function arrayMap(arr, map, scope) {
    var length = arr.length, index = 0, el, result = new Array(length);
    while(index < length) {
      if(index in arr) {
        el = arr[index];
        result[index] = transformArgument(el, map, scope, [el, index, arr]);
      }
      index++;
    }
    return result;
  ...
  function arrayFilter(arr, f, scope) {
    var length = arr.length, index = 0, result = [];
    while(index < length) {
      if(index in arr && multiMatch(arr[index], f, scope, [index, arr])) {
  ...
  function arrayIndexOf(arr, search, fromIndex) {
    var length = arr.length, index = toIntegerWithDefault(fromIndex, 0);
    if(object.isString(arr)) {
      return arr.indexOf(search, fromIndex);
    }
    if(index < 0) {
  ...
  function checkLocaleFormatsAdded(loc) {
    ...
    addFormat('(' + arrayMap(arrayFilter(loc['modifiers'], function(m){ return m.name === 'day'; }), 'text').join('|') + ')', ['day'], code);
  ...
  function initializeLocalization(code, set) {
    ...
    function eachAlternate(str, fn) {
      str = arrayMap(str.split('+'), function(split) {
        return split.replace(/(.+):(.+)$/, function(full, base, suffixes) {
          return arrayMap(suffixes.split('|'), function(suffix) {
            return base + suffix;
          }).join('|');
        });
      }).join('|');
    ...
    function setArray(name, abbreviate, multiple) {
      ...
      if(abbreviate) arr = arrayMap(arr.concat(set[name]), function(str) {
    ...
    if(set['monthSuffix']) {
      ...
      set['months'] = arrayMap((1).upto(12), function(n) { return n + set['monthSuffix']; });
    }
    console.info(set);
  ...
  extend(Localization, true, false, {
    ...
    getNumber: function(n) {
      ...
      if(object.isNumber(n)) {
        return n;
      } else if(n && arrayIndexOf(i = this['numbers'], n) !== -1) {
        return (i + 1) % 10;
      } else {
        return 1;
      }
    ...
    getEnglishUnit: function(n) {
      return English['units'][arrayIndexOf(this['units'], n) % 8];
    },
  ...
  function convertAsianDigits(str, key) {
    str = str.replace(/[１２３４５６７８９０]/g, function(d) { return String.fromCharCode(d.charCodeAt(0) - 65248); });
