...
(function() {
  ...
  function buildDateRange() {
    DateRange.prototype.each = function(increment, fn) {
      var current = this.start, result = [], set = {};
      if(object.isString(increment)) {
        set[increment.toLowerCase()] = 1;
      } else {
        set['milliseconds'] = increment;
      }
      while(increment && current <= this.end) {
        result.push(current);
        if(fn) fn(current);
        current = current.clone().advance(set);
      }
      return result;
    }
    DateUnits.each(function(u) {
      ...
      DateRange.prototype['each' + unit] = function(fn) {
        this.start = this.start['beginningOf' + unit]();
        return this.each(unit, fn);
      }
    });
  }
