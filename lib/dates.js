...
(function() {
  ...
  // TODO GET DA INFO ON SLASH VARIANTS
  ...
  function getLocalization(code, set) {
    var loc;
    if(!code || !object.isString(code)) return CurrentLocalization;
    loc = Localizations[code] || Localizations[code.slice(0,2)];
    if(!loc || set)  loc = setLocalization(code, set);
    return loc;
  }
  ...
  function setLocalization(code, set) {
    ...
    addDateInputFormat('(' + loc['weekdays'].compact().join('|') + ')' + time, ['weekday'].concat(TimeFormat), loc);
    addDateInputFormat('(' + loc['modifiers'].filter(function(m){ return m.name === 'day'; }).map('src').join('|') + ')' + time, ['day'].concat(TimeFormat), loc);
  ...
  object.merge(Localization.prototype, {
    ...
    addFormat: function(src, add) {
      ...
      addDateInputFormat(src, to, this);
    }
  ...
  function getExtendedDate(f, code) {
    var d = new date(), relative = false, loc, format, set, unit, num, tmp;
    if(object.isDate(f)) {
      d = f;
      ...
      DateInputFormats.each(function(dif) {
        ...
        if(match) {
          ...
          if(format.variant && !object.isString(set['month']) && (object.isString(set['date']) || loc.hasVariant(code))) {
            tmp = set['month'];
            set['month'] = set['date'];
            set['date']  = tmp;
          }
  ...
  date.extend({
     ...
     *
     ***/
    'addFormat': function(format, match, code) {
      addDateInputFormat(format, match, getLocalization(code), false, true);
    }
