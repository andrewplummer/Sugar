...
(function() {
  ...
  // TODO past/future
  // TODO enumerable-esque methods for objects?
  // TODO split into new core
  // TODO more for ranges?
  // TODO caching formats
  ...
  var CommonLocales = {
    ...
    'ja': '1;月;;日曜日,月曜日,火曜日,水曜日,木曜日,金曜日,土曜日;ミリ秒,秒,分,時間,日,週間|週,ヶ月|ヵ月|月,年;;;;午前,午後;時,分,秒;{num}{unit}{sign};{shift}{unit=5-7}{weekday?},{year}年{month?}月?{date?}日?,{month}月{date?}日?,{date}日;{yyyy}年{M}月{d}日 {Weekday};{H}時{mm}分{ss}秒;一昨日,昨日,今日,明日,明後日;,前,,後;,去|先,,来',

    'ko': '1;월;;일요일,월요일,화요일,수요일,목요일,금요일,토요일;밀리초,초,분,시간,일,주,개월|달,년;일|한,이,삼,사,오,육,칠,팔,구,십;;;오전,오후;시,분,초;{num}{unit} {sign},{shift?} {unit=5-7};{shift} {unit=5?} {weekday},{year}년{month?}월?{date?}일?,{month}월{date?}일?,{date}일;{yyyy}년{M}월{d}일 {Weekday};{H}시{mm}분{ss}초;그저께,어제,오늘,내일,모레;,전,,후;,지난|작,이번,다음|내',
    ...
    'zh-TW': '1;月;;星期日|週日,星期一|週一,星期二|週二,星期三|週三,星期四|週四,星期五|週五,星期六|週六;毫秒,秒鐘,分鐘,小時,天,個星期|週,個月,年;;;日|號;上午,下午;點,分鐘?,秒;{num}{unit}{sign},{shift}{unit=5-7};{shift}{weekday},{year}年{month?}月?{date?}{0},{month}月{date?}{0},{date}{0};{yyyy}年{M}月{d}日 {Weekday};{tt}{h}:{mm}:{ss};前天,昨天,今天,明天,後天;,前,,後;,上|去,這,下|明',

    'zh-CN': '9;月;;星期日|周日,星期一|周一,星期二|周二,星期三|周三,星期四|周四,星期五|周五,星期六|周六;毫秒,秒钟,分钟,小时,天,个星期|周,个月,年;;;日|号;上午,下午;点,分钟?,秒;{num}{unit}{sign},{shift}{unit=5-7};{shift}{weekday},{year}年{month?}月?{date?}{0},{month}月{date?}{0},{date}{0};{yyyy}年{M}月{d}日 {Weekday};{tt}{h}:{mm}:{ss};前天,昨天,今天,明天,后天;,前,,后;,上|去,这,下|明'
  ...
  function getCommonLocalization(localeCode) {
    ...
    ['day','sign','shift','edge'].each(function(name, i) {
      if(!pre[i + 14]) return;
      pre[i + 14].split(',').each(function(t, j) {
  ...
  function formatDate(date, format, relative, localeCode) {
    ...
    if(!date.isValid()) {
      ...
      format = Date[format];
    } else if(object.isFunction(format)) {
  ...
  function updateDate(date, params, reset, utc, advance) {
    ...
    DateUnitsReversed.each(function(u) {
      ...
      if(paramExists(u.unit)) {
        ...
        callDateMethod(date, 'set', utc, u.method, (isDay ? 1 : 0));
      }
    });
    ...
    DateUnits.each(function(u,i) {
      ...
      if(advance) {
        ...
        value = (value * advance) + callDateMethod(date, 'get', false, method);
  ...
  function buildDateMethods() {
    ...
    DateUnits.each(function(u, i) {
      ...
      methods['add'+caps+'s'] = function(num, reset) {
        var set = {};
        set[unit] = num;
  ...
  function buildDateInputFormats() {
    ...
    English.addFormat('('+ month +')[-.](\\d{4,4})', false, ['month','year']);
  ...
  function buildDateOutputShortcuts() {
    var methods = {};
    ['short','long','full'].each(function(name) {
  ...
  date.extend({
    ...
    'advance': function() {
      var args = collectDateArguments(arguments);
      return updateDate(this, args[0], args[1], false, 1, true);
    },
  ...
  function buildDateRange() {
    ...
    DateRange.prototype.every = function(increment, fn) {
      var current = this.start.clone(), result = [], method;
      if(object.isString(increment)) {
        method = 'add' + increment.capitalize() + 's';
        current[method](0, true);
        increment = 1;
      } else {
        method = 'addMilliseconds';
      }
      while(increment && current <= this.end) {
        result.push(current);
        if(fn) fn(current);
    ...
    DateUnits.each(function(u) {
      var unit = u.unit.capitalize();
      DateRange.prototype['each' + unit] = function(fn) {
        return this.every(unit, fn);
      }
    });
  }
