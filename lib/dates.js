...
(function() {
  ...
  // todo can replace static formats with {month} now?
  // todo add comments
  ...
  function getCommonLocalization(localeCode) {
    ...
    ['day','sign','shift','edge'].each(function(name, i) {
      if(!pre[i + 15]) return;
      pre[i + 15].split(',').each(function(t, j) {
  ...
  function getExtendedDate(f, localeCode, prefer) {
    ...
    if(object.isDate(f)) {
      ...
      // If we are a future or past date is preferred, compare
      // against the units that allow ambiguity and shift as needed.
      if(isDefined(prefer) && d.isFuture() != prefer > 0) {
        DisambiguationDateUnits.each(function(u) {
          if(isUndefined(set[u.unit]) && isDefined(set[u.disambiguates])) {
  ...
  function buildDateUnits() {
    DateUnits.each(function(u, i) {
      DateUnitsReversed.unshift(u);
      if(u.disambiguates) {
        DisambiguationDateUnits.push(u);
      }
      if(i !== 2) {
        DateArgumentUnits.push(u);
      }
    });
  }
  ...
  function buildDateMethods() {
    var methods = {};
    DateUnits.each(function(u, i) {
      var unit = u.unit, caps = unit.capitalize(), multiplier = u.multiplier(), since, until;
  ...
  function buildDateInputFormats() {
    ...
    English.addFormat('([+-])?(\\d{4,4})[-.]?('+ month +')[-.]?(\\d{1,2})?', true, ['year_sign','year','month','date'], false, true);
  ...
  function buildDateRange() {
    ...
    DateUnits.each(function(u) {
      DateRange.prototype['each' + u.unit.capitalize()] = function(fn) {
        return this.every(u.unit, fn);
      }
    });
  }
