...
(function() {
  ...
  var DateOutputFormats = [
    ...
    },
    {
      ...
      format: function(d, loc, n, format) {
        ...
        if(format.length === 1) str = str.first();
        if(format.first() === 'T') str = str.toUpperCase();
        return str;
      }
    },
    {
  ...
  var CommonLocales = {
    ...
    'zh-TW': '1;月;;日,一,二,三,四,五,六;毫秒,秒鐘,分鐘,小時,天,個星期|週,個月,年;;;日|號;上午,下午;點,分鐘?,秒;{num}{unit}{sign},{shift}{unit=5-7};星期{weekday},{shift}{unit=5}{weekday},{year}年{month?}月?{date?}{0},{month}月{date?}{0},{date}{0};{yyyy}年{M}月{d}日 {tt}{h}:{mm};前天,昨天,今天,明天,後天;,前,,後;,上|去,這,下|明',

    'zh-CN': '9;月;;日,一,二,三,四,五,六;毫秒,秒钟,分钟,小时,天,个星期|周,个月,年;;;日|号;上午,下午;点,分钟?,秒;{num}{unit}{sign},{shift}{unit=5-7};星期{weekday},{shift}{unit=5}{weekday},{year}年{month?}月?{date?}{0},{month}月{date?}{0},{date}{0};{yyyy}年{M}月{d}日 {tt}{h}:{mm};前天,昨天,今天,明天,后天;,前,,后;,上|去,这,下|明'
  ...
  function getLocalization(localeCode, set, skipFallback) {
    var loc;
  ...
  function collectDateArguments(args) {
    ...
    if(object.isObject(args[0])) {
      return args;
    } else if (object.isNumber(args[0]) && !object.isNumber(args[1])) {
      return [args[0]];
    }
    obj = {};
    ...
    if(object.isDate(f)) {
      ...
      baseLocalization.compiledFormats.each(function(dif) {
        ...
        if(match) {
          ...
          if(format.variant && !object.isString(set['month']) && (object.isString(set['date']) || baseLocalization.hasVariant(localeCode))) {
            tmp = set['month'];
            set['month'] = set['date'];
            set['date']  = tmp;
          }
  ...
  date.extend({
    ...
    'setLocale': function(localeCode, set) {
      var loc = getLocalization(localeCode, set, true);
      if(loc) {
