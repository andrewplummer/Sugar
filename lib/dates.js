...
(function() {
  ...
  var LowerAsianDigits   = '〇一二三四五六七八九';
  var UpperAsianDigits   = '十百千万';
  //var LowerAsianDigitReg = regexp('[' + LowerAsianDigits + ']', 'g');
  //var UpperAsianDigitReg = regexp('[' + UpperAsianDigits + ']', 'g');
  var AsianDigitReg = regexp('^[' + LowerAsianDigits + UpperAsianDigits + ']+$');
  ...
  var CommonLocales = {
    ...
    'zh-TW': '1;月;;日,一,二,三,四,五,六;毫秒,秒鐘,分鐘,小時,天,個星期|週,個月,年;;;日|號;上午,下午;點,分,秒;{num}{unit}{sign},星期{weekday},{shift}{unit=5-7},{shift}{unit=5}{weekday},{year}年{month?}月?{date?}{0},{month}月{date?}{0},{date}{0};{yyyy}年{M}月{d}日;前天,昨天,今天,明天,後天;,前,,後;,上|去,這,下|明',

    'zh-CN': '1;月;;日,一,二,三,四,五,六;毫秒,秒钟,分钟,小时,天,个星期|周,个月,年;;;日|号;上午,下午;点,分,秒;{num}{unit}{sign},星期{weekday},{shift}{unit=5-7},{shift}{unit=5}{weekday},{year}年{month?}月?{date?}{0},{month}月{date?}{0},{date}{0};{yyyy}年{M}月{d}日;前天,昨天,今天,明天,后天;,前,,后;,上|去,这,下|明'
  ...
  function getCommonLocalization(code) {
    ...
    if(bool(1)) {
      ...
      if(set['numbers'].length > 0) {
        ...
        set['numbers'] = LowerAsianDigits.slice(1).split('');
      }
  ...
  function getExtendedDate(f, locale) {
    ...
    if(object.isDate(f)) {
      ...
      //f = convertAsianDigits(f.hankaku('n'));
