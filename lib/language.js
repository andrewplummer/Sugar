 ...
 * @module Language
 * @namespace String
 * @description Detecting language by character block. Full-width <-> half-width
 *              character conversion. Hiragana and Katakana conversions.
 *
 ***/
...
function buildUnicodeScripts() {
  defineInstanceSimilar(sugarString, UNICODE_SCRIPTS, function(methods, source, names) {
    var is = RegExp('^['+ source +'\\s]+$');
    var has = RegExp('['+ source +']');
...
defineInstance(sugarString, {
   ...
   * @extra [mode] accepts any combination of "a" ("alphabet"), "n" ("numbers"),
