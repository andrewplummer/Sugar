...
var TYPE_CHECK_NAMES = ['Boolean','Number','String','Date','RegExp','Function','Array','Error','Set','Map'];
...
function buildClassChecks() {
  ...
  function addCoreTypes() {
    ...
    isDate   = buildClassCheck(TYPE_CHECK_NAMES[3]);
    isRegExp = buildClassCheck(TYPE_CHECK_NAMES[4]);
    ...
    isFunction = buildClassCheck(TYPE_CHECK_NAMES[5]);


    isArray = Array.isArray || buildClassCheck(TYPE_CHECK_NAMES[6]);
    isError = buildClassCheck(TYPE_CHECK_NAMES[7]);

    isSet = buildClassCheck(TYPE_CHECK_NAMES[8], typeof Set !== 'undefined' && Set);
    isMap = buildClassCheck(TYPE_CHECK_NAMES[9], typeof Map !== 'undefined' && Map);

    // Add core types as known so that they can be checked by value below,
    // notably excluding Functions and adding Arguments and Error.
    addKnownType('Arguments');
    addKnownType(TYPE_CHECK_NAMES[0]);
    addKnownType(TYPE_CHECK_NAMES[1]);
    addKnownType(TYPE_CHECK_NAMES[2]);
    addKnownType(TYPE_CHECK_NAMES[3]);
    addKnownType(TYPE_CHECK_NAMES[4]);
    addKnownType(TYPE_CHECK_NAMES[6]);

  }

  function addArrayTypes() {
    var types = 'Int8,Uint8,Uint8Clamped,Int16,Uint16,Int32,Uint32,Float32,Float64';
    forEach(commaSplit(types), function(str) {
  ...
  function isKnownType(obj, className) {
  ...
  canCompareByValue = function(obj, className) {
    ...
    return isKnownType(obj, className) || isPlainObject(obj, className);
  };
...
function isEqual(a, b, stack) {
  ...
  if (canCompareByValue(a, aClass) && canCompareByValue(b, bClass)) {
...
function map(arr, fn) {
  // perf: Not using fixed array here as they may be sparse.
  var result = [];
  for (var i = 0, len = arr.length; i < len; i++) {
    if (i in arr) {
...
function createFormatMatcher(bracketMatcher, percentMatcher, precheck) {
  ...
  var compiledFormats = new MemoizeMap();
  ...
  function compile(str) {
    ...
    compiledFormats.set(str, format);
    return format;
  }

  return function(str, obj, opt) {
    var format = compiledFormats.get(str) || compile(str), result = '';
...
function buildMemoizeMap() {

  var MEMOIZE_LIMIT = 1000;

  MemoizeMap = function() {
    this.reset();
  };

  MemoizeMap.prototype.has = function(key) {
    return hasOwn(this.data, key);
  };

  MemoizeMap.prototype.get = function(key) {
    return getOwn(this.data, key);
  };

  MemoizeMap.prototype.set = function(key, val) {
    if (this.counter === MEMOIZE_LIMIT) {
      this.reset();
    }
    this.data[key] = val;
    this.counter++;
  ...
  MemoizeMap.prototype.reset = function() {
    this.data = {};
    this.counter = 0;
  };
