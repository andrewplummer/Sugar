  ...
  // Importing utility functions from the core
  var hasOwn             = Sugar.hasOwn;
  var setProperty        = Sugar.setProperty;
  var defineProperty     = Sugar.defineProperty;
  var iterateOverObject  = Sugar.iterateOverObject;
  var wrapInstanceMethod = Sugar.wrapInstanceMethod;

  var isBoolean  = buildPrimitiveClassCheck(TYPE_CHECK_NAMES[0]),
  ...
  function buildPrimitiveClassCheck(klass) {
    ...
    return function(obj) {
      return typeof obj === type || isObjectType(obj) && isClass(obj, klass);
    }
  }
  ...
  function wrapNamespace(name) {
    return function(sugarNamespace, arg1, arg2) {
      sugarNamespace[name](arg1, arg2);
    }
  }

  var alias                       = wrapNamespace('alias');
  var defineStatic                = wrapNamespace('defineStatic');
  var defineInstance              = wrapNamespace('defineInstance');
  var defineStaticPolyfill        = wrapNamespace('defineStaticPolyfill');
  var defineInstancePolyfill      = wrapNamespace('defineInstancePolyfill');
  var defineInstanceAndStatic     = wrapNamespace('defineInstanceAndStatic');
  var defineStaticWithArguments   = wrapNamespace('defineStaticWithArguments');
  var defineInstanceWithArguments = wrapNamespace('defineInstanceWithArguments');
  ...
  function hasValidPlainObjectPrototype(obj) {
    ...
    return (!hasConstructor && !hasToString) ||
           (hasConstructor && !hasOwn(obj, CONSTRUCTOR) && hasOwn(obj.constructor.prototype, IS_PROTOTYPE_OF));
  }
