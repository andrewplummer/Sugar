...
function iterateOverSparseArray(arr, fn, startIndex, loop) {
  var indexes = getSparseArrayIndexes(arr, startIndex, loop), index;
...
function getSparseArrayIndexes(arr, startIndex, loop, fromRight) {
  var indexes = [], i;
  for (i in arr) {
    if (isArrayIndex(i) && (loop || (fromRight ? i <= startIndex : i >= startIndex))) {
      indexes.push(+i);
    }
  }
  indexes.sort(function(a, b) {
    var aLoop = a > startIndex;
    var bLoop = b > startIndex;
    if (aLoop !== bLoop) {
      return aLoop ? -1 : 1;
    }
    return a - b;
