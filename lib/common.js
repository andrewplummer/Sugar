  ...
  function deepGetProperty(obj, prop) {
    var ns = obj || undefined, ps, bs, key;
    if(prop == null) return;
    prop = String(prop);
    ps = periodSplit(prop);
    ...
    for (var i = 0, ilen = ps.length; i < ilen; i++) {
      key = ps[i];
      if (key.indexOf(LEFT_SQUARE_BRACKET) !== -1) {
        bs = key.split(LEFT_SQUARE_BRACKET);
        for (var j = 0, jlen = bs.length; j < jlen; j++) {
          key = bs[j];
          // A key like users[5] will be split into 2 parts, one with a closing
          // brace and one without, so if the index is greater than 0, chomp it.
          if (j) {
            key = key.slice(0, -1);
          }
          ...
          if (key || j) {
            ns = getPropertyWithIndex(ns, key);
          }
        }
      } else {
        ns = getPropertyWithIndex(ns, key);
