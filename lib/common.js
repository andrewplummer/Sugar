  ...
  function privateGetterSetter(key) {
  ...
  function deepGetProperty(obj, key) {
    var val = obj || undefined, bs, ps, key, range;
    if(key == null) return;
    key = String(key);

    if (key.indexOf('..') !== -1) {
      return getPropertyRange(obj, key);
    }
    bs = key.split('[');
    ...
    for (var i = 0, blen = bs.length; i < blen; i++) {
      ...
      for (var j = 0, plen = ps.length; j < plen; j++) {
        ...
        // A key like users[n][n] will be split into 3 parts, the first part
        // without a closing bracket and the rest with one. If there are
        // trailing dot properties, then only the first one will contain the
        // bracket, so we can confidently remove the last character when
        // j is 0 and i is not 0.
        if (i && !j) {
          key = key.slice(0, -1);
        }

        // Bracket keys may look like users[5] or just [5]. The first bit is
        // optional, so if it doesn't exist then move on, otherwise attempt
        // to traverse into the namespace, even if the key is falsey.
        if (i || key || blen === 1) {
          val = getPropertyWithIndex(val, key);
        }
      }
    }
    return val;
  ...
  // Get object property with negative array index support.
  function getPropertyWithIndex(obj, key) {
    var val = obj && obj[key];
    if (isUndefined(val) && key && obj && obj.length) {
      return obj[+key + obj.length];
    }
    return val;
  }

  // Get object property with support for 0..1 style range notation.
  function getPropertyRange(obj, key) {
    var match, start, end, leading, trailing;
    ...
    // A range of 0..1 is inclusive, so we need to add 1 to the end.
    // If this pushes the index from -1 to 0, then it needs to be undefined,
    // otherwise .slice will not include the last element in the array.
    end = start < 0 && end === -1 ? undefined : end + 1;
    if (leading) {
      obj = deepGetProperty(obj, leading);
    }
  ...
  function isPlainObject(obj, klass) {
    ...
    if (hasOwnConstructor(obj)) {
      return false;
    }
    // === on the constructor is not safe across iframes
    // 'hasOwnProperty' ensures that the object also inherits
    // from Object, which is false for DOMElements in IE.
    return !!obj && 'hasOwnProperty' in obj && (klass || className(obj) === '[object Object]');
  }

  // Having own "constructor" with its own "isPrototypeOf" property cannot
  // be a plain object. This code was borrowed from jQuery.isPlainObject
  function hasOwnConstructor(obj) {
    try {
  ...
  function buildStringToNumberHelpers() {
    var digit, i;
    for(i = 0; i <= 9; i++) {
  ...
  var _utc = privateGetterSetter('utc');
