  ...
  /***COMPILER REMOVE***/
    ...
    // A quick reference to our extend method.
    var extend = Sugar.extend;

    // A reference to the augmented hasOwnProperty method.
    var hasOwnProperty = Sugar.has;

    // A reference to object iteration method.
    var iterateOverObject = Sugar.iterate;

    // A reference to the defineProperty method.
    var defineProperty = Sugar.define;

    // A reference to the global context.
    var globalContext = Sugar.global;

    // Natives by name.
    var natives = Sugar.natives;
  ...
  function extendSimilar(klass, set, fn, instance, polyfill) {
    var methods = {};
    set = isString(set) ? set.split(',') : set;
    set.forEach(function(name, i) {
      fn(methods, name, i);
    });
    extend(klass, methods, instance, polyfill);
  }
  ...
  function buildObjectInstanceMethods(set, target) {
    extendSimilar(target, set, function(methods, name) {
      methods[name + (name === 'equal' ? 's' : '')] = function() {
        return Sugar.Object[name].apply(null, [this].concat(multiArgs(arguments)));
      }
    });
  }
