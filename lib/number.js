  ...
  var MEMORY_UNITS       = '|KMGTPE';
  var METRIC_UNITS_SHORT = 'nμm|k';
  var METRIC_UNITS_FULL  = 'yzafpnμm|KMGTPEZY';
  ...
  function abbreviateNumber(num, precision, units, bytes) {
    ...
    mid = units.indexOf('|');
    if (mid === -1) {
      // Skipping the placeholer means the units should start from zero,
      // otherwise assume they end at zero.
      mid = units.charAt(0) === '_' ? 0 : units.length;
    }
    i = max(min(floor(significant / 3), units.length - mid - 1), -mid);
    unit = units.charAt(i + mid);
    while (unit === '_') {
      i += i < 0 ? -1 : 1;
      unit = units.charAt(i + mid);
    }
  ...
  defineInstance(sugarNumber, {
     ...
     *
     ***/
    'bytes': function(n, precision, si) {
      return abbreviateNumber(n, precision, MEMORY_UNITS, si !== true) + 'B';
    },
     ...
     * @extra Returns the collected return values of all calls to <fn>.
     * @example
     *
     *   (8).times(function(i) {
