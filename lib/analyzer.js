(function(global) {
    ...
    },
    {
      ...
      methods: [
        ...
        },
        {
          ...
          original: "fn = fn.wrap(function(original){ // do some stuff })",
          sugar: "fn = (function(o){ return function(original){ // do some stuff }.bind(null, o); })(fn)"
        }
      ]
        /***
         *
  ...
  var UnderscoreMethods = [
    {
      ...
      methods: [
        ...
        {
          name: 'map',
          description: '_.map exists natively in modern browsing engines as Array.prototype.map.\nSugar provides this method when it is not supported, and additionally augments it to allow passing a string as a shortcut.',
          original: '_.map([1,2,3], function(a){ return a * 2; })',
          sugar: '[1,2,3].map(function(a){ return a * 2; })',
          ref: 'Array/map'
        }
  ...
  var warn = function(message, stackLevel, skipMeta, docs, logLevel) {
    ...
    if(stack) {
      files = stack.match(/(https?|file):[^:]+:\d+(?::\d+)?/g);
      file = files[stackLevel];
      if(file.match(/(prototype|underscore|analyzer)[^\/]+\.js/)) {
        return;
      }
  ...
  var initialize = function() {
    if(typeof _ != 'undefined') {
      ...
      if(SUGAR_ANALYZER_PROTOTYPE_NOTIFICATIONS !== false) {
