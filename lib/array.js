  ...
  extend(array, {
     ...
     * @method swap(<f>, <swap>)
     * @returns Array
     * @short Swaps all elements that match <f> with <swap>. This method will change the array! If you don't intend the array to be changed use %clone% first.
     * @example
     *
     *   ['a','b','c'].swap('b', 'd') -> ['a','d','c']
     *   [3,4,3,5].swap(3, 7)  -> [7,4,7,5]
     *   [[1,a],{b:'cd'}].swap([1,a],{d:4}).swap({b:'cd'},55) -> [{b:'cd'},55]]
     *   ['canada','california'].swap(/^ca/,'mexico') -> ['mexico','mexico']
     *
     ***/

    'swap': function(f, swap) {
      var index = 0;
      while(!isUndefined(index)) {
        index = arrayFind(this, f, index, false, true);
