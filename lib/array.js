...
defineInstanceWithArguments(sugarArray, {
  ...
  'zip': function(arr, args) {
	...
	/***
		* @method iSlice([start], [end])
		* @returns Array
		* @short Returns a new array that contains all elements that is outside the
		*        selected span. Original array will be untouched.
		* @extra This will work as a inverse slice.
		*
		*
		* @example
		*
		*   [1,2,3].iSlice(0,1)     -> [2,3]
		*   [1,2,3,4,5,6,7,8,9].iSlice(4,7)     -> [1,2,3,4,8,9]
		*
		***/
	'iSlice': function(arr, start, end) {
		var beginning = arr.slice(0, start);
		var ending = arr.slice(end, arr.length);
		var newArr = beginning.concat(ending);
		return newArr;
	}
