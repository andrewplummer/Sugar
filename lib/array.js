  ...
  function arrayFindIndex(arr, f, index, loop) {
  ...
  function callNativeWithMatcher(name, arr, f, context) {
    var nativeFn = arrayNativeMethods[name], fn, matcher;
    if (isFunction(f)) {
      fn = f;
    } else {
      matcher = getMatcher(f);
    ...
    return nativeFn.call(arr, fn, context);
  }
  ...
  function enhancedMap(arr, f, context) {
    var nativeMap = arrayNativeMethods.map, fn;
    if (isFunction(f)) {
      fn = f;
    } else {
    ...
    return nativeMap.call(arr, fn, context);
  }
  ...
  function buildEnhancements() {
    ...
    defineInstanceSimilar(sugarArray, 'every,some,filter,find,findIndex', function(methods, name) {
      storeNative(name);
      methods[name] = function (arr, f, context) {
        return callNativeWithMatcher(name, arr, f, context);
      }
    });
  ...
  function buildEnumerableMethods(names, mapping) {
    defineStaticSimilar(object, names, function(methods, name) {
      methods[name] = function(obj, arg1, arg2) {
        var result, coerced = keysWithObjectCoercion(obj), matcher;
        if (!mapping) {
          matcher = getMatcher(arg1, true);
        }
        result = sugarArray[name].call(coerced, function(key) {
          var value = obj[key];
          if (mapping) {
