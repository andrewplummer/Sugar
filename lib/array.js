  ...
  function iterateOverSparseArray(arr, fn, startIndex, loop) {
    var indexes = getSparseArrayIndexes(arr, startIndex);
  ...
  function buildArrayEnhancements() {
    defineInstanceSimilar(sugarArray, 'map,every,some,filter,find,findIndex', function(methods, name) {
      var nativeFn = Array.prototype[name], fn;
      if (!nativeFn) {
      ...
      if (name === 'map') {
        fn = callMapWithTransform;
      } else {
        fn = function(arr, f, context, argLen) {
          return callNativeWithMatcher(nativeFn, arr, f, context, argLen);
        }
      }
      methods[name] = fixArgumentLength(fn);
  ...
  function getSortEquivalents() {
    var equivalents = {};
    arrayEach(commaSplit('AÁÀÂÃÄ,CÇ,EÉÈÊË,IÍÌİÎÏ,OÓÒÔÕÖ,Sß,UÚÙÛÜ'), function(set) {
  ...
  defineInstance(sugarArray, {
     ...
     * @extra [m] may be a function mapping the value to be summed or a string acting as a shortcut.
     * @example
     *
     *   [1,2,2].sum() -> 5
     ...
     * @extra [m] may be a function mapping the value to be averaged or a string acting as a shortcut.
     * @example
     *
     *   [1,2,3].average() -> 2
     ...
     *
     ***/
    'median': function(arr, m) {
      var len = arr.length, middle;
      if (!len) return;
      arr = m ? callMapWithTransform(arr, m) : arrayClone(arr);
  ...
  function buildObjectEnumerable() {
    ...
    function createStaticAndHash(sugarFn, methods, name, fn) {
      if (!sugarFn) {
        // Array#find is a polyfill and may not exist here.
        return;
      }
      methods[name] = fn;
    ...
    function callEnumerableForObject(sugarFn, obj, arg2, fn) {
      var result = sugarFn(keysWithObjectCoercion(obj), fn, arg2);
      if (isArray(result)) {
    ...
    defineInstanceAndStaticSimilar(sugarObject, 'sum,average,min,max,least,most', function(methods, name) {
      var sugarFn = sugarArray[name];
      createStaticAndHash(sugarFn, methods, name, function(obj, arg1, arg2) {
        return callEnumerableForObject(sugarFn, obj, arg2, function(key) {
          var value = obj[key];
