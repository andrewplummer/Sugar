...
function arrayCreate(obj, map, context) {
  if (!map && isArray(obj)) {
    return obj
  } else if (isObjectType(obj) || isString(obj)) {
    return Array.from(obj, map, context);
  }
...
defineStatic(sugarArray, {
   ...
   * @extra This method is similar to native `Array.from` except that it quickly
   *        returns <obj> if it is an array and [map] is not passed. Additionally
   *        it will not fail on `undefined`, `null`, or numbers, producing an
   *        empty array in the case of `undefined` and wrapping <obj> otherwise.
   * @example
   *
   *   Array.create()          -> []
   *   Array.create(8)         -> [8]
   ...
   *
   ***/
  'create': function(obj, map, context) {
    return arrayCreate(obj, map, context);
  },
