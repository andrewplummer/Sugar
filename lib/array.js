  ...
  function transformArgument(el, map, context, mapArgs) {
    if(!map) {
      ...
      return map.map(function(m) { return transformArgument(el, m, context, mapArgs); });
  ...
  extend(array, true, true, {
    ...
    'sortBy': function(map, desc) {
      var cmp = function(aProperty, bProperty) {
        if(isString(aProperty) && isString(bProperty)) {
          return collateStrings(aProperty, bProperty);
        } else if(isArray(aProperty) && isArray(bProperty)) {
          if(aProperty.length < bProperty.length) {
            return -1;
          } else if(aProperty.length > bProperty.length) {
            return 1;
          } else {
            for(var i = 0; i < aProperty.length; i++) {
              var c = comp(aProperty[i], bProperty[i]);
              if(c != 0) {
                return c;
              }
            }
      ...
      arr.sort(function(a, b) {
        ...
        return cmp(aProperty, bProperty) * (desc ? -1 : 1);
      });
      return arr;
    },
