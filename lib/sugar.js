(function() {
  ...
  function transformArgument(el, map, context, mapArgs, shortcutArgs) {
    if(isUndefined(map)) {
      return el;
    } else if(object.isFunction(map)) {
      return map.apply(context, mapArgs || []);
    } else if(object.isFunction(el[map])) {
      return el[map].apply(el, shortcutArgs || []);
    } else {
      return el[map];
    }
  }
  ...
  extend(object, false, false, {
    ...
    'tap': function(obj, fn) {
      transformArgument(obj, getArgs(arguments, 1), obj, obj);
      return obj;
    },
  ...
  extend(array, true, function() { var a = arguments; return a.length === 0 || object.isFunction(a[0]); }, {
    ...
    'map': function(map, scope) {
      ...
      while(index < length) {
        if(index in this) {
          result[index] = transformArgument(this[index], map, this, [this[index], index, this]);
        }
        index++;
      }
      return result;
