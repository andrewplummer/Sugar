...
(function(){
  ...
  extend(Array, {
    ...
    'findAll': function(f, reject){
      var result = [];
      for(var i=0,len=this.length; i<len; i++){
        if(multiMatch(this[i], f)) result.push(this[i]);
      }
      return result;
    },
    ...
    'mode': function(block){
      ...
      for(var key in grouped){
        ...
        var length = grouped[key].length;
        if(length >= top){
          if(length > top) mode = [];
          top = length;
          var num = parseFloat(key);
          mode.push(isNaN(num) ? key : num);
        }
      }
