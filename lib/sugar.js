(function() {
  ...
  function equal(a, b, stack) {
    ...
    if(a === b) {
      ...
      return object.keys(a).length === object.keys(b).length &&
       ...
       return instance(b, primitive) && a.valueOf() === b.valueOf();
     }
  }
  ...
  function cloneObject(obj, deep) {
    ...
    if(object.isArray(obj)) {
      result = [];
    } else if(!isObjectPrimitive(obj) || isNull(obj)) {
      return obj;
    } else {
  ...
  extend(object, false, function() { return arguments.length < 2; }, {
    ...
    'keys': function(obj, fn) {
      if(isNull(obj) || (!isObjectPrimitive(obj) && !object.isFunction(obj))) {
    ...
    'intersect': function() {
      ...
      this.each(function(el, i) {
        multiArgs(args, function(merge) {
          ...
          if(isUndefined(arrayFind(result, el)) && !isUndefined(arrayFind(merge, el))) {
            result.push(el);
          }
        });
      });
    ...
    'compact': function(all) {
      ...
      arrayEach(this, function(el, i) {
        if(object.isArray(el)) {
          ...
          result.push(el);
        } else if(!all && !isUndefined(el) && !isNull(el) && (!object.isNumber(el) || !isNaN(el))) {
          result.push(el);
        }
      });
      return result;
  ...
  function buildBase64(key) {
    if(!isUndefined(this.btoa)) return;
      ...
      arrayEach(DateInputFormats, function(dif) {
        ...
        if(match) {
          ...
          if(set[tokens.unit]) {
            ...
            if(!isUndefined(set[tokens.weekday])) {
  ...
  function updateDate(date, params, reset, utc, advance) {
    ...
    if(!advance && isUndefined(params[tokens.day]) && !isUndefined(params[tokens.weekday])) {
    ...
    arrayEach(DateUnitsReversed, function(u) {
      if(!isUndefined(params[u.unit]) || !isUndefined(params[u.unit + 's'])) {
    ...
    if(advance && !isUndefined(params[tokens.month])) {
    ...
    arrayEach(DateUnits, function(u,i) {
      var unit   = u.unit;
      var method = u.method;
      var value = !isUndefined(params[unit]) ? params[unit] : params[unit + 's'];
    ...
    'fill': function() {
      ...
      return function() {
        ...
        arrayEach(curried, function(arg, index) {
          if(!isUndefined(arg) || index >= args.length) args.insert(arg, index);
        });
