(function() {
  ...
  var regexShorthandSupport = /./.exec !== Undefined;

  mapTypeMethods(object);

  function extend(klass, instance, override, methods) {
    var extendee = instance ? klass.prototype : klass;
    storeMethods(klass, instance, methods);
    iterateOverObject(methods, function(name, method) {
      if(object.isFunction(override)) {
  ...
  function equal(a, b, stack) {
    ...
    if(isObjectPrimitive(a) && isObjectPrimitive(b)) {
      if(a === null) return b === null;
      if(object.isRegExp(a) && object.isRegExp(b)) {
  ...
  function multiArgs(args, fn) {
    arrayEach(arrayFlatten(getArgs(args)), fn);
  }

  function reg(r) {
    return regexShorthandSupport ? r : regexp(r);
  }
  ...
  function cloneObject(obj, deep) {
    ...
    if(object.isArray(obj)) {
      ...
      result = obj['keys'] ? object.extended() : {};
    }
    iterateOverObject(obj, function(k,v) {
      if(deep === true && (object.isObject(v) || object.isArray(v))) {
  ...
  function mapHashMethods(obj) {
    arrayEach(['keys','values','each','merge','isEmpty','clone','equal','watch'], function(m) {
      ...
      defineProperty(obj, name, function() {
        return object[m].apply(null, [this].concat(getArgs(arguments)));
      });
    });
  }
  ...
  function mapObjectPrototypeMethods() {
    mapTypeMethods(object.prototype);
    mapHashMethods(object.prototype);
  }
  ...
  extend(object, false, function() { return arguments.length < 2; }, {
    ...
    'keys': function(obj, fn) {
      if(obj === null || (!isObjectPrimitive(obj) && !object.isFunction(obj))) {
  ...
  extend(array, true, false, {
    ...
    'groupBy': function(map, fn) {
      var result = object.extended(), key;
    ...
    'compact': function(all) {
      ...
      arrayEach(this, function(el, i) {
        if(object.isArray(el)) {
          ...
          result.push(el);
        } else if(!all && el !== Undefined && el !== null && (!object.isNumber(el) || !isNaN(el))) {
          result.push(el);
        }
      });
      return result;
  ...
  extend(string, true, false, {
    ...
    'truncate': function(length, append, split) {
      ...
      CHANGE = regexp('[^' + getTrimmableCharacters() + a + '][' + getTrimmableCharacters() + a + ']');
      while(length > 0 && !CHANGE.test(this.slice(length - 1, length + 1)) && split !== true) {
        length--;
      }
  ...
  var TimeFormat = [tokens.hour,tokens.minute,tokens.second,tokens.millisecond,tokens.meridian,tokens.utc,tokens.offset_sign,tokens.offset_hours,tokens.offset_minutes]
  ...
  var StaticInputFormats = [
    ...
    // @date_format 2010/05/25
    { src: '([+-])?(\\d{4})[-.]?({month})[-.]?(\\d{1,2})?', to: [tokens.year_sign,tokens.year,tokens.month,tokens.date] },
    // @date_format 05-25
    // @date_format 05/25
    // @date_format 05.25
    ...
    // @date_format 05/25/2010
    // @date_format 05.25.2010
    { src: '(\\d{1,2})[-.\\/]({month})[-.\\/]?(\\d{2,4})?', to: [tokens.month,tokens.date,tokens.year], variant: true }
  ];
  ...
  var DateOutputFormats = [
    ...
    },
    {
      ...
      format: function(d, loc, n, t) {
        return loc[tokens.weekdays][d.getDay() + (n - 1) * 7];
      }
    },
    {
      token: 'MM?',
    ...
    },
    {
      ...
      format: function(d, loc, n, len) {
        return loc[tokens.months][d.getMonth() + (n - 1) * 12];
      }
    },
    {
      token: 'y{2,4}|year',
  ...
  function checkLocaleFormatsAdded(loc) {
    var addFormat = date.addFormat, code = loc[tokens.code];
    if(loc.formatsAdded) return;
    addFormat('(' + loc[tokens.months].compact().join('|') + ')', [tokens.month], code);
    addFormat('(' + loc[tokens.weekdays].compact().join('|') + ')', [tokens.weekday], code);
    addFormat('(' + loc[tokens.modifiers].filter(function(m){ return m.name === tokens.day; }).map('text').join('|') + ')', [tokens.day], code);
    arrayEach(loc[tokens.formats], function(src) {
      var to = [];
      src = src.replace(/\s+/g, '[-,. ]*');
      src = src.replace(/\{(.+?)\}/g, function(all, k) {
        ...
        if(nc) {
          value = loc[tokens.optionals][nc[0] - 1];
        } else if(loc[key]) {
          value = loc[key];
        } else if(loc[key + 's']) {
          value = loc[key + 's'];
          if(slice) {
            ...
            arrayEach(value, function(m,i) {
              var mod = i % (loc[tokens.units] ? 8 : value.length);
  ...
  function getLocalization(code, fallback, set) {
    if(fallback && (!object.isString(code) || !code)) code = Date[tokens.currentLocale];
  ...
  function initializeLocalization(code, set) {
    ...
    function getDigit(start, stop) {
      ...
      if(set[tokens.digits]) str += '|[' + set[tokens.digits] + ']+';
      return str;
    }

    function getNum() {
      var arr = [getDigit()].concat(set[tokens.articles]);
      if(!set[tokens.digits]) arr = arr.concat(set[tokens.numbers]);
    ...
    function setModifiers() {
      var arr = [];
      set.modifiersByName = {};
      arrayEach(set[tokens.modifiers], function(modifier) {
      ...
      arr.groupBy('name', function(name, group) {
        ...
        if(name === tokens.day) group = group.concat(set[tokens.weekdays]);
        set[name] = group.join('|');
      });
      set[tokens.modifiers] = arr;
    }

    setArray(tokens.months, true, 12);
    setArray(tokens.weekdays, true, 7);
    setArray(tokens.units, false, 8);
    setArray(tokens.numbers, false, 10);

    set[tokens.code] = code;
    set[tokens.date] = getDigit(1,2);
    set[tokens.year] = getDigit(4,4);
    set[tokens.num]  = getNum();

    setModifiers();

    if(set[tokens.monthSuffix]) {
      set[tokens.month] = getDigit(1,2);
      set[tokens.months] = (1).upto(12).map(function(n) { return n + set[tokens.monthSuffix]; });
    }
  ...
  function getCommonLocalization(code) {
    ...
    arrayEach([tokens.months,tokens.weekdays,tokens.units,tokens.numbers,tokens.articles,tokens.optionals,tokens.formats], function(name, i) {
      set[name] = pre[i + 2] ? pre[i + 2].split(',') : [];
    });
    set[tokens.outputFormat] = pre[9];
    arrayEach([tokens.day,tokens.sign,tokens.shift,tokens.edge], function(name, i) {
      if(!pre[i + 10]) return;
      arrayEach(pre[i + 10].split(','),  function(t, j) {
        if(t) set[tokens.modifiers].push({ name: name, text: t, value: j - 2 });
      });
    });
    if(bool(1)) {
      set[tokens.digits] = LowerAsianDigits + UpperAsianDigits;
      if(set[tokens.numbers].length > 0) {
        set[tokens.digits] += set[tokens.numbers].join('');
      } else {
        set[tokens.numbers] = LowerAsianDigits.split('');
      }
      set[tokens.monthSuffix] = pre[1];
    }
    set[tokens.capitalizeUnit] = (code == 'de');
    set[tokens.hasPlural] = bool(2);
    set[tokens.pastRelativeFormat] = set[tokens.formats][0];
    set[tokens.futureRelativeFormat] = set[tokens.formats][bool(3) ? 1 : 0];
    return set;
  }

  function getVariant(locale) {
    if(!locale) locale = Date[tokens.currentLocale];
  ...
  extend(Localization, true, false, {
    ...
    getMonth: function(n) {
      if(object.isNumber(n)) {
        ...
        return arrayFind(this[tokens.months], regexp(n, 'i'), 0, false, true) % 12;
      }
    },

    getWeekday: function(n) {
      return arrayFind(this[tokens.weekdays], regexp(n, 'i'), 0, false, true) % 7;
    },
    ...
    getNumber: function(n) {
      ...
      if(object.isNumber(n)) {
        return n;
      } else if(n && (i = this[tokens.numbers].indexOf(n)) !== -1) {
        return (i + 1) % 10;
      } else {
        return 1;
      }
    ...
    getNumericDate: function(n) {
      var self = this;
      return n.replace(this[tokens.numbers][9], '').each(function(d) {
        return self.getNumber(d);
      }).join('');
    },

    getEnglishUnit: function(n) {
      return English[tokens.units][this[tokens.units].indexOf(n) % 8];
    },

    relative: function(num, u, ms) {
      var format, sign, unit, last, mult;
      format = ms > 0 ? this[tokens.futureRelativeFormat] : this[tokens.pastRelativeFormat];
      if(this[tokens.code] == 'ru') {
        ...
        mult = this[tokens.hasPlural] && num > 1 ? 1 : 0;
      }
      unit = this[tokens.units][mult * 8 + u] || this[tokens.units][u];
      if(this[tokens.capitalizeUnit]) unit = unit.capitalize();
      sign = arrayFind(this[tokens.modifiers], function(m) { return m.name == tokens.sign && m.value == (ms > 0 ? 1 : -1); });
  ...
  function convertAsianDigits(str, key) {
    if(key != tokens.date && key != tokens.month && key != tokens.year) return str;
  ...
  function getFormatMatch(match, arr) {
    ...
    arrayEach(arr, function(key, i) {
      ...
      if(key === tokens.year) obj.yearAsString = value;
      if(key === tokens.millisecond) value = value * Math.pow(10, 3 - value.length);
    ...
    if(object.isDate(f)) {
      ...
      arrayEach(DateInputFormats, function(dif) {
        ...
        if(match) {
          ...
          if(format.variant && !object.isString(set[tokens.month]) && (object.isString(set[tokens.date]) || variant)) {
            tmp = set[tokens.month];
            set[tokens.month] = set[tokens.date];
            set[tokens.date] = tmp;
          }

          // If the year is 2 digits then get the implied century.
          if(set[tokens.year] && set.yearAsString.length === 2) {
            set[tokens.year] = getYearFromAbbreviation(set[tokens.year]);
          }

          // Set the month which may be localized.
          if(set[tokens.month]) {
            set[tokens.month] = loc.getMonth(set[tokens.month]);
            if(set[tokens.shift] && !set[tokens.unit]) set[tokens.unit] = tokens.year;
          }

          // If there is both a weekday and a date, the date takes precedence.
          if(set[tokens.weekday] && set[tokens.date]) {
            delete set[tokens.weekday];
          // Otherwise set a localized weekday.
          } else if(set[tokens.weekday]) {
            set[tokens.weekday] = loc.getWeekday(set[tokens.weekday]);
            if(set[tokens.shift] && !set[tokens.unit]) set[tokens.unit] = tokens.week;
          }

          // Relative day localizations such as "today" and "tomorrow".
          if(set[tokens.day] && (tmp = loc.modifiersByName[set[tokens.day]])) {
            set[tokens.day] = tmp.value;
            d.resetTime();
            relative = true;
          // If the day is a weekday, then set that instead.
          } else if(set[tokens.day] && (tmp = loc.getWeekday(set[tokens.day])) > -1) {
            delete set[tokens.day];
            set[tokens.weekday] = tmp;
          }

          if(set[tokens.date] && !object.isNumber(set[tokens.date])) {
            set[tokens.date] = loc.getNumericDate(set[tokens.date]);
          }

          // If the time is 1pm-11pm advance the time by 12 hours.
          if(set[tokens.meridian]) {
            if(set[tokens.meridian] === 'pm' && set[tokens.hour] < 12) set[tokens.hour] += 12;
          }

          // Adjust for timezone offset
          if(set[tokens.offset_hours] || set[tokens.offset_minutes]) {
            set[tokens.utc] = true;
            set[tokens.offset_minutes] = set[tokens.offset_minutes] || 0;
            set[tokens.offset_minutes] += set[tokens.offset_hours] * 60;
            if(set[tokens.offset_sign] === '-') {
              set[tokens.offset_minutes] *= -1;
            }
            set[tokens.minute] -= set[tokens.offset_minutes];
          }

          // Date has a unit like "days", "months", etc. are all relative to the current date.
          if(set[tokens.unit]) {
            relative = true;
            num = loc.getNumber(set[tokens.num]);
            unit = loc.getEnglishUnit(set[tokens.unit]);

            // Shift and unit, ie "next month", "last week", etc.
            if(set[tokens.shift] || set[tokens.edge]) {
              num *= (tmp = loc.modifiersByName[set[tokens.shift]]) ? tmp.value : 0;

              // Relative month and static date: "the 15th of last month"
              if(unit === tokens.month && set[tokens.date] !== Undefined) {
                d.set({ 'day': set[tokens.date] }, true);
                delete set[tokens.date];
              }

              // Relative year and static month/date: "June 15th of last year"
              if(unit === tokens.year && set[tokens.month] !== Undefined) {
                d.set({ 'month': set[tokens.month], 'day': set[tokens.date] }, true);
                delete set[tokens.month];
                delete set[tokens.date];
              }
            }

            // Unit and sign, ie "months ago", "weeks from now", etc.
            if(set[tokens.sign] && (tmp = loc.modifiersByName[set[tokens.sign]])) {
              num *= tmp.value;
            }

            // Units can be with non-relative dates, set here. ie "the day after monday"
            if(set[tokens.weekday] !== Undefined) {
              d.set({'weekday': set[tokens.weekday] }, true);
              delete set[tokens.weekday];
            }
      ...
      if(set && set[tokens.edge]) {
        tmp = loc.modifiersByName[set[tokens.edge]];
        ...
        if(unit === tokens.year) set.specificity = tokens.month;
        else if(unit === tokens.month || unit === tokens.week) set.specificity = tokens.day;
        d[(tmp.value < 0 ? tokens.endOf : tokens.beginningOf) + unit.capitalize()]();
  ...
  function compareDate(d, find, buffer) {
    ...
    if(p.set && p.set.specificity) {
      ...
      if(p.set[tokens.edge] || p.set[tokens.shift]) {
        p.date[tokens.beginningOf + p.set.specificity.capitalize()]();
      }
      if(!override && p.set[tokens.sign] && p.set.specificity != tokens.millisecond) {
  ...
  function updateDate(date, params, reset, utc, advance) {
    ...
    if(params[tokens.date]) params[tokens.day] = params[tokens.date];
    // If a weekday is included in the params, set it ahead of time and set the params
    // to reflect the updated date so that resetting works properly.
    if(!advance && params[tokens.day] === Undefined && params[tokens.weekday] !== Undefined) {
      callDateMethod(date, tokens.set, utc, tokens.Weekday, params[tokens.weekday])
      params[tokens.day] = callDateMethod(date, tokens.get, utc, tokens.Date);
      delete params[tokens.weekday];
    }
    ...
    arrayEach(DateUnitsReversed, function(u) {
      if(params[u.unit] !== Undefined || params[u.unit + 's'] !== Undefined) {
        ...
        return false;
      } else if(reset && u.unit !== tokens.week && u.unit !== tokens.year) {
        callDateMethod(date, tokens.set, utc, u.method, (u.unit === tokens.day) ? 1 : 0);
      }
    });
    ...
    if(advance && params[tokens.month] !== Undefined) {
    ...
    arrayEach(DateUnits, function(u,i) {
      ...
      if(advance) {
        if(unit === tokens.week) {
          value  = (params[tokens.day] || 0) + (value * 7);
          method = tokens.Date;
        }
        value = (value * advance) + callDateMethod(date, tokens.get, '', method);
      }
      callDateMethod(date, tokens.set, utc, method, value);
    });
    return date;
  }
  ...
  function getShortHour(d, utc) {
    var hours = callDateMethod(d, tokens.get, utc, 'Hours');
  ...
  function checkMonthTraversal(d, set, advance) {
    ...
    if(targetDate < 29 || (set.specificity != tokens.month && set.specificity != tokens.year)) return;
    daysInTargetMonth = new date(d.getFullYear() + ((set[tokens.year] * advance) || 0), d.getMonth() + (set[tokens.month] * advance)).daysInMonth();
  ...
  function buildDateMethods() {
    ...
    arrayEach(DateUnits, function(u, i) {
      ...
      if(i < 4) {
        defineProperty(date.prototype, tokens.beginningOf + caps, function() {
          var set = {};
          switch(unit) {
            case tokens.year:  set[tokens.year] = this.getFullYear(); break;
            case tokens.month: set[tokens.month] = this.getMonth(); break;
            case tokens.day:   set[tokens.day] = this.getDate(); break;
            case tokens.week:  set[tokens.weekday] = 0; break;
          }
          return this.set(set, true);
        });
        defineProperty(date.prototype, tokens.endOf + caps, function() {
          var set = { 'hours': 23, 'minutes': 59, 'seconds': 59, 'milliseconds': 999 };
          switch(unit) {
            case tokens.year:  set[tokens.month] = 11; set[tokens.day] = 31; break;
            case tokens.month: set[tokens.day] = this.daysInMonth(); break;
            case tokens.week:  set[tokens.weekday] = 6; break;
          }
          return this.set(set, true);
        });
      }
  ...
  function buildDateInputFormats() {
    ...
    var monthReg = '\\d{1,2}|' + English[tokens.months].join('|');
  ...
  function buildRelativeAliases() {
    var weekdays = English[tokens.weekdays].slice(0,7);
    var months = English[tokens.months].slice(0,12);
    arrayEach(['today','yesterday','tomorrow',tokens.weekday,tokens.weekend,tokens.future,tokens.past].concat(weekdays).concat(months), function(s) {
  ...
  extend(date, false, false, {
    ...
    'setLocale': function(code, set) {
      ...
      if(loc) {
        Date[tokens.currentLocale] = code;
  ...
  extend(date, true, false, {
    ...
    'is': function(d, margin) {
      ...
      if(object.isString(d)) {
        ...
        switch(true) {
          case d === tokens.future:  return this.getTime() > new date().getTime();
          case d === tokens.past:    return this.getTime() < new date().getTime();
          case d === tokens.weekday: return this.getDay() > 0 && this.getDay() < 6;
          case d === tokens.weekend: return this.getDay() === 0 || this.getDay() === 6;
          case (tmp = English[tokens.weekdays].indexOf(d) % 7) > -1: return this.getDay() === tmp;
          case (tmp = English[tokens.months].indexOf(d) % 12) > -1:  return this.getMonth() === tmp;
        }
      }
  ...
  regexp.NPCGSupport = reg(/()??/).exec('')[1] === Undefined; // NPCG: nonparticipating capturing group
