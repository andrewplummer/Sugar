(function() {
  ...
  var shiftableRanges = [
    { start: 12449, end: 12527, shift: 52929, is: 'k' }
  ];
  ...
  var getConversionMode = function(args) {
    var mode = Array.prototype.slice.call(args).join('');
    mode = mode.replace(/all/, '');
    mode = mode.replace(/(\w)lphabet|umbers?|atakana|paces|unctuation/g, '$1');
    return mode;
  }

  var characterCodeRanges = [
    {
      name: 'a',
      shift: 65248,
      lower: /[\u0041-\u005A\u0061-\u007A]/,
  ...
  var matchCodeRange = function(mode, c, range) {
    characterCodeRanges.each(function(r) {
      if((!mode || mode.has(r.name)) && r[range].test(c)) {
        console.info(c.charCodeAt(0));
        c = String.fromCharCode(c.charCodeAt(0) - 65248);
        console.info('well wtf', c);
        return false;
      }
    });
    console.info('hmm', c);
  ...
  extend(String, true, {
     ...
     ***/
    'hankaku': function() {
      var str = this, mode = getConversionMode(arguments);
      str = str.replace(/[\uff01-\uff5e]/g, function(c) {
        return matchCodeRange(mode, c, 'upper');
        //return String.fromCharCode(c.charCodeAt(0) - 65248);
        //code = c.charCodeAt(0);
        //console.info('whoa', arguments);
      });
      return str;
      var result = '';
      this.codes(function(c) {
        console.info(c);
        shiftableRanges.each(function(s) {
