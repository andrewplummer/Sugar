(function() {
  ...
  function cloneObject(obj, deep) {
    ...
    iterateOverObject(obj, function(k,v) {
      if(deep === true && (Object.isObject(v) || Array.isArray(v))) {
  ...
  extend(Number, true, false, {
    ...
    'format': function(place, comma, period) {
      var split, integer, decimal, r = reg(/(\d+)(\d{3})/), autoDecimal = place === undefined;
      place = place || 0;
      comma = comma || ',';
      period = period || '.';
      ...
      integer = split[0];
      decimal = split[1] || '';
      decimal = split.length > 1 || !autoDecimal ? period + padString(decimal, '0', 0, place - decimal.length) : '';
      while (r.test(integer)) {
        integer = integer.replace(r, '$1' + comma + '$2');
      }
