(function() {
  ...
  var checkLanguageInitialized = function(lang) {
    return;
    if(InitializedLanguages[lang]) return;
  ...
  var getExtendedDate = function(f) {
    ...
    var utc = false;
    var d = new Date();
    console.info('wtf', dateInputFormats.length);
    ...
    if(Object.isDate(f)) {
      ...
      f = f.trim().replace(/\.+$/,'').replace(/^now$/, '');
        console.info('WHOAAA', dateInputFormats.length);
  ...
  var createDate = function(args) {
    ...
    if(Object.isString(args[0])) {
      ...
      checkLanguageInitialized(args[1] || 'en');
    }
  ...
  extend(Date, false, false, {
     ...
     ***/
    'create': function() {
      console.info(dateInputFormats.length);
...
dateInputFormats = 'sdflkajslfkjs';
console.info(dateInputFormats.length);
      if(set.months) {
        var allMonths = set.months.map(function(m) {
          var index = set.abbreviatedMonthLength;
          return index ? m.to(index) + '(?:\\.|' + m.from(index)+')?' : m;
        }).join('|');
      }

      if(set.weekdays) {
        var allWeekdays = set.weekdays.map(function(w) {
          var index = set.abbreviatedWeekdayLength;
          return index ? w.to(index) + '(?:\\.|' + w.from(index)+')?' : w;
        }).join('|');
      }

      if(set.numbers) {
        var allNumbers = set.numbers.join('|');
      }

      arrayEach(TextBasedDateFormats, function(f) {
        var src = '^' + f.src;
        var to = f.to.concat();
        src = src.replace(/{WEEKDAYS}/, allWeekdays);
        src = src.replace(/{MONTHS}/, allMonths);
        src = src.replace(/{NUMBER}/, allNumbers);
        if(!f.timeImplied) {
          src = src + optionalTime;
          to = to.concat(['hour','minute','second','millisecond','meridian','utc','offset_sign','offset_hours','offset_minutes']);
        }
        console.info(src, to, dateInputFormats.length);
        //dateInputFormats.push({ reg: new RegExp(src, 'i'), to: to });
        console.info(src, to, dateInputFormats.length);
      });
