(function() {
  ...
  extend(String, true, false, {
    ...
    'truncate': function(length, append, split) {
      ...
      if(!length || this.length <= length + append.length) return this;
      a = /^(.)\1+$/.test(append) ? append.slice(0,1) : '';
      reg = new RegExp('[^' + trimmableCharacters + a + '][' + trimmableCharacters + a + ']');
      while(length && !reg.test(this.slice(length - 1, length + 1)) && split !== true) {
        length--;
      }
      return this.slice(0, length) + (length ? append : '');
    }
