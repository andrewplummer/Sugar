(function() {
  ...
  function formatDate(date, f, relative, locale) {
    var adu, loc = getLocalization(locale, true), caps = /^[A-Z]/, value;
    if(!date.isValid()) {
      return 'Invalid Date';
    } else if(Date[f]) {
      f = Date[f];
  ...
  function compareDate(d, find, buffer) {
    ...
    if(p.set && p.set.specificity) {
      ...
      if(!override && p.set.sign && p.set.specificity != 'millisecond') {
        // If the time is relative, there can occasionally be an disparity between when the relative date
        // and "now", which it is being compared to, so pis set an
        loBuffer = 5;
        hiBuffer = -5;
      }
    }
