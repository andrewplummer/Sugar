(function() {
  ...
  function transformArgument(arr, map, el, index, scope) {
    if(map === Undefined) {
      return el;
    } else if(object.isFunction(map)) {
      return map.apply(scope || arr, [el].concat([index, arr]));
  ...
  function arrayUnique(arr, map) {
    var prop, test = function(el) { return transformArgument(arr, map, el) === prop; };
    return arr.reduce(function(result, next, index) {
      prop = transformArgument(arr, map, next, index);
  ...
  function getMinOrMax(obj, map, which, isArray) {
    ...
    iterateOverObject(obj, function(key) {
      var entry = obj[key];
      var test = transformArgument(obj, map, entry, isArray ? key.toNumber() : null);
  ...
  extend(array, true, function() { var a = arguments; return a.length === 0 || object.isFunction(a[0]); }, {
    ...
    'map': function(map, scope) {
      ...
      while(index < length) {
        if(index in this) {
          result[index] = transformArgument(this, map, this[index], index, scope);
        }
        index++;
      }
      return result;
  ...
  extend(array, true, false, {
    ...
    'groupBy': function(map, fn) {
      ...
      arrayEach(arr, function(el, index) {
        key = transformArgument(arr, map, el, index);
    ...
    'sortBy': function(map, desc) {
      ...
      arr.sort(function(a, b) {
        var aProperty = transformArgument(arr, map, a);
        var bProperty = transformArgument(arr, map, b);
