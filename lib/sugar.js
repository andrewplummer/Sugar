(function() {
  ...
  var compareDate = function(d, find, buffer, dir, edges) {
    ...
    if(p.set && p.set.specificity) {
      unit = dateUnits.find(function(u) {
        return u.unit === p.set.specificity;
      });
      accuracy = unit.multiplier(p.date, d - p.date) - 1;
      console.info("GOTCHO ACCURACY", p.set, accuracy / 60 / 60 / 24, p.date, p.format);
    /*
    arrayEach(dateUnits, function(u) {
    ...
    */
      if(p.format.to.find('shift')) {
        var beginning = p.date['beginningOf'+ unit.unit.capitalize()];
        if(beginning) {
