...
(function(){
  ...
  var getMinOrMax = function(obj, which, transform){
    ...
    if((max && edge === -Infinity) || (min && edge === Infinity)) return null;
    return result;
  }
  ...
  var splitAllowsUndefined = 'a'.split(/a/).length === 2;

  if(!splitAllowsUndefined || true){
    ...
    String.prototype.split = function(separator, limit){
      ...
      while(match = separator.exec(str)){
        if(match.index + match[0].length === match.index){
          // zero-width match
          ...
          result.push(stored + match.input.slice(0, match.index));
          stored = '';
          str = str.slice(match.index + match.length);
        }
      }
      result.push(stored + str);
      return result;
    }
  }
  ...
  // Unconditionally overriding the native .split method,
  // which is unstable for complicated expressions using RegExp.
  // /*

      /*
  var nativeStringSplit = String.prototype.split;
  //var delimiter = '\u001F';
  var delimiter = '{SPLITHERE}';
  var removeEnds = new RegExp('(?:^'+delimiter+'|'+delimiter+'$)', 'g');



  String.prototype.split3 = function(separator, limit){
    if(!(separator instanceof RegExp)){
      return nativeStringSplit.apply(this, arguments);
    }
    ...
    var reg = new RegExp(separator.source, flags + 'g');
    if(this.length == 0){
      return reg.test(this) ? [] : [''];
    }
    str = str.replace(reg, delimiter);
    ...
    var npcg = /()??/.exec('')[1] === undefined; // NPCG: nonparticipating capturing group
  ...
  extend(Array, {
    ...
    'min': function(){
      var args = getArgumentsWithOptionalBlock(arguments);
      var result = getMinOrMax(this, 'min', args.block);
      return (result === null || (args.first === true && args.block)) ? result : result.first();
    },

    'max': function(){
      var args = getArgumentsWithOptionalBlock(arguments);
      var result = getMinOrMax(this, 'max', args.block);
      return (result === null || (args.first === true && args.block)) ? result : result.first();
    },
    ...
    'least': function(){
      ...
      var result = getMinOrMax(grouped, 'min', 'length');
      if(args.first === true && args.second === true) return result;
      else if(args.first === true) return result[0];
      else return args.block ? args.block(result[0][0]) : result[0][0];
    },
    ...
    'most': function(){
      ...
      var args = getArgumentsWithOptionalBlock(arguments);
      var grouped = this.group(args.block);
      var result = getMinOrMax(grouped, 'max', 'length');
      if(args.first === true && args.second === true) return result;
      else if(args.first === true) return result[0];
      else return args.block ? args.block(result[0][0]) : result[0][0];
    },
    ...
    'inGroups': function(num){

          var division = this.length / num;
          var modulo = this.length % num;
          var fill_with = 'hmm';
          ...
          num.times(function(index){
            ...
            var padding = (fill_with != false && modulo > 0 && length == division ? 1 : 0)
            var slice = arr.slice(start, length);
            if(padding == 1) slice.concat([fill_with]);
            groups.push(slice);
            start += length;
          });
        return groups;
