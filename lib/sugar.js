    ...
    if(Object.isDate(f)) {
      ...
      arrayEach(dateInputFormats, function(df) {
        ...
        if(match) {
          ...
          console.info('sorry', f, df, df.reg.source, m);
          ...
          if(m.month) {
            ...
            if(isNaN(num)) {
              ...
              m.month = DateLanguages[lang].months.indexOf(m.month) % 12;
            } else {
          ...
          if(m.day) {
            ...
            var val;
            // SHORTEN ME
            if((val = DateLanguages[lang].weekdays.indexOf(m.day) % 7) !== -1) {
              delete m.day;
              set.weekday = val;
            } else if(val = DateLanguages[lang].modifiersByName[m.day]) {
              delete m.day;
              format.relative = true;
              set.day = val.value;
              console.info('ARFFFF', set, set.day, typeof set.day);
            }
            /*
            ...
            if(fuzzy === 'yesterday') {
              ...
              dayOffset = 0;
              if(m.sign && !m.unit && !m.modifier_amount) {
                m.unit = 'week';
              }
            }
          ...
          if(m.unit) {
            // SHORTEN ME
            var amt  = (parseFloat(m.num) || (DateLanguages[lang].numbers.find(m.num) + 1) || 1);
            // SHORTEN ME
            var engUnit = EnglishUnits[DateLanguages[lang].units.indexOf(m.unit) % 8];
            /* DEPRECATE ME!!
            var textNumIndex;
            if(amt === 'the' || amt === 'a') {
              amt = 1;
           ...
           if(m.shift) {
             format.relative = true;
             var tmp = DateLanguages[lang].modifiersByName[m.shift];
             var shift = tmp ? tmp.value : 0;
             amt *= shift;
           }
            if(m.sign) {
              format.relative = true;
              amt *= DateLanguages[lang].modifiersByName[m.sign].value;
            }
            /* NEED THIS????
            if(m.unit === DateLanguages[lang].units[6] && !format.relative) {
              set.year = d.getFullYear();
            } else if(m.unit === DateLanguages[lang].units[5] && !format.relative) {
              set.month = d.getMonth();
            } else*/
           if(m.unit === DateLanguages[lang].units[4] && !format.relative) {
             set.day = d.getDate();
             unit = 'day';
             amt *= 7;
           }
           ...
           set[engUnit] += amt;
           console.info(set, m.unit, set[engUnit], 'whoaaaa', amt, ' ohhhhhhhhh', m.modifier_amount, 'meeeeeee');
         }
          if(m.edge) {
            //console.info('GETTING IT', m.edge, DateLanguages[lang].modifiersByName[m.edge]);
            //var edge = m.edge;
            //var amt  = (parseFloat(m.modifier_amount) || (DateLanguages[lang].numbers.find(m.modifier_amount) + 1) || 1) * (DateLanguages[lang].modifiersByName[m.sign].value || 1);
            if(DateLanguages[lang].modifiersByName[m.edge].value === 1) {
              if(m.unit === 'week') {
                set.month = d.getMonth();
                set.weekday = 0;
              } else if(m.unit === 'month' || m.month) {
                set.day = 1;
              }
              //if(!edge.match(/day/)) {
                ...
                //set.millisecond = 0;
              //}
            } else if(DateLanguages[lang].modifiersByName[m.edge].value === -1) {
              console.info('GDSFFD?');
              if(m.unit === 'week') {
                set.month = d.getMonth();
                set.weekday = 6;
