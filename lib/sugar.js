...
(function(){
  ...
  var dateFormats = [
    ...
    { reg: /^([+-])?(\d{4})[-\/.]?(\d{1,2})[-\/.]?(\d{1,2})/, to: ['year_sign','year','month','day'] }, // Standard ISO8601
    ...
    { reg: new RegExp('^(?:'+weekdayReg+')?\\s*('+monthReg+')[\\s\\-.](\\d{1,2})(?:st|nd|rd|th)?,?[\\s\\-.](\\d{2,4})'), to: ['month','day','year'] }, // (dow) month dd yyyy
    { reg: new RegExp('^(\\d{1,2}) ('+monthReg+'),? (\\d{4})'), to: ['day','month','year'] }, // dd month yyyy
    { reg: new RegExp('^(?:(the|a|'+textNumberReg+'|\\d+) (day|week|month|year)s? (before|after)\\s+)?(today|tomorrow|yesterday|'+weekdayReg+')'), to: ['modifier_amount','modifier_unit','modifier_sign','fuzzy_day'] }, // dd month yyyy
    { reg: new RegExp('^(a|'+textNumberReg+'|\\d+) (milliseconds|seconds|minutes|hours|day|week|month|year)s? (from now|ago)'), to: ['modifier_amount','modifier_unit','modifier_sign'], relative: true }, // dd month yyyy
  ...
  dateFormats.each(function(d){
    d.reg = new RegExp(d.reg.source + timeReg.source, 'i');
    d.to = d.to.concat(['hours','minutes','seconds','milliseconds','meridian','utc','offset_sign','offset_hours','offset_minutes']);
  });
  ...
  var getDateFromString = function(str, variant){
    ...
    dateFormats.each(function(format){
      ...
      if(match){
        ...
        if(f.modifier_amount){
          var amt = f.modifier_amount;
          var textNumIndex;
          if(amt === 'the' || amt === 'a'){
            amt = 1;
          ...
          if(f.modifier_sign === 'before' || f.modifier_sign === 'ago'){
            amt *= -1;
          }
          f.modifier_amount = amt;
        }
        */
       ...
       /*
        ...
        if(f.modifier_sign === 'from now' || f.modifier_sign === 'ago'){
          var method = 'get' + f.modifier_unit.capitalize();
          if(method === 'getYear') method = 'getFullYear';
          if(method === 'getDay')  method = 'getDate';
          console.info('hoboy', f.modifier_sign, method);
          if(d[method]){
            f[f.modifier_unit] = d[method].call(d);
          }
        }
        */
        if(f.milliseconds){
        ...
        ['year','month','day','hours','minutes','seconds','milliseconds'].each(function(unit){
          if(f[unit] !== undefined){
...
//    d.setMinutes(d.getMinutes() - d.getTimezoneOffset());
  ...
  var dateMethods = [
    ...
    { unit: 'day', method: 'Date' },
    { unit: 'hour', method: 'Hours' },
    { unit: 'minute', method: 'Minutes' },
    { unit: 'second', method: 'Seconds' },
    { unit: 'millisecond', method: 'Milliseconds' }
  ];

  extend(Date, {
    set: function(params, reset, utc){
      var d = this;
      ...
      dateMethods.each(function(m, i){
        ...
        if(value === undefined && reset) value = defaults[i - 1];
        if(value !== undefined){
          d['set'+utc+m.method].call(d, value);
        }
      });
      return d;
    },

    setUTC: function(params, reset){
      this.set(params, reset, true);
    },

    advance: function(params){
      var d = this;
      var obj = typeof params === 'object';
      dateMethods.each(function(m, i){
        var value;
        if(obj){
          value = params[m.unit];
        ...
        if(value !== undefined){
          d['set'+m.method].call(d, (d['get'+m.method].call(d) + value));
        }
      });
      return d;
    }
