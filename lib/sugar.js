  ...
  */
  ...
  extend(Localization, true, false, {
    ...
    'relativeFormat': function(num, u, dir) {
      var format, sign, units = this.units, unit = units[u], index;
      format = this.formats[dir > 0 ? this.futureRelativeFormat : this.pastRelativeFormat].replace(/\{\d+\}/g, '');
      if(this.conjugations) {
        arrayEach(this.conjugations, function(n, i) {
          index = 1 + u + i * 8;
          if(num < n && units[index]) {
            unit = units[index];
          }
        });
      }
...
//
      ...
      var setArray = function(name, abbreviate, multiple) {
        var arr = [];
        if(set[name]) {
          arrayEach(set[name], function(n, i) {
            eachAlternate(n, function(m, j) {
              arr[j * multiple + i] = m.toLowerCase();
            });
          });
        }
        //revisit!!
      ...
      var setModifiers = function(name) {
        var modifiers = set.modifiers, arr = [];
        set.modifiersByName = {};
        arrayEach(modifiers, function(n) {
      ...
      console.info('mooooooooooooooo00000ooooo', set);
