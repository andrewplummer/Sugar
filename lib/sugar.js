(function() {
  ...
  var setParamsObject = function(obj, key, value) {
    var reg = /\[(.*)\]$/, match, inner;
    if(match = key.match(reg)) {
      key = key.replace(reg, '');
      inner = match[1];
      if(!obj[key]) {
        obj[key] = !inner || inner.match(/^\d+$/) ? [] : {};
      }
      if(!inner && Object.isArray(obj[key])) inner = obj[key].length.toString();
      setParamsObject(obj[key], inner, value);
  ...
  extend(Object, false, false, {
    ...
    /***
     * @method Object.fromQueryString(<str>, <sep1> = '&', <sep2> = '=')
     * @returns Object
     ...
     *   Object.fromQueryString('a|few,more|params')   -> { a: 'few', more: 'params' }
     *
     ***/
    'fromQueryString': function(str) {
      var result = {}, split, key, value;
      str.unescapeURL().split('&').each(function(p) {
        var split = p.split('=');
        setParamsObject(result, split[0], split[1]);
      });
      return result;
    }
  ...
  extend(String, true, false, {
    ...
    /***
     * @method truncate(<num>, [split] = false, [append] = '...')
     * @returns Object
     ...
     *
     ***/
    'truncate': function(num, split, append) {
      var reg, a;
      if(append === undefined) append = '...';
      if(!num || this.length <= num + append.length) return this;
      a = /^(.)\1+$/.test(append) ? append.slice(0,1) : '';
      reg = new RegExp('[^' + trimmableCharacters + a + '][' + trimmableCharacters + a + ']');
      while(num && !reg.test(this.slice(num - 1, num + 1)) && split !== true) {
        num--;
      }
      return this.slice(0, num) + (num ? append : '');
    },
