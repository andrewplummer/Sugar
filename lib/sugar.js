(function() {
      ...
      arrayEach(dateInputFormats, function(dif) {
        ...
        if(match) {
          ...
          */
          ...
          if(set.unit) {
            // SHORTEN ME
            var amt  = (parseFloat(set.num) || (DateLanguages[lang].numbers.find(set.num) + 1) || 1);
            // SHORTEN ME
          ...
          if(set.edge) {
            ...
            if(DateLanguages[lang].modifiersByName[set.edge].value === 1) {
              if(set.unit === 'week') {
                set.month = d.getMonth();
                set.weekday = 0;
              } else if(set.unit === 'month' || set.month) {
                set.day = 1;
              }
              //if(!edge.match(/day/)) {
  ...
  var buildDateInputFormats = function() {
    ...
    arrayEach(numeralInputFormats, function(f) {
      ...
      console.info("WOOWOWOWOWO", f.src);
...
//
      ...
      var setModifiers = function(name) {
        arrayEach(set.modifiers.map('name').unique(), function(t) {
          poop[t] = set.modifiers.filter(function(m) {
            if(!set.modifiersByName[m.text]) set.modifiersByName[m.text] = m;
            return m.name === t;
          }).map('text').join('|');
        });
      }
  ...
  extend(Date, true, false, {
     ...
     * @extra This method can accept multiple formats including a single number as a timestamp, an object, or enumerated parameters (as with the Date constructor). If [reset] is %true%, any units that are not specified will be reset (except the year).
     * @example
     *
