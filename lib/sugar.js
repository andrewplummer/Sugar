(function() {
  ...
  var getLocalization = function(lang) {
    return Localizations[lang] || Localizations[currentLocalization];
  };

  var setLocaleFormats = function(code) {
    if(!Object.isString(code)) code = 'en';
  ...
  var initializeLocale = function(code, set, current) {
    if(current) currentLocalization = code;
    ...
    var getNum = function(start, stop) {
      var text = '', extra = '', suffix = '';
      if(!start || set.numbersAreOrdinal) {
        suffix = '+';
      } else {
        suffix = '{' + start + (stop ? ',' + stop : '') + '}';
      }
      if(set.numbersAreOrdinal) {
        extra = set.numbers.join('');
      } else if(!start) {
        text = set.numbers.concat(set.articles || []).join('|') + '|';
      }
      return text + '[0-9０-９' + extra + ']' + suffix;
    }
  ...
  var getLocalizationSet = function(code) {
    // UPDATE ME!!
    return CommonLocalizations[code];
  };
