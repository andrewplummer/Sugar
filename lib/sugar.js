(function() {
  ...
  extend(String, true, false, {
     ...
     *
     ***/
    'truncate': function(n, splitWords, append) {
      var reg, a;
      if(append === undefined) append = '...';
      if(!n || this.length <= n + append.length) return this;
      a = /^(.)\1+$/.test(append) ? append.slice(0,1) : '';
      reg = new RegExp('[^' + trimmableCharacters + a + '][' + trimmableCharacters + a + ']');
      while(n && !reg.test(this.slice(n - 1, n + 1)) && splitWords !== true) {
        n--;
      }
      return this.slice(0, n) + (n ? append : '');
    },
  ...
  extend(Function, true, false, {
     ...
     * @extra This method will wrap the function inside another that will wait until the CPU is free to execute. This prevents "freezing" on heavy operations. Although this will act exactly as %defer% in other frameworks, it also works in conjunction with loops like %each%, in which the returned function will be called multiple times. By default each iteration of <fn> will be called 1ms after the last, or the iteration number * [wait]. By passing in a smaller value for [wait] (can be a decimal < 1), you can "tighen up" the execution time so that the iterations happen faster. By passing in a greater value for [wait], you can space the function execution out to prevent thread blocking. Playing with this number is the easiest way to strike a balance for heavier operations.
     * @example
     *
     *   (function() {
     *     // Executes 1ms later
    ...
    'lazy': function(wait) {
     ...
     /***
     * @method delay(<wait>, [arg1], ...)
     * @returns Function
     * @short Executes the function after <wait> milliseconds.
     * @extra Functions using the delay method can be canceled using the %cancel% method. Can also curry arguments passed in after <wait>.
     * @example
     *
     *   (function(arg1) {
     *     // called 1s later
    ...
    'cancel': function() {
      ...
      if(this.timers) this.timers.each(clearTimeout);
      return this;
    }
