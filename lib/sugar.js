(function() {
  ...
  var regexShorthandSupport = typeof /./.exec !== 'undefined';
  ...
  extend(Number, true, false, {
    ...
    'format': function(place, comma, period) {
      ...
      str = Object.isNumber(place) ? this.toFixed(place) : this.toString();
      comma = comma || ',';
      period = period || '.';
      split = str.split('.');
      result = split[0];
  ...
  function getLocalization(code, fallback, set) {
    if(fallback && (!Object.isString(code) || !code)) code = Date[poop.currentLocale];
    ...
    if(Object.isDate(f)) {
      ...
      arrayEach(DateInputFormats, function(dif) {
        ...
        if(match) {
          ...
          if(format.variant && !Object.isString(set[poop.month]) && (Object.isString(set[poop.date]) || variant)) {
            tmp = set[poop.month];
          ...
          if(set[poop.date] && !Object.isNumber(set[poop.date])) {
    ...
    arrayEach(DateOutputFormats, function(dof) {
      f = f.replace(new RegExp('\\{('+dof.token+')(\\d)?\\}', dof.word ? 'i' : ''), function(m,t,d) {
        ...
        if(dof.word) {
          ...
          val = (Object.isNumber(val) ? val.pad(l) : val).last(l);
        }
        return val;
      });
    });
  ...
  function updateDate(date, params, reset, utc, advance) {
    ...
    if(!advance && params[poop.day] === undefined && params[poop.weekday] !== undefined) {
    ...
    if(advance && params[poop.month] !== undefined) {
