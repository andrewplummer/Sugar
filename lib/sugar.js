(function() {
  ...
  var extendWithNativeCondition = function(klass, instance, condition, methods) {
    ...
    iterateOverObject(methods, function(name, fn) {
      if(condition === false || !extendee[name]) {
        defineProperty(extendee, name, fn);
  ...
  var buildObject = function() {
    defineProperty(Object, 'isArray', Array.isArray);
  ...
  extendWithNativeCondition(Object, false, function() { return arguments.length < 2; }, {
  ...
  extendWithNativeCondition(Array, true, function() { return arguments.length === 0 || Object.isFunction(arguments[0]); }, {
    ...
    'every': function(f, scope) {
      ...
      while(index < length) {
        if(this.hasOwnProperty(index) && !multiMatch(this[index], f, scope, [index, this])) {
          return false;
        }
        index++;
      }
    ...
    'map': function(f, scope) {
      ...
      while(index < length) {
        if(this.hasOwnProperty(index)) {
          result[index] = transformArgument(this[index], f, scope, [index, this]);
        }
        index++;
      }
      return result;
  ...
  extend(Array, true, {
    ...
    'indexOf': function(search, fromIndex) {
      ...
      while(index < length) {
        if(this.hasOwnProperty(index) && this[index] === search) {
          return index;
        }
        index++;
      }
     ...
     * @extra Parameters passed to <fn> are identical to %forEach%, ie. the first parameter is the current element, second parameter is the current index, and third parameter is the array itself. If <fn> returns %false% at any time it will break out of the loop. Once %each% finishes, it will return the array. If [fromIndex] is passed, <fn> will begin at that index and work its way to the end. If [loop] is true, it will then start over from the beginning of the array and continue until it reaches [fromIndex] - 1.
     * @example
     *
     ...
     *
     ***/
    'each': function(fn, fromIndex, loop) {
      var length, index, i;
      if(!fn || !fn.call) {
        throw new TypeError();
      }
      i = toIntegerWithDefault(fromIndex, 0);
      length = loop === true ? this.length + i : this.length;
      while(i < length) {
        index = i % this.length;
        if(!this.hasOwnProperty(index)) {
          return iterateOverSparseArray(this, fn, i, loop);
        } else if(fn.call(this, this[index], index, this) === false) {
          break;
        }
        i++;
      }
    ...
    /***
     * @method findLast(<f>, [index] = 0, [loop] = false)
     * @returns Mixed
     * @short Returns the first element that matches <f>.
     * @extra <f> will match a string, number, array, object, or alternately test against a function or regex. Starts at [index], and will continue from index = 0 if [loop] is true.
     * @example
     *
     +   [{a:1,b:2},{a:1,b:3},{a:1,b:4}].findLast(function(n) {
     *     return n['a'] == 1;
     *   });                                     -> {a:1,b:3}
     *   ['cuba','japan','canada'].findLast(/^c/, 2) -> 'canada'
     *
     ***/
    'findLast': function(f, index, loop) {
      return this.reverse().find(f, index, loop);
    },
  ...
  var buildTrim = function() {
    var supported = /^\s+$/.test(trimmableCharacters);
    var trimL = new RegExp('^['+trimmableCharacters+']+');
    var trimR = new RegExp('['+trimmableCharacters+']+$');
    extendWithNativeCondition(String, true, supported, {
  ...
  extend(String, true, {
     ...
     *   'タロウ　ＹＡＭＡＤＡです！'.hankaku('s') -> 'タロウ ＹＡＭＡＤＡです！'
     *   'タロウ　ＹＡＭＡＤＡです！'.hankaku('p') -> 'タロウ　ＹＡＭＡＤＡです!'
     *
     * 52929
     *
     * halfwidth = ff61 - ff9f
     ...
     *   'ﾀﾛｳ YAMADAです!'.zenkaku('s') -> 'ﾀﾛｳ　YAMADAです!'
     *   'ﾀﾛｳ YAMADAです!'.zenkaku('p') -> 'ﾀﾛｳ YAMADAです！'
     *
     ***/
    'zenkaku': function() {
  ...
  var checkMonthTraversal = function(d, set, advance) {
    ...
    dateUnits.slice(2).each(function(u) {
  ...
  var checkISOString = function() {
    ...
    extendWithNativeCondition(Date, true, support, {
  ...
  var buildBind = function() {
    ...
    if(Function.prototype.bind) {
      var F = function() {};
      var B = F.bind();
      supported = (new B instanceof B) && !(new F instanceof B);
    }
    extendWithNativeCondition(Function, true, supported, {
