(function() {
  ...
  //var optionalTime  = '\\s*(?:(?:t|at |\\s+)(\\d{1,2}):?(\\d{2})?:?(\\d{2})?(\\.\\d{1,6})?(am|pm)?(?:(Z)|(?:([+-])(\\d{2})(?::?(\\d{2}))?)?)?)?';
  ...
  */
  ...
  extend(Localization, true, false, {
    ...
    'getMonth': function(n) {
      var i;
      if(!isNaN(i = parseInt(n))) {
        return i - 1;
      } else if((i = this.months.indexOf(n) % 12) !== -1) {
        return i;
      } else {
        return null;
      }
    ...
    'getDay': function(n) {
      var i;
      if(i = this.modifiersByName[n]) {
        return i.value;
      } else if(!isNaN(i = parseInt(n))) {
        return i;
      } else if((i = this.weekdays.indexOf(n) % 7) !== -1) {
        return i;
      } else {
        return null;
      }
  ...
  var getExtendedDate = function(f) {
    var d = new Date(), utc = false, relative = false, match, format, set, unit, setAfter, loc, tmp;
    if(Object.isDate(f)) {
      d = f;
      ...
      arrayEach(dateInputFormats, function(dif) {
        if(match = NPCGMatch(f, dif.reg)) {
          format = dif;
          set = getFormatMatch(match, format.to);
          loc = DateLanguages[format.lang || 'en'];
          ...
          if(set.weekday && set.date) {
            ...
            set.day = loc.getWeekday(set.weekday);
          ...
          */
          ...
          if(set.unit) {
           ...
           if(set.shift && (tmp = loc.modifiersByName[set.shift])) {
             //tmp = DateLanguages[lang].modifiersByName[set.shift];
           ...
           if(set.sign && (tmp = loc.modifiersByName[set.sign])) {
             amt *= tmp.value;
             //DateLanguages[lang].modifiersByName[set.sign].value;
           }
      ...
      if(set && set.edge) {
        // TODO: CLEANUP
        var mod = loc.modifiersByName[set.edge];
        ...
        var method = (mod.value === -1 ? 'endOf' : 'beginningOf') + unit.capitalize();
        d[method].call(d);
...
//
    ...
    //{ reg: '(a|{NUMBER}|\\d+) (millisecond|second|minute|hour|day|week|month|year)s? (from now|ago)', to: ['modifier_amount','modifier_unit','modifier_sign'], relative: true },
      ...
      */
      ...
      DateLanguages[code] = new Localization(set);
