  ...
  var getExtendedDate = function(f, lang) {
    ...
    if(Object.isDate(f)) {
      ...
      arrayEach(dateInputFormats, function(df) {
        ...
        if(match) {
          format = df;
          console.info('sorry', f, df, df.reg.source);
          ...
          if(m.modifier_unit && m.modifier_sign) {
            // SHORTEN ME
            var amt  = (parseFloat(m.modifier_amount) || (DateLanguages[lang].numbers.find(m.modifier_amount) + 1) || 1) * DateLanguages[lang].modifiers.find(function(mod){ return mod.text === m.modifier_sign; }).value;
...
//'({MONTHS})[\\s\\-.](\\d{4})'
//
      var setArray = function(name, def, len) {
        var arr = set[name] || def;
        //arr = arr.map('toLowerCase');
        if(len) arr = arr.concat(arr.map(function(s) { return s.to(len); }));
        set[name] = arr;
      }
      ...
      arrayEach(set.formats, function(src) {
        ...
        src = src.replace(/\{(.+?)\}/g, function(m, name) {
          if(name === 'num') {
            to.push('modifier_amount');
            return '(' + (set.articles || '') + set.numbers.join('|') + '|\\d+)';
          } else if(name === 'unit') {
            to.push('modifier_unit');
            return '(' + set.units.join('|') + ')';
          } else {
            to.push('modifier_' + name);
            return '(' + set.modifiers.filter(function(m){ return m.name == name; }).map('text').join('|') + ')';
          }
        });
        addFormat(src, { to: to });
        console.info(dateInputFormats[0].reg.source, to);
      });
