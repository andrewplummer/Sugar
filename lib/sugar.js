(function(){
  ...
  var getRelativeDateString = function(val, unit, past){
    var suffix = past ? 'from now' : 'ago';
    return val + ' ' + (val > 1 ? unit + 's' : unit) + ' ' + suffix;
  }
  ...
  var buildDateMethods = function(){
    dateUnits.each(function(u, i){
      ...
      Date.prototype[unit+'sAgo'] = Date.prototype[unit+'sUntil'];
      Date.prototype[unit+'sFromNow'] = Date.prototype[unit+'sSince'];
  ...
  extend(Date, true, {
     ...
     * @extra Note here that the method %getTimezoneOffset% as well as %format% will still show an offset even after this method is called, as this method effectively just rewinds the date. You must yourself keep track of which dates have been converted to UTC.
     * @example
     *
     ...
     * @extra <format> will accept a number of tokens as well as pre-determined formats. Additionally can accept relative formats including the token %dynamic%, which will construct the relative date based on a best guess. See @date_format for more details.
     * @example
     *
     ...
     *   Date.create('beginning of this week').format('{dynamic}') -> ex. 3 days ago
     *
     ***/
    'format': function(str, utc){
      ...
      str = str.replace(new RegExp('\\{('+ dateUnits.map('unit').join('|') +')s? ago\\}', 'i'), function(m,unit){
        var value = d[unit + 'sAgo']();
        return getRelativeDateString(value, unit);
      });
      str = str.replace(/\{dynamic\}/, function(){
        var next, ms = d.millisecondsAgo(), ams = Math.abs(ms), value = ams, unit = 'millisecond';
        dateUnits.concat().reverse().slice(1).each(function(u){
          next = Math.floor(ams / u.multiplier(d));
          if(next >= 1){
            value = next;
            unit = u.unit;
          }
        });
        return getRelativeDateString(value, unit, ms < 0);
      });
      return str;
    },
  ...
  extend(Date, false, {
    ...
    'ISO8601': Date.ISO8601_DATETIME,
     ...
     /***
     * @method Date.create(<d>, [variant] = false)
     * @returns Date
     * @short Alternate Date constructor which understands various formats.
     * @extra Accepts a multitude of text formats, a timestamp, or another date. If no argument is given, date is assumed to be now. %Date.create% additionally can accept enumerated parameters as with the standard date constructor. [variant] indicates if the month and day are reversed when there is ambiguity. For more information, see @date_format.
     * @example
     *
