(function(){
  ...
  var getRelativeDateString = function(val, unit){
    var suffix = val < 0 ? 'from now' : 'ago';
    val = Math.abs(val);
  ...
  extend(Date, true, {
    ...
    'format': function(str, utc){
      ...
      str = str.replace(/\{dynamic\}/, function(){
        var next, ms = d.millisecondsAgo(), value = ms, unit = 'millisecond';
        var dir = value < 0 ? 'ceil' : 'floor';
        dateUnits.concat().reverse().slice(1).each(function(u){
          next = Math[dir](ms / u.multiplier(d));
          if(Math.abs(next) >= 1){
            value = next;
            unit = u.unit;
          }
