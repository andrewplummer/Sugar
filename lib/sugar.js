(function() {
  ...
  extend(array, false, false, {
    ...
    'zip' : function() {
      ...
      var model = args[0];
      if (args.length === 1 && !Array.isArray(model)) {
        var length = -1;
        var keys = [];
        for (var key in model) {
          ...
          if (length === -1) {
            ...
            throw new Error(key + " array has not the length " + length);
          }
          keys.add(key);
        }
        var result = [];
        ...
        args.each(function(array) {
          ...
          if (length === -1) {
            ...
            throw new Error(array + " length is not equals " + length);
          }
        });
       	for (var i = 0; i < length; ++i) {
       	  var entry = [];
