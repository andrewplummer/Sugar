...
(function(){
  ...
  var textNumberReg = textNumbers.join('|');
  ...
  var dateFormats = [
    ...
    { reg: new RegExp('^('+monthReg+')[\\s\\-.](\\d{4})'), to: ['month','year'] },
    // @date_format Tuesday May 25th, 2010
    { reg: new RegExp('^(?:'+weekdayReg+')?\\s*('+monthReg+')[\\s\\-.](\\d{1,2})(?:st|nd|rd|th)?,?[\\s\\-.](\\d{2,4})'), to: ['month','date','year'] },
    // @date_format 25 May 2010
    { reg: new RegExp('^(\\d{1,2}) ('+monthReg+'),? (\\d{4})'), to: ['date','month','year'] },
    ...
    { reg: new RegExp('^(?:(the|a|'+textNumberReg+'|\\d+) (day|week|month|year)s? (before|after|from)\\s+)?(today|tomorrow|yesterday|'+weekdayReg+')'), to: ['modifier_amount','modifier_unit','modifier_sign','fuzzy_day'] },
    // @date_format a second ago
    // @date_format two days from now
    // @date_format 25 minutes/hours/days/weeks/months/years from now
    { reg: new RegExp('^(a|'+textNumberReg+'|\\d+) (milliseconds?|seconds?|minutes?|hours?|day|week|month|year)s? (from now|ago)'), to: ['modifier_amount','modifier_unit','modifier_sign'], relative: true },
    // @date_format last wednesday
    // @date_format next friday
    // @date_format this week tuesday
    { reg: new RegExp('^(this|next|last)?\\s*(?:week\\s*)?('+weekdayReg+')'), to: ['modifier_sign','fuzzy_day'] },
    // @date_format monday of this/next/last week
    { reg: new RegExp('^('+weekdayReg+') (?:of\\s*)?(this|next|last) week'), to: ['fuzzy_day','modifier_sign'] },
    // @date_format May 25th of this/next/last year
    { reg: new RegExp('^('+monthReg+')(?: (\\d{1,2})(?:st|nd|rd|th)?)? of (this|next|last) (year)'), to: ['month','date','modifier_sign','modifier_unit'] },
    // @date_format the first day of the month
    // @date_format the last day of March
    // @date_format the 23rd of last month
    { reg: new RegExp('^(?:the\\s)?(?:(first|last)(?: day)?)?(\\d{1,2}(?:st|nd|rd|th))? of (?:(the|this|next|last) (month)|('+monthReg+'))'), to: ['modifier_edge_day', 'date','modifier_sign','modifier_unit','month'] },
    // @date_format the beginning of this week/month/year
    // @date_format the end of next month
    { reg: /^(?:the\s)?(beginning|end) of (the|this|next|last) (week|month|year)/, to: ['modifier_edge','modifier_sign','modifier_unit'] },
    ...
    { reg: /(midnight|noon)(?: (tonight|today|tomorrow|yesterday))?/, to: ['hours','fuzzy_day'], time: true },
    // @date_format 12pm
    // @date_format 12:30pm
  ...
  var timeReg = /(?:[\st](\d{1,2}):?(\d{2})?:?(\d{2})?(\.\d{1,6})?(am|pm)?(?:(Z)|(?:([+-])(\d{2})(?::?(\d{2}))?)?)?)?$/;
  ...
  var getDateFromString = function(str, variant){
    ...
    dateFormats.each(function(format){
      ...
      if(match){
        var f = getFormatMatch(match, format.to);
        ...
        if(f.year && f.year.length === 2){
          // If the year is two digits, add the most appropriate century prefix.
          var num = f.year.toNumber();
          f.year = (new Date).getFullYear().round(-2) - num.round(-2) + num;
        }
        if(f.month){
          var num = f.month.toNumber();
          if(isNaN(num)){
            // If the month is not a number, find it in the array of text months.
            f.month = abbreviatedMonths.indexOf(f.month.to(2));
          } else {
            // Otherwise decrement by 1.
            f.month = num - 1;
          }
        }
        if(f.hours === 'noon' || f.hours === 'midnight'){
          f.hours = f.hours === 'noon' ? 12 : 24;
          if(!set.date && !f.fuzzy_day) f.fuzzy_day = 'today';
        }
        if(f.fuzzy_day){
          // Fuzzy day can be today, tomorrow, yesterday, or a day of the week.
          // This resolves to an offset of the current date.
          var dayOffset = 0;
          var fuzzy = f.fuzzy_day;
          var weekday;
          if(fuzzy === 'yesterday'){
            dayOffset = -1;
            ...
            d.setDay(weekday);
            dayOffset = 0;
            if(f.modifier_sign && !f.modifier_unit && !f.modifier_amount){
              f.modifier_unit = 'week';
            }
          }
        ...
        if(f.milliseconds){
          ...
          f.milliseconds = (parseFloat(f.milliseconds, 10) * 1000).round();
        }

        // Now turn this into actual numbers
        ['year','month','date','day','hours','minutes','seconds','milliseconds'].each(function(unit){
          if(f[unit] !== undefined){
            set[unit] = f[unit].toNumber();
          }
        });

        if(f.meridian){
          // If the time is after 1pm-11pm advance the time by 12 hours.
          if(f.meridian === 'pm' && f.hours < 12) set.hours  += 12;
        }
        if(f.utc || f.offset_hours || f.offset_minutes){
          // Adjust for timezone offset
          utc = true;
          var offset = 0;
          if(f.offset_hours){
            offset += f.offset_hours.toNumber() * 60;
          }
          if(f.offset_minutes){
            offset += f.offset_minutes.toNumber();
          }
          if(f.offset_sign && f.offset_sign === '-'){
            offset *= -1;
          }
          set.minutes -= offset;
        }
        if(f.modifier_unit && f.modifier_sign){
          var amt = f.modifier_amount || 1;
          var textNumIndex;
          if(amt === 'the' || amt === 'a'){
            amt = 1;
          ...
          if(f.modifier_sign === 'before' || f.modifier_sign === 'ago' || f.modifier_sign === 'last'){
            amt *= -1;
          } else if(f.modifier_sign === 'this' || f.modifier_sign === 'the'){
            amt = 0;
          }
          if(f.modifier_unit === 'day'){
            f.modifier_unit = 'date';
          } else if(f.modifier_unit === 'week'){
            amt *= 7;
            set.date = relative ? amt : d.getDate() + amt;
          } else if(f.modifier_unit === 'month' && !relative){
            set.month = d.getMonth();
          } else if(f.modifier_unit === 'year' && !relative){
            set.year = d.getFullYear();
          }
          if(set[f.modifier_unit] === undefined){
            set[f.modifier_unit] = 0;
          }
          set[f.modifier_unit] += amt;
        }
        if(f.modifier_edge || f.modifier_edge_day){
          var edge = f.modifier_edge || f.modifier_edge_day;
          if(edge === 'beginning' || edge === 'first'){
            if(f.modifier_unit === 'week'){
              set.month = d.getMonth();
              set.day = 0;
            } else if(f.modifier_unit === 'month' || f.month){
              set.date = 1;
            }
          } else if(edge === 'end' || edge === 'last'){
            if(f.modifier_unit === 'week'){
              set.month = d.getMonth();
              set.day = 6;
            } else if(f.modifier_unit === 'month' || f.month){
              set.date = 32 - new Date(d.getFullYear(), set.month, 32).getDate();
            } else if(f.modifier_unit === 'year'){
              set.month = 11;
              set.date  = 31;
            }
            if(f.modifier_edge){
  ...
  Date.create = function(str, variant){
    ...
    return getDateFromString(str, variant);
  }
  ...
  var updateDate = function(date, params, reset, utc, advance){
    ...
    dateMethods.each(function(m, i){
      ...
      if(value !== undefined){
        if(advance){
          value = (value * advance) + date['get'+m.method].call(date);
        }
        date['set'+utc+m.method].call(date, value);
      }
    });
    return date;
  }
