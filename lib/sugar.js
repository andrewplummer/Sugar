(function() {
  ...
  var storeMethods = function(klass, instance, methods) {
    if(!klass.SugarMethods) {
      klass.SugarMethods = { 'true': {}, 'false': {} };
    }
    objectMerge(klass.SugarMethods[instance], methods);
    if(!klass.sugar) {
      defineProperty(klass, 'sugar', function(type) {
        var a = arguments,
            b = typeof a[0] === 'boolean',
            all = a.length === 0,
            allInstance = a.length === 1 && a[0] === true,
            allClass = a.length === 1 && a[0] === false,
            method;
        if(!all && !allInstance && !allClass) {
          arrayEach(arguments, function(name){
            if(!b || type === false) {
              defineProperty(klass, name, klass.SugarMethods[false][name]);
            }
            if(!b || type === true) {
              defineProperty(klass.prototype, name, klass.SugarMethods[true][name]);
            }
          }, b ? 1 : 0);
          return;
        }
        if(all || allInstance) {
          extend(klass, true, true, klass.SugarMethods[true]);
        }
        if(all || allClass) {
          extend(klass, false, true, klass.SugarMethods[false]);
        }
      });
    }
  }
  ...
  extend(Object, false, false, {
     ...
     ***/
    'merge': function() {
      console.info('HMMMMMMMMMM', this, arguments);
      return objectMerge.apply(this, arguments);
    },
