...
(function(){
  ...
  /*
   * Much thanks to "kangax" for his informative aricle about how problems with instance of and constructor
  ...
  var dateFormats = [
    ...
    { reg: new RegExp('^(?:'+weekdayReg.source+')?\\s*('+monthReg.source+')[\\s\\-.]?(?:(\\d{1,2})(?:st|nd|rd|th)?)?,?[\\s\\-.]?(\\d{2,4})?'), to: ['month','day','year'] },
    // @date_format 25 May 2010
    { reg: new RegExp('^(\\d{1,2}) ('+monthReg.source+'),? (\\d{4})'), to: ['day','month','year'] },
    ...
    { reg: new RegExp('^(a|'+textNumberReg.source+'|\\d+) (millisecond|second|minute|hour|day|week|month|year)s? (from now|ago)'), to: ['modifier_amount','modifier_unit','modifier_sign'], relative: true },
    ...
    { reg: new RegExp('^('+monthReg.source+')(?: (\\d{1,2})(?:st|nd|rd|th)?)? of (this|next|last) (year)'), to: ['month','day','modifier_sign','modifier_unit'] },
    // @date_format the first day of the month
    // @date_format the last day of March
    // @date_format the 23rd of last month
    { reg: new RegExp('^(?:the\\s)?(first day|last day)?(\\d{1,2}(?:st|nd|rd|th))? of (?:(the|this|next|last) (month)|('+monthReg.source+'))'), to: ['modifier_edge', 'day','modifier_sign','modifier_unit','month'] },
    ...
    { reg: new RegExp('^(midnight|noon)(?: (tonight|today|tomorrow|yesterday'+weekdayReg.source+'))?'), to: ['hour','fuzzy_day'], timeIncluded: true },
    // @date_format 12pm
    // @date_format 12:30pm
  ...
  extend(Date, false, {
    'create': function(f, variant){
      if(f === undefined) return new Date();
      else if(Object.isNumber(f)) return new Date(f);
      else if(Object.isDate(f)) return f;
      else return getDateFormat(f, variant).date;
    },
    ...
    'ISO8601_DATETIME': '{yyyy}-{MM}-{dd}T{hh}:{mm}:{ss}{isotz}'
  });
    ...
    'is': function(find, buffer){
      ...
      if(!Object.isString(find)){
        ...
        if(find === 'weekday'){
          ...
          var p = getDateFormat(find);
          var unit;
          if(!p.date.isValid()) return false;
          dateUnits.each(function(u){
            if(p.set[u.unit] !== undefined){
              unit = u.unit;
              accuracy = u.multiplier(p.date);
            }
          });
          ...
          // Relative dates have a 5ms min buffer to compensate for complications that arise
          // from the fact that they are instanciated slightly after this method is called.
          if(p.format.relative) buffer = 5;
       //   console.info(p.date.getTime(), this.getTime(), buffer, max + buffer, accuracy);
        //  console.info(min, t, max, (t >= (min - buffer) && t < (max + buffer)));
  ...
  var timeAlias = function(unit, multiplier){
    var baseFunc   = function(){  return this * multiplier; }
    var beforeFunc = function(d){ return getOffsetDate(this, d, unit, 'rewind'); }
    var afterFunc  = function(d){ return getOffsetDate(this, d, unit, 'advance'); }
    Number.prototype[unit] = baseFunc;
    Number.prototype[unit + 's'] = baseFunc;
    Number.prototype[unit + 'Ago'] = beforeFunc;
    Number.prototype[unit + 'sAgo'] = beforeFunc;
    Number.prototype[unit + 'Before'] = beforeFunc;
    Number.prototype[unit + 'sBefore'] = beforeFunc;
    Number.prototype[unit + 'After'] = afterFunc;
    Number.prototype[unit + 'sAfter'] = afterFunc;
    Number.prototype[unit + 'FromNow'] = afterFunc;
    Number.prototype[unit + 'sFromNow'] = afterFunc;
  }
  ...
  dateUnits.each(function(u, i){
    ...
    Date.prototype[unit+'sSince'] = function(f){
      return (this.getTime() - Date.create(f).getTime()) / multiplier;
    }
    Date.prototype[unit+'sUntil'] = function(f){
      return (Date.create(f).getTime() - this.getTime()) / multiplier;
    }
  ...
  ['Array','Boolean','Date','Function','Number','String','RegExp'].each(function(c){
