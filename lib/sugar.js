  ...
  */
  ...
  var CommonLanguages = {
    ...
    'ru': {
      ...
      units: ['миллисекунд:а|у|ы|','секунд:а|у|ы|','минут:а|у|ы|','час:||а|ов','день:||я|ей','недел:я|ю|и|ь','месяц:||а|ов','год|год|года|лет'],
      numbers: ['один','два','три','четыре','пять','шесть','семь','восемь','девять','десять','одну','две'],
      optionals: ['в|на'],
      pastRelativeFormat: 0,
      futureRelativeFormat: 1,
      modifiers: [
...
//'({MONTHS})[\\s\\-.](\\d{4})'
//
      var setArray = function(name, def, length, optionals) {
      ...
      var getDigit = function(start, stop) {
        var suffix = '{' + start + (stop ? ',' + stop : '') + '}', num = '';
        if(set.numbersAreOrdinal) {
          num = set.numbers.join('');
          start = null;
        }
        return '[0-9０-９' + num + ']' + (start ? suffix : '+');
      }
     ...
     //console.info('hahahaha', getDigit(4), getDigit(1,2), getDigit());
      ...
      addFormat((set.weekdayPrefix || '') + '(' + set.weekdays.join('|') + ')', { to: ['weekday'] });
      addFormat('(' + set.modifiers.filter(function(m){ return m.name === 'day'; }).map('text').join('|') + ')', { to: ['day'] });
