(function() {
  ...
  */
  ...
  var checkLocalizationInitialized = function(lang) {
    if(lang && !DateLanguages[lang] && CommonLanguages[lang]) {
  ...
  var getFormatMatch = function(match, arr) {
    ...
    arrayEach(arr, function(key, i) {
      ...
      else if(key === 'year') obj.yearAsString = value;
      else if(key === 'millisecond') value = value * Math.pow(10, 3 - value.length);
    ...
    if(Object.isDate(f)) {
      ...
      arrayEach(dateInputFormats, function(dif) {
        ...
        if(match) {
          ...
          } else if(set.day && (tmp = loc.getWeekday(set.day, true))) {
            delete set.day;
            set.weekday = tmp;
          }
  ...
  var buildDateInputFormats = function() {
    // TODO UPDATE?
    Date.registerLanguage('en', CommonLanguages['en']);
...
//
    ...
    //{ reg: '(a|{NUMBER}|\\d+) (millisecond|second|minute|hour|day|week|month|year)s? (from now|ago)', to: ['modifier_amount','modifier_unit','modifier_sign'], relative: true },
     ...
     console.info('hoofa', code);
     DateLanguages[code] = currentLocalization = new Localization(set);
    ...
    'is': function(d, margin) {
      ...
      if(!Object.isString(d)) {
        ...
        if(d === 'future') {
          ...
          return this.getDay() === 0 || this.getDay() === 6;
        } else if((tmp = currentLocalization.getWeekday(d)) > 0) {
          return this.getDay() === tmp;
        } else if((tmp = currentLocalization.getMonth(d)) > 0) {
