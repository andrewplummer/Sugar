...
(function(){
  ...
  var foof = '(?:(?:\\s+|t)(\\d{1,2}):?(\\d{2})?:?(\\d{2})?(\\.\\d{1,6})?(am|pm)?(?:(Z)|(?:([+-])(\\d{2})(?::?(\\d{2}))?)?)?)?$';
  var nofoof = '(?:(\\d{1,2}):?(\\d{2})?:?(\\d{2})?(\\.\\d{1,6})?(am|pm)?(?:(Z)|(?:([+-])(\\d{2})(?::?(\\d{2}))?)?)?)?$';
//(?:(?:\\s+|t)

  var buildDate = function(){
    dateFormats.each(function(m){
      var src = '^' + m.reg;
      src = src.replace(/{WEEKDAYS}/, weekdayReg.source);
      src = src.replace(/{MONTHS}/, monthReg.source);
      src = src.replace(/{NUMBER}/, textNumberReg.source);
      src = m.timeIncluded ? src : src + timeReg.source;
      if(!m.timeIncluded){
        var time = m.timeOnly ? nofoof : foof;
        src = src + time;
        m.to = m.to.concat(['hour','minute','second','millisecond','meridian','utc','offset_sign','offset_hours','offset_minutes']);
      }
        console.info(/^(?=\s*.+)(?:(?:\s+|t)(\d{1,2}):?(\d{2})?:?(\d{2})?(\.\d{1,6})?(am|pm)?(?:(Z)|(?:([+-])(\d{2})(?::?(\d{2}))?)?)?)?$(?:(\d{1,2}):?(\d{2})?:?(\d{2})?(\.\d{1,6})?(am|pm)?(?:(Z)|(?:([+-])(\d{2})(?::?(\d{2}))?)?)?)?$/i);
