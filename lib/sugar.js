...
(function(){
  ...
  extend(Number, true, {
     ...
     * @parameters power
     * @returns Number
     * @description Returns the number to the power of <power>.
     *
     */
    'pow': function(power){
  ...
  extend(String, true, {
    ...
    /**
     * @method pad
     * @parameters num, padding, direction
     * @returns String
    ...
    'camelize': function(first){
      ...
      var text = '';
      for(var i=0; i<split.length; i++){
        if(!first && i === 0) text += split[i].toLowerCase();
  ...
  extend(Array, true, {
    ...
    'intersect': function(arr){
      ...
      this.each(function(el){
        var match = el === null ? arr.indexOf(el) > -1 : arr.find(el) !== undefined;
    ...
    'min': function(){
      ...
      var args = getArgumentsWithOptionalFunction(arguments);
      var result = getMinOrMax(arr, 'min', args.fn);
      return args.first === true ? result.unique() : result.first() || undefined;
    },

    'max': function(){
      var arr = this.compact();
      var args = getArgumentsWithOptionalFunction(arguments);
      var result = getMinOrMax(arr, 'max', args.fn);
      return args.first === true ? result.unique() : result.first() || undefined;
    },

    'least': function(){
      var args = getArgumentsWithOptionalFunction(arguments);
      var grouped = this.groupBy(args.fn);
