(function() {
  ...
  var EnglishUnits   = ['millisecond','second','minute','hour','day','week','month','year'];
  var timeReg  = '(\\d{1,2}):?(\\d{2})?:?(\\d{2})?(\\.\\d{1,6})?(am|pm)?(?:(Z)|(?:([+-])(\\d{2})(?::?(\\d{2}))?)?)?';
  var timeSuffix = '(?:(?:\\s+|t)' + timeReg + ')?';
  ...
  var checkLanguageInitialized = function(lang) {
    return;
    if(DateLanguages[lang]) return;
    if(CommonLanguages[lang]) {
      initializeLanguage(lang);
    } else {
      throw new TypeError('Specified language does not exist');
    }
  };

  var initializeLanguage = function(lang) {
  };
  ...
  var getExtendedDate = function(f, lang) {
    var d = new Date(), utc = false, relative = false, match, format, set, tmp;
    ...
    if(Object.isDate(f)) {
      ...
      arrayEach(dateInputFormats, function(dif) {
        ...
        if(match) {
          ...
          console.info('ofuck', set, match, set.week);
          ...
          */
          ...
          if(set.unit) {
            ...
            // SHORTEN ME
            var engUnit = EnglishUnits[DateLanguages[lang].units.indexOf(set.unit) % 8];
            /* DEPRECATE ME!!
            var textNumIndex;
            if(amt === 'the' || amt === 'a') {
              amt = 1;
           ...
           if(set.shift) {
             ...
             console.info('shift is ', shift, amt);
             amt *= shift;
           }
           if(set.sign) {
           ...
           */
          console.info("UMMM", amt, set);
           if(set[engUnit] === undefined) {
             set[engUnit] = 0;
           }
           set[engUnit] += amt;
         }
          if(set.edge) {
            //console.info('GETTING IT', set.edge, DateLanguages[lang].modifiersByName[set.edge]);
            //var edge = set.edge;
            ...
            var method = (mod.value === -1 ? 'endOf' : 'beginningOf') + engUnit.capitalize();
            if(d[method]) {
              d[method].call(d);
            }
            /*
  ...
  var createDate = function(args) {
    ...
    if(Object.isString(args[0])) {
      // If a string was passed, then make sure the language is set
      checkLanguageInitialized(lang);
    }
    if(Object.isNumber(args[1])) {
      // If the second argument is a nuber, then we have an enumerated constructor type as in "new Date(2003, 2, 12);"
      f = collectDateArguments(args)[0];
    } else {
      f = args[0];
    }
    return getExtendedDate(f, lang).date;
  }
  ...
  var buildDateInputFormats = function() {
    var monthReg = '\\d{1,2}|' + months.map(function(m) { return m.to(3)+'(?:'+m.from(3)+')?'; }).join('|');
    arrayEach(numeralInputFormats, function(f) {
