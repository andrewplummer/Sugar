  ...
  extend(Array, {
    ...
    'every': function(block, scope){
      for(var i=0,len=this.length; i < len; i++){
        if(!block.call(scope, this[i], i, this)){
          return false;
        }
      }
      return true;
    ...
    'findAll': function(f){
      ...
      for(var i=0,len=this.length; i<len; i++){
        var match = false;
        if(f && f.test !== undefined){
          match = f.test(this[i]);
        } else if(typeof f == 'function'){
          match = f.call(this, this[i]);
        } else if(typeof f == 'object'){
          match = deepEquals(f, this[i]);
        } else {
          match = f === this[i];
        }
        if(match) result.push(this[i]);
      }
      return result;
    },
    ...
    'median': function(block){
      ...
      return arr.length.even() ? ((arr[mid] + arr[mid - 1]) / 2) : arr[mid];
    },
    ...
    'mode2': function(){
      ...
      var occurrences = {};
      this.each(function(el,i,a){
        occurrences[el] = occurrences[el] ? occurrences[el] + 1 : 1;
      });
      var most;
      var count = 0;
      for(var x in occurrences){
        if(!occurrences.hasOwnProperty(x)) continue;
        if(occurrences[x] > count){
          count = occurrences[x];
          most = x;
        }
      }
