...
(function(){
  ...
  var dateFormats = [
    ...
    { reg: new RegExp('^(?:(the|a|'+textNumberReg+'|\\d+) (day|week|month|year)s? (before|after)\\s+)?(today|tomorrow|yesterday)'), to: ['modifier_amount','modifier_unit','modifier_sign','fuzzy_day'] } // dd month yyyy
  ];
  ...
  var getDateFromString = function(str, variant){
    ...
    dateFormats.each(function(format){
      ...
      if(match){
        ...
        if(f.fuzzy_day){
          // Today, yesterday, and tomorrow also implicitly set the month
          // (year is handled automatically in the set method)
          f.month = d.getMonth();
          var day_offset;
          switch(f.fuzzy_day){
            case 'today':     day_offset =  0; break;
            case 'yesterday': day_offset = -1; break;
            case 'tomorrow':  day_offset =  1; break;
          }
          f.day = d.getDate() + day_offset;
        }
        if(f.milliseconds){
        ...
        if(f.offset_hours || f.offset_minutes){
        ...
        if(f.modifier_amount && f.modifier_unit && f.modifier_sign){
