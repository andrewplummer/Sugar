(function() {
  ...
  function arrayUnique(arr, args) {
    return arr.reduce(function(a,b) {
      var compare = args ? a.map(args[0]) : a;
      console.info('umm', args, args.length);
      if(compare.none(transformArgument(args, b))) {
        a.push(b);
      }
      return a;
    }, []);
    ...
    arr.each(function(el) {
      if(arrayFind(map ? result.map(map) : result, el) === undefined) result.push(el);
    });
    return result;
    */
  }
  ...
  extend(Array, true, false, {
     ...
     ***/
    'unique': function() {
      return arrayUnique(this, arguments);
    },
