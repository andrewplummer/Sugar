(function() {
  ...
  // Note also here that we are wrapping all regexp shorthands through the RegExp object, due to failures in
  // The SpiderMonkey version run for CouchDB, in which regexp shorthands do not have any methods. See:
  // https://github.com/andrewplummer/Sugar/issues/27
  ...
  extend(number, true, false, {
    ...
    'format': function(place, thousands, decimal) {
      var str, split, method, after, r = regexp(/(\d+)(\d{3})/);
  ...
  var ZenkakuTable = {};
  var HankakuTable = {};
  var allHankaku   = regexp(/[\u0020-\u00A5]|[\uFF61-\uFF9F][ﾞﾟ]?/g);
  var allZenkaku   = regexp(/[\u3000-\u301C]|[\u301A-\u30FC]|[\uFF01-\uFF60]|[\uFFE0-\uFFE6]/g);
  var hankakuPunctuation  = '｡､｢｣¥¢£';
  var zenkakuPunctuation  = '。、「」￥￠￡';
  var voicedKatakana      = regexp(/[カキクケコサシスセソタチツテトハヒフヘホ]/);
  var semiVoicedKatakana  = regexp(/[ハヒフヘホヲ]/);
  ...
  function buildBase64(key) {
    ...
    var base64reg = regexp(/[^A-Za-z0-9\+\/\=]/g);
    btoa = function(str) {
      var output = '';
      var chr1, chr2, chr3;
  ...
  function buildTrim() {
    var support = regexp(/^\s+$/).test(getTrimmableCharacters());
  ...
  extend(string, true, false, {
    ...
    'truncate': function(length, append, split) {
     var reg, repeatedCharacter;
      ...
      repeatedCharacter = regexp(/^(.)\1+$/).test(append) ? append.slice(0,1) : '';
