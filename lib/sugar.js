(function() {
  ...
  var getExtendedDate = function(f, lang) {
    ...
    if(Object.isDate(f)) {
      ...
      console.info(dateInputFormats[0].reg.source);
...
//
      ...
      setArray('numbers', []);
      setArray('units', []);
      setArray('pluralizedUnits', set.units.concat().map(function(u) {
          return set.pluralizedSuffix ? u + set.pluralizedSuffix : u;
      }));
      setArray('allUnits', set.units.concat(set.pluralizedUnits));
      setArray('articles', []);
      ...
      arrayEach(set.formatOrders, function(fo) {
        ...
        dateInputFormats.unshift({ reg: new RegExp('^' + tokens.join('[\\s,]*') + '$', 'i'), to: to });
      });

      var moof = ['(' + set.articles.join('|') + this.numbers.join('|') + '\\d+)', '(' + set.allUnits.join('|') + ')', '(' + + ')'],
