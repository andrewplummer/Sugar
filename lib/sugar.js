(function() {
  ...
  function mergeObject(obj, target, deep) {
    if(!isObjectPrimitive(obj) || isNull(obj)) {
      return obj;
    } else if(!target) {
      target = Object.isArray(obj) ? [] : {};
    }
    console.info('hokay', target);
    iterateOverObject(obj, function(k, v) {
      console.info('INSIDE AND ', deep);
      if(deep === true && isObjectPrimitive(v)) {
        if(!target[k]) target[k] = Object.isArray(v) ? [] : {};
        mergeObject(v, target[k], deep);
      } else {
        target[k] = v;
      }
    });
  ...
  extend(object, false, false, {
     ...
     ***/
    'merge': function() {
      var args = Array.create(arguments), target = args.first();
      arrayEach(args, function(obj) {
        mergeObject(obj, target, true);
      }, 1);
      /*
      multiArgs(args.from(1), function(a) {
    ...
    'clone': function(obj, deep) {
      return mergeObject(obj, obj && obj['keys'] ? Object.extended() : {}, deep);
    },
