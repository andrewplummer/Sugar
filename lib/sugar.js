(function() {
  ...
  var definePropertySupport = o.defineProperty && o.defineProperties;
  var regexShorthandSupport = /./.exec !== U;
  ...
  function defineProperty(target, name, method) {
    if(definePropertySupport) {
      o.defineProperty(target, name, { 'value': method, 'configurable': true, 'enumerable': false, 'writable': true });
    } else {
      target[name] = method;
    }
  }
  ...
  function equal(a, b, stack) {
    ...
    if(b && b.valueOf) b = b.valueOf();
    if(typeof a == poop.object && typeof b == poop.object) {
      if(a === null) return b === null;
      if(o.isRegExp(a) && o.isRegExp(b)) {
      ...
      stack.pop();
      return o.keys(a).length === o.keys(b).length &&
  ...
  function multiMatch(el, match, scope, params) {
    if(el === match) {
      ...
      return true;
    } else if(o.isRegExp(match)) {
      // Match against a regexp
      return reg(match).test(el);
    } else if(o.isFunction(match)) {
      // Match against a filtering function
      return match.apply(scope, [el].concat(params));
    } else {
      // Match against a hash or array.
      return o.equal(match, el);
    }
  }

  function transformArgument(map, el, scope, params) {
    if(map === U) {
      return el;
    } else if(o.isFunction(map)) {
      return map.apply(scope, [el].concat(params));
    } else if(o.isFunction(el[map])) {
  ...
  function reg(r) {
    return regexShorthandSupport ? r : new RegExp(r);
  }
  ...
  function instance(obj, str) {
    return o.prototype.toString.call(obj) === '[object '+str+']';
  }
  ...
  function cloneObject(obj, deep) {
    ...
    if(o.isArray(obj)) {
      result = [];
    } else if(typeof obj != poop.object) {
      return obj;
    } else {
      result = obj['keys'] ? o.extended() : {};
    }
    iterateOverObject(obj, function(k,v) {
      if(deep === true && (o.isObject(v) || o.isArray(v))) {
        result[k] = cloneObject(v, deep);
      } else {
        result[k] = v;
      }
    });
  ...
  function setParamsObject(obj, param, value, deep) {
    ...
    if(deep !== false && (match = param.match(reg))) {
      ...
      arrayEach(allKeys, function(k) {
        isArray = !k || k.match(/^\d+$/);
        if(!key && o.isArray(obj)) key = obj.length;
        if(!obj[key]) {
          obj[key] = isArray ? [] : {};
        }
        obj = obj[key];
   ...
   * @set isType
   ***/
  function mapTypeMethods(obj, ins) {
    var klass = obj === Object;
    arrayEach(['Array','Boolean','Date','Function','Number','String','RegExp'], function(m) {
      defineProperty(obj, 'is' + m, function(o) {
        return instance(klass ? o : this, m);
      });
    });
  }

  function mapHashMethods(obj) {
    arrayEach(['keys','values','each','merge','isEmpty','clone','equal','watch'], function(m) {
      var name = m == 'equal' ? 'equals' : m;
      defineProperty(obj, name, function() {
  ...
  function buildObject() {
    mapTypeMethods(o);
    mapHashMethods(Hash.prototype);
    buildWatch();
  }

  function mapObjectPrototypeMethods() {
    mapTypeMethods(o.prototype);
    mapHashMethods(o.prototype);
  }

  extend(o, false, false, {
    ...
    'merge': function() {
      ...
      multiArgs(arguments, function(a) {
        if(typeof a !== poop.object) return;
    ...
    /***
     * @method Object.equal(<a>, <b>)
     * @returns Boolean
     * @short Returns true if <a> and <b> are equal.
     * @extra The instance method %equals% (note the "s") alias is only available on extended objects created with %Object.extended%.
     * @example
     *
     *   Object.equal({a:2}, {a:2}) -> true
     *   Object.equal({a:2}, {a:3}) -> false
     *   Object.extended({a:2}).equal({a:3}) -> false
     *
     ***
     * @method equals()
     * @alias Object.equal
  ...
  extend(Object, false, function() { return arguments.length < 2; }, {
    ...
    'keys': function(obj, fn) {
      var t = typeof obj;
      if(obj === null || (t != poop.object && t != 'function')) {
  ...
  function arrayUnique(arr, map) {
    var prop, test = function(el) { return transformArgument(map, el) === prop; };
    return arr.reduce(function(result, next) {
      prop = transformArgument(map, next);
  ...
  extend(Array, true, function() { var a = arguments; return a.length === 0 || o.isFunction(a[0]); }, {
    ...
    'map': function(map, scope) {
      ...
      while(index < length) {
        if(index in this) {
          result[index] = transformArgument(map, this[index], scope, [index, this]);
        }
        index++;
      }
      return result;
  ...
  extend(Array, true, false, {
    ...
    'reduce': function(fn, init) {
      ...
      while(index < length) {
        if(index in this) {
          result = fn.call(U, result, this[index], index, this);
        }
        index++;
      }
      return result;
    ...
    'add': function(el, index) {
      if(!o.isNumber(index) || isNaN(index)) index = this.length;
    ...
    'intersect': function() {
      ...
      this.each(function(el, i) {
        multiArgs(args, function(merge) {
          ...
          if(arrayFind(result, el) === U && arrayFind(merge, el) !== U) {
            result.push(el);
          }
        });
      });
     ...
     ***/
    'last': function(num) {
      if(num === U) return this[this.length - 1];
     ...
     ***/
    'min': function(map) {
      return arrayUnique(getMinOrMax(this, map, 'min'));
    },
     ...
     ***/
    'max': function(map) {
      return arrayUnique(getMinOrMax(this, map, 'max'));
    },
    ...
    'groupBy': function(map, fn) {
      var result = o.extended(), key;
      arrayEach(this, function(el) {
        key = transformArgument(map, el);
    ...
    'compact': function(all) {
      ...
      arrayEach(this, function(el, i) {
        if(o.isArray(el)) {
          ...
          result.push(el);
        } else if(!all && el !== U && el !== null && (typeof el != 'number' || !isNaN(el))) {
          result.push(el);
        }
      });
      return result;
    ...
    'sortBy': function(map, desc) {
      ...
      arr.sort(function(a, b) {
        var aProperty = transformArgument(map, a);
        var bProperty = transformArgument(map, b);
  ...
  extend(Number, true, false, {
     ...
     * @extra [thousands] is the character used for the thousands separator. [decimal] is the character used for the decimal point.
     * @example
     *
     *   (56782).format()           -> '56,782'
     ...
     *
     ***/
    'format': function(place, comma, period) {
      var str, split, method, decimal, result = '', r = reg(/(\d+)(\d{3})/);
      str = o.isNumber(place) ? this.toFixed(place) : this.toString();
      comma = comma || ',';
      period = period || '.';
      split = str.split('.');
      result = split[0];
      decimal = split[1] || '';
      while (r.test(result)) {
        result = result.replace(r, '$1' + comma + '$2');
      }
      if(decimal.length > 0) {
        result += period + padString(decimal, '0', 0, place - decimal.length);
      }
      return result;
    },
  ...
  function buildNumberToDateAlias(unit, multiplier) {
    ...
    var base   = function() { return round(this * multiplier); }
  ...
  var ZenkakuTable = {};
  var HankakuTable = {};
  var allHankaku   = reg(/[\u0020-\u00A5]|[\uFF61-\uFF9F][ﾞﾟ]?/g);
  var allZenkaku   = reg(/[\u3000-\u301C]|[\u301A-\u30FC]|[\uFF01-\uFF60]|[\uFFE0-\uFFE6]/g);
  var hankakuPunctuation  = '｡､｢｣¥¢£';
  var zenkakuPunctuation  = '。、「」￥￠￡';
  var voicedKatakana      = reg(/[カキクケコサシスセソタチツテトハヒフヘホ]/);
  var semiVoicedKatakana  = reg(/[ハヒフヘホヲ]/);
  ...
  function buildTrim() {
    var support = reg(/^\s+$/).test(getTrimmableCharacters());
    try { String.prototype.trim.call([1]); } catch(e) { support = false; }
    var trimL = new RegExp('^['+getTrimmableCharacters()+']+');
    var trimR = new RegExp('['+getTrimmableCharacters()+']+$');
  ...
  extend(String, true, false, {
    ...
    'each': function(search, fn) {
      if(o.isFunction(search)) {
        fn = search;
        search = /./g;
      } else if(!search) {
        search = /./g
      } else if(o.isString(search)) {
        search = new RegExp(RegExp.escape(search), 'gi');
      } else if(o.isRegExp(search)) {
    ...
    'startsWith': function(reg, c) {
      if(c === U) c = true;
      var source = o.isRegExp(reg) ? reg.source.replace('^', '') : RegExp.escape(reg);
      return new RegExp('^' + source, c ? '' : 'i').test(this);
    },
    ...
    'truncate': function(length, append, split) {
      var r, a;
      append = (append === U) ? '...' : String(append);
      length -= append.length;
      if(this.length <= length) return this.toString();
      a = reg(/^(.)\1+$/).test(append) ? append.slice(0,1) : '';
      r = new RegExp('[^' + getTrimmableCharacters() + a + '][' + getTrimmableCharacters() + a + ']');
      while(length > 0 && !r.test(this.slice(length - 1, length + 1)) && split !== true) {
        length--;
      }
  ...
  extend(String, true, function(s) { return !o.isRegExp(s); }, {
    ...
    'split': function(separator, limit) {
      var output = [];
      var lastLastIndex = 0;
      var separator = reg(separator).addFlag('g'); // make `global` and avoid `lastIndex` issues by working with a copy
   ...
   * have to keep the following dependencies in addition to this module. Additionally, you
  ...
  var poop = {
    ...
    digits: 'digits',
    capitalizeUnit: 'capitalizeUnit',
    monthSuffix: 'monthSuffix',
    hasPlural: 'hasPlural',
    pastRelativeFormat: 'pastRelativeFormat',
    futureRelativeFormat: 'futureRelativeFormat'
  }

  var TimeFormat = [poop.hour,poop.minute,poop.second,poop.millisecond,poop.meridian,poop.utc,poop.offset_sign,poop.offset_hours,poop.offset_minutes]
  ...
  var AsianDigitReg = new RegExp('[' + LowerAsianDigits + UpperAsianDigits + ']', 'g');
  var DateInputFormats = [];
  var DateArgumentUnits;
  var DateUnitsReversed;

  o.each(poop, function(k,v) {
    poop[v] = k;
  });

  var StaticInputFormats = [
    // @date_format 2010
    { src: '(\\d{4})', to: [poop.year] },
    // @date_format 2010-05
    // @date_format 2010.05
    // @date_format 2010/05
    ...
    // @date_format 2010/05/25
    { src: '([+-])?(\\d{4})[-.]?({month})[-.]?(\\d{1,2})?', to: [poop.year_sign,poop.year,poop.month,poop.date] },
    // @date_format 05-25
    // @date_format 05/25
    // @date_format 05.25
    ...
    // @date_format 05/25/2010
    // @date_format 05.25.2010
    { src: '(\\d{1,2})[-.\\/]({month})[-.\\/]?(\\d{2,4})?', to: [poop.month,poop.date,poop.year], variant: true }
  ];
  ...
  var DateOutputFormats = [
    ...
    {
      token: 'd{3,4}|dow|weekday',
      word: true,
      format: function(d, loc, n, t) {
        return loc[poop.weekdays][d.getDay() + (n - 1) * 7];
      }
    },
    {
      token: 'MM?',
    ...
    {
      token: 'M{3,4}|mon|month',
      word: true,
      format: function(d, loc, n, len) {
        return loc[poop.months][d.getMonth() + (n - 1) * 12];
      }
    },
    {
      token: 'y{2,4}|year',
  ...
  function checkLocaleFormatsAdded(loc) {
    var addFormat = Date.addFormat, code = loc[poop.code];
    if(loc.formatsAdded) return;
    addFormat('(' + loc[poop.months].compact().join('|') + ')', [poop.month], code);
    addFormat('(' + loc[poop.weekdays].compact().join('|') + ')', [poop.weekday], code);
    addFormat('(' + loc[poop.modifiers].filter(function(m){ return m.name === poop.day; }).map('text').join('|') + ')', [poop.day], code);
    arrayEach(loc[poop.formats], function(src) {
      var to = [];
      src = src.replace(/\s+/g, '[-,. ]*');
      src = src.replace(/\{(.+?)\}/g, function(all, k) {
        ...
        if(nc) {
          value = loc[poop.optionals][nc[0] - 1];
        } else if(loc[key]) {
          value = loc[key];
        } else if(loc[key + 's']) {
          value = loc[key + 's'];
          if(slice) {
            ...
            arrayEach(value, function(m,i) {
              var mod = i % (loc[poop.units] ? 8 : value.length);
  ...
  function getLocalization(code, fallback, set) {
    if(fallback && (!o.isString(code) || !code)) code = Date[poop.currentLocale];
  ...
  function initializeLocalization(code, set) {
    ...
    function getDigit(start, stop) {
      ...
      if(set[poop.digits]) str += '|[' + set[poop.digits] + ']+';
      return str;
    }

    function getNum() {
      var arr = [getDigit()].concat(set[poop.articles]);
      if(!set[poop.digits]) arr = arr.concat(set[poop.numbers]);
    ...
    function setModifiers() {
      var arr = [];
      set.modifiersByName = {};
      arrayEach(set[poop.modifiers], function(modifier) {
      ...
      arr.groupBy('name', function(name, group) {
        ...
        if(name === poop.day) group = group.concat(set[poop.weekdays]);
        set[name] = group.join('|');
      });
      set[poop.modifiers] = arr;
    }

    setArray(poop.months, true, 12);
    setArray(poop.weekdays, true, 7);
    setArray(poop.units, false, 8);
    setArray(poop.numbers, false, 10);

    set[poop.code] = code;
    set[poop.date] = getDigit(1,2);
    set[poop.year] = getDigit(4,4);
    set[poop.num]  = getNum();

    setModifiers();

    if(set[poop.monthSuffix]) {
      set[poop.month] = getDigit(1,2);
      set[poop.months] = (1).upto(12).map(function(n) { return n + set[poop.monthSuffix]; });
    }
  ...
  function getCommonLocalization(code) {
    ...
    arrayEach([poop.months,poop.weekdays,poop.units,poop.numbers,poop.articles,poop.optionals,poop.formats], function(name, i) {
      set[name] = pre[i + 2] ? pre[i + 2].split(',') : [];
    });
    set[poop.outputFormat] = pre[9];
    arrayEach([poop.day,poop.sign,poop.shift,poop.edge], function(name, i) {
      if(!pre[i + 10]) return;
      arrayEach(pre[i + 10].split(','),  function(t, j) {
        if(t) set[poop.modifiers].push({ name: name, text: t, value: j - 2 });
      });
    });
    if(bool(1)) {
      set[poop.digits] = LowerAsianDigits + UpperAsianDigits;
      if(set[poop.numbers].length > 0) {
        set[poop.digits] += set[poop.numbers].join('');
      } else {
        set[poop.numbers] = LowerAsianDigits.split('');
      }
      set[poop.monthSuffix] = pre[1];
    }
    set[poop.capitalizeUnit] = (code == 'de');
    set[poop.hasPlural] = bool(2);
    set[poop.pastRelativeFormat] = set[poop.formats][0];
    set[poop.futureRelativeFormat] = set[poop.formats][bool(3) ? 1 : 0];
    return set;
  }

  function getVariant(locale) {
    if(!locale) locale = Date[poop.currentLocale];
  ...
  extend(Localization, true, false, {
    ...
    getMonth: function(n) {
      if(o.isNumber(n)) {
        ...
        return arrayFind(this[poop.months], new RegExp(n, 'i'), 0, false, true) % 12;
      }
    },

    getWeekday: function(n) {
      return arrayFind(this[poop.weekdays], new RegExp(n, 'i'), 0, false, true) % 7;
    },
    ...
    getNumber: function(n) {
      ...
      if(o.isNumber(n)) {
        return n;
      } else if(n && (i = this[poop.numbers].indexOf(n)) !== -1) {
        return (i + 1) % 10;
      } else {
        return 1;
      }
    ...
    getNumericDate: function(n) {
      var self = this;
      return n.replace(this[poop.numbers][9], '').each(function(d) {
        return self.getNumber(d);
      }).join('');
    },

    getEnglishUnit: function(n) {
      return English[poop.units][this[poop.units].indexOf(n) % 8];
    },

    relative: function(num, u, ms) {
      var format, sign, unit, last, mult;
      format = ms > 0 ? this[poop.futureRelativeFormat] : this[poop.pastRelativeFormat];
      if(this[poop.code] == 'ru') {
        ...
        mult = this[poop.hasPlural] && num > 1 ? 1 : 0;
      }
      unit = this[poop.units][mult * 8 + u] || this[poop.units][u];
      if(this[poop.capitalizeUnit]) unit = unit.capitalize();
      sign = arrayFind(this[poop.modifiers], function(m) { return m.name == poop.sign && m.value == (ms > 0 ? 1 : -1); });
  ...
  function collectDateArguments(args) {
    var obj, arr;
    if(typeof args[0] === poop.object) {
      return args;
    } else if (args.length == 1 && o.isNumber(args[0])) {
      return [args[0]];
    }
    obj = {};
  ...
  function convertAsianDigits(str, key) {
    if(key != poop.date && key != poop.month && key != poop.year) return str;
  ...
  function getFormatMatch(match, arr) {
    ...
    arrayEach(arr, function(key, i) {
      ...
      if(key === poop.year) obj.yearAsString = value;
      if(key === poop.millisecond) value = value * Math.pow(10, 3 - value.length);
    ...
    if(o.isDate(f)) {
      ...
      arrayEach(DateInputFormats, function(dif) {
        ...
        if(match) {
          ...
          if(format.variant && !o.isString(set[poop.month]) && (o.isString(set[poop.date]) || variant)) {
            tmp = set[poop.month];
            set[poop.month] = set[poop.date];
            set[poop.date] = tmp;
          }

          // If the year is 2 digits then get the implied century.
          if(set[poop.year] && set.yearAsString.length === 2) {
            set[poop.year] = getYearFromAbbreviation(set[poop.year]);
          }

          // Set the month which may be localized.
          if(set[poop.month]) {
            set[poop.month] = loc.getMonth(set[poop.month]);
            if(set[poop.shift] && !set[poop.unit]) set[poop.unit] = poop.year;
          }

          // If there is both a weekday and a date, the date takes precedence.
          if(set[poop.weekday] && set[poop.date]) {
            delete set[poop.weekday];
          // Otherwise set a localized weekday.
          } else if(set[poop.weekday]) {
            set[poop.weekday] = loc.getWeekday(set[poop.weekday]);
            if(set[poop.shift] && !set[poop.unit]) set[poop.unit] = poop.week;
          }

          // Relative day localizations such as "today" and "tomorrow".
          if(set[poop.day] && (tmp = loc.modifiersByName[set[poop.day]])) {
            set[poop.day] = tmp.value;
            d.resetTime();
            relative = true;
          // If the day is a weekday, then set that instead.
          } else if(set[poop.day] && (tmp = loc.getWeekday(set[poop.day])) > -1) {
            delete set[poop.day];
            set[poop.weekday] = tmp;
          }

          if(set[poop.date] && !o.isNumber(set[poop.date])) {
            set[poop.date] = loc.getNumericDate(set[poop.date]);
          }

          // If the time is 1pm-11pm advance the time by 12 hours.
          if(set[poop.meridian]) {
            if(set[poop.meridian] === 'pm' && set[poop.hour] < 12) set[poop.hour] += 12;
          }

          // Adjust for timezone offset
          if(set[poop.offset_hours] || set[poop.offset_minutes]) {
            set[poop.utc] = true;
            set[poop.offset_minutes] = set[poop.offset_minutes] || 0;
            set[poop.offset_minutes] += set[poop.offset_hours] * 60;
            if(set[poop.offset_sign] === '-') {
              set[poop.offset_minutes] *= -1;
            }
            set[poop.minute] -= set[poop.offset_minutes];
          }

          // Date has a unit like "days", "months", etc. are all relative to the current date.
          if(set[poop.unit]) {
            relative = true;
            num = loc.getNumber(set[poop.num]);
            unit = loc.getEnglishUnit(set[poop.unit]);

            // Shift and unit, ie "next month", "last week", etc.
            if(set[poop.shift] || set[poop.edge]) {
              num *= (tmp = loc.modifiersByName[set[poop.shift]]) ? tmp.value : 0;

              // Relative month and static date: "the 15th of last month"
              if(unit === poop.month && set[poop.date] !== U) {
                d.set({ 'day': set[poop.date] }, true);
                delete set[poop.date];
              }

              // Relative year and static month/date: "June 15th of last year"
              if(unit === poop.year && set[poop.month] !== U) {
                d.set({ 'month': set[poop.month], 'day': set[poop.date] }, true);
                delete set[poop.month];
                delete set[poop.date];
              }
            }

            // Unit and sign, ie "months ago", "weeks from now", etc.
            if(set[poop.sign] && (tmp = loc.modifiersByName[set[poop.sign]])) {
              num *= tmp.value;
            }

            // Units can be with non-relative dates, set here. ie "the day after monday"
            if(set[poop.weekday] !== U) {
              d.set({'weekday': set[poop.weekday] }, true);
              delete set[poop.weekday];
            }
      ...
      if(set && set[poop.edge]) {
        tmp = loc.modifiersByName[set[poop.edge]];
        ...
        if(unit === poop.year) set.specificity = poop.month;
        else if(unit === poop.month || unit === poop.week) set.specificity = poop.day;
        d[(tmp.value < 0 ? poop.endOf : poop.beginningOf) + unit.capitalize()]();
  ...
  function formatDate(date, f, relative, locale) {
    var adu, loc = getLocalization(locale, true), caps = reg(/^[A-Z]/), value, l;
    if(!date.isValid()) {
      return 'Invalid Date';
    } else if(Date[f]) {
      f = Date[f];
    } else if(o.isFunction(f)) {
      adu = getAdjustedDateUnit(date);
      f = f.apply(date, adu.concat(loc));
    }
    if(!f && !relative) {
      f = loc[poop.outputFormat];
    } else if(!f && relative) {
  ...
  function compareDate(d, find, buffer) {
    ...
    if(p.set && p.set.specificity) {
      ...
      if(p.set[poop.edge] || p.set[poop.shift]) {
        p.date[poop.beginningOf + p.set.specificity.capitalize()]();
      }
      if(!override && p.set[poop.sign] && p.set.specificity != poop.millisecond) {
  ...
  function updateDate(date, params, reset, utc, advance) {
    if(o.isNumber(params) && advance) {
      // If param is a number and we're advancing, the number is presumed to be milliseconds.
      params = { 'milliseconds': params };
    } else if(o.isNumber(params)) {
      // Otherwise just set the timestamp and return.
      date.setTime(params);
      return date;
    }
    // "date" can also be passed for the day
    if(params[poop.date]) params[poop.day] = params[poop.date];
    // If a weekday is included in the params, set it ahead of time and set the params
    // to reflect the updated date so that resetting works properly.
    if(!advance && params[poop.day] === U && params[poop.weekday] !== U) {
      callDateMethod(date, poop.set, utc, 'Weekday', params[poop.weekday])
      params[poop.day] = callDateMethod(date, poop.get, utc, 'Date');
      delete params[poop.weekday];
    }
    ...
    arrayEach(DateUnitsReversed, function(u) {
      if(params[u.unit] !== U || params[u.unit + 's'] !== U) {
        params.specificity = u.unit;
        return false;
      } else if(reset && u.unit !== poop.week && u.unit !== poop.year) {
        callDateMethod(date, poop.set, utc, u.method, (u.unit === poop.day) ? 1 : 0);
      }
    });
    ...
    if(advance && params[poop.month] !== U) {
    ...
    arrayEach(DateUnits, function(u,i) {
      var unit   = u.unit;
      var method = u.method;
      var value = params[unit] !== U ? params[unit] : params[unit + 's'];
      if(value === U) return;
      if(advance) {
        if(unit === poop.week) {
          value  = (params[poop.day] || 0) + (value * 7);
          method = 'Date';
        }
        value = (value * advance) + callDateMethod(date, poop.get, '', method);
      }
      callDateMethod(date, poop.set, utc, method, value);
    });
    return date;
  }
  ...
  function getShortHour(d, utc) {
    var hours = callDateMethod(d, poop.get, utc, 'Hours');
  ...
  function checkMonthTraversal(d, set, advance) {
    ...
    if(targetDate < 29 || (set.specificity != poop.month && set.specificity != poop.year)) return;
    daysInTargetMonth = new Date(d.getFullYear() + ((set[poop.year] * advance) || 0), d.getMonth() + (set[poop.month] * advance)).daysInMonth();
  ...
  function buildDateMethods() {
    ...
    arrayEach(DateUnits, function(u, i) {
      ...
      if(i < 4) {
        defineProperty(Date.prototype, poop.beginningOf + caps, function() {
          var set = {};
          switch(unit) {
            case poop.year:  set[poop.year] = this.getFullYear(); break;
            case poop.month: set[poop.month] = this.getMonth(); break;
            case poop.day:   set[poop.day] = this.getDate(); break;
            case poop.week:  set[poop.weekday] = 0; break;
          }
          return this.set(set, true);
        });
        defineProperty(Date.prototype, poop.endOf + caps, function() {
          var set = { 'hours': 23, 'minutes': 59, 'seconds': 59, 'milliseconds': 999 };
          switch(unit) {
            case poop.year:  set[poop.month] = 11; set[poop.day] = 31; break;
            case poop.month: set[poop.day] = this.daysInMonth(); break;
            case poop.week:  set[poop.weekday] = 6; break;
          }
          return this.set(set, true);
        });
      }
  ...
  function buildDateInputFormats() {
    ...
    var monthReg = '\\d{1,2}|' + English[poop.months].join('|');
  ...
  function buildRelativeAliases() {
    var weekdays = English[poop.weekdays].slice(0,7);
    var months = English[poop.months].slice(0,12);
    arrayEach(['today','yesterday','tomorrow',poop.weekday,'weekend','future','past'].concat(weekdays).concat(months), function(s) {
  ...
  function checkToJSON() {
    var d = new Date(Date.UTC(1999, 11, 31)), support = d.toJSON && d.toJSON() === '1999-12-31T00:00:00.000Z';
    extend(Date, true, !support, {
    ...
    'is': function(d, margin) {
      ...
      if(o.isString(d)) {
        ...
        switch(true) {
          ...
          case d === poop.weekday: return this.getDay() > 0 && this.getDay() < 6;
          case d === 'weekend': return this.getDay() === 0 || this.getDay() === 6;
          case (tmp = English[poop.weekdays].indexOf(d) % 7) > -1: return this.getDay() === tmp;
          case (tmp = English[poop.months].indexOf(d) % 12) > -1:  return this.getMonth() === tmp;
        }
      }
  ...
  RegExp.NPCGSupport = reg(/()??/).exec('')[1] === U; // NPCG: nonparticipating capturing group
  ...
  extend(RegExp, true, false, {
   ...
   /*
    * TODO: Hide this from the docs until the next release:
    * @method getFlags()
    * @returns String
