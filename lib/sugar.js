...
//
      ...
      var poop = {
        ...
        'unit': set.units.join('|'),
        'num': set.numbers.concat('\\d+').concat(set.modifiers.filter(function(m){ return m.type == 'num'; }).map('text')).join('|'),
      }
      ...
      arrayEach(set.formats, function(src) {
        ...
        src= src.replace(/ /g, '\\s*');
        src = src.replace(/\{(.+?)\}/g, function(match, k) {
          var key = k.replace(/\?$/, ''); optional = key !== k, value = poop[key];
          if(value === undefined) {
