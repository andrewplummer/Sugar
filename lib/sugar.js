      ...
      arrayEach(dateInputFormats, function(df) {
        ...
        if(match) {
          ...
          if(m.unit && m.sign) {
            ...
            */
           // SHORTEN ME
            if(unit === DateLanguages[lang].units[7] && !format.relative) {
              set.year = d.getFullYear();
            } else if(unit === DateLanguages[lang].units[6] && !format.relative) {
              set.month = d.getMonth();
            } else if(unit === DateLanguages[lang].units[5] && !format.relative) {
              set.day = d.getDate();
              unit = 'day';
              amt *= 7;
            }
            ...
            console.info(set, unit, set[unit], 'whoaaaa', amt, ' ohhhhhhhhh', m.modifier_amount, 'meeeeeee', unit);
            set[unit] += amt;
          }
          if(m.edge) {
...
//
      ...
      var setModifiers = function(name) {
        var arr = setArray(name, []), value;
        arrayEach(arr, function(m, i) {
          arrayEach(m.split('|'), function(n) {
            val = arr.length === 2 && i === 1 ? 1 : i - 1;
            set.modifiers[n] = { value: val };
          });
        });
        poop[name] = arr.join('|');
      }
      ...
      */
      ...
      var poop = {
        ...
        'day': set.day.join('|'),
        'year': '\\d{4}',
        'month': set.months.join('|'),
        'weekday': set.weekdays.join('|'),
        'unit': set.units.join('|'),
        'dunit': set.units.filter(function(u,i){ return (i % 8) > 4; }).join('|'),
