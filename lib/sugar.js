(function() {
  ...
  extend(String, true, false, {
     ...
     *
     ***/
    'toDate': function(lang) {
      return createDate([this.toString(), lang]);
    },
  ...
  var optionalTime  = '\\s*(?:(?:t|at |\\b)(\\d{1,2}):?(\\d{2})?:?(\\d{2})?(\\.\\d{1,6})?(am|pm)?(?:(Z)|(?:([+-])(\\d{2})(?::?(\\d{2}))?)?)?)?';
  ...
  var StaticInputFormats = [
    ...
    { src: '(\\d{2})-?(\\d{2})-?(\\d{2})', to: ['year','month','date'] },
    // @date_format 05-25
    // @date_format 05/25
    // @date_format 05.25
    { src: '(\\d{1,2})[-\\/.](\\d{1,2})', to: ['month','date'], variant: true },
  ...
  //{ reg: '({MONTHS})[\\s\\-.](\\d{4})', to: ['month','year'] },
    ...
    //{ reg: '(midnight|noon)(?: (tonight|today|tomorrow|yesterday|{WEEKDAYS}))?', to: ['hour','fuzzy_day'], timeImplied: true },
    // @date_format 12pm
    // @date_format 12:30pm
    // @date_format 12:30:40
    // @date_format 12:30:40.299
    // @date_format 12:30:40.299+01:00
    { src: '^(?:(\\d{1,2}):?(\\d{2})?:?(\\d{2})?(\\.\\d{1,6})?(am|pm)?(?:(Z)|(?:([+-])(\\d{2})(?::?(\\d{2}))?)?)?)$', to: timeArray, today: true, timeImplied: true }
  ];

  var TextBasedDateFormats = [
    // @date_format May 2010
    { src: '({MONTHS})[\\s\\-.](\\d{4})', to: ['month','year'] },
    // @date_format Tuesday January 21st, 2010
    // @date_format Tuesday January 21, 2010
    // @date_format Jan-21-10
    // @date_format January 2010 NO
    // @date_format 2010 NO
    { src: '(?:{WEEKDAYS})?\\s*({MONTHS})[\\s\\-.]?(\\d{1,2})(?:{ORDINALS}),?[\\s\\-.]?(\\d{2,4})?', to: ['month','day','year'] },
  ];
  ...
  var CommonLanguages = {
    'en': {
      ...
      modifiers: [
        ...
        { name: 'edge', text: 'last day', value: -1, time: true },
      ...
      formats: [
        ...
        '{shift} {unit=5-7}',
        '{3} {edge} of {shift?} {unit=4-7?}{month?}{year?}',
        '{weekday} {2} {shift} week',
        'the {date}{1} of {month}',
        '{3}{month?} {date?}{1} of {shift} {unit=6-7}',
        '{day} at {time}',
        '{time} {day}'
      ]
    }
  ...
  var checkLocalizationInitialized = function(lang) {
    if(!DateLanguages[lang] && CommonLanguages[lang]) {
      initializeLocalization(lang);
    }
  };

  var initializeLocalization = function(lang) {
    Date.registerLanguage(lang, CommonLanguages[lang]);
  };
  ...
  var getExtendedDate = function(f) {
    var d = new Date(), utc = false, relative = false, match, format, set, setAfter, lang, tmp;
    ...
    if(Object.isDate(f)) {
      ...
      arrayEach(dateInputFormats, function(dif) {
        ...
        if(match) {
          ...
          if(NPCGMatch(set.date, /^[a-z]+$/) || (Date.allowVariant && format.variant)) {
          ...
          if(set.year) {
            if(set.yearAsString.length === 2) {
              set.year = getYearFromAbbreviation(parseFloat(set.year));
            }
          }
          if(set.month) {
            tmp = parseFloat(set.month);
            if(isNaN(tmp)) {
              // If the month is not a number, find it in the array of text months.
              set.month = DateLanguages[lang].months.indexOf(set.month) % 12;
              if(set.shift && !set.unit) set.unit = 'month';
            } else {
          ...
          if(set.weekday && set.date) {
            // Hard dates take precedence over weekdays.
            delete set.weekday;
          } else if(set.weekday) {
            // "Fuzzy" days include weekdays, "today", "tomorrow", etc.
            set.day = set.weekday;
          ...
          if(set.day) {
            ...
            // SHORTEN ME
            if((tmp = DateLanguages[lang].weekdays.indexOf(set.day) % 7) !== -1) {
              delete set.day;
              set.weekday = tmp;
            } else if(tmp = DateLanguages[lang].modifiersByName[set.day]) {
              set.day = tmp.value;
              d.resetTime();
              relative = true;
            }
            ...
            var dayOffset = 0;
            var fuzzy = set.fuzzy_day;
            var weekday;
            if(fuzzy === 'yesterday') {
              dayOffset = -1;
            } else if(fuzzy === 'tomorrow') {
              dayOffset = 1;
            } else if((weekday = DateLanguages[lang].weekdays.indexOf(fuzzy) % 7) !== -1) {
              d.setWeekday(weekday);
              dayOffset = 0;
              if(set.sign && !set.unit && !set.modifier_amount) {
                set.unit = 'week';
              }
            }
          ...
          if(set.millisecond) {
            ...
            set.millisecond = Math.round(parseFloat(set.millisecond, 10) * 1000);
          }
          /*
          ...
          */
          ...
          if(set.utc || set.offset_hours || set.offset_minutes) {
            utc = true;
            // Adjust for timezone offset
            var offset = 0;
            if(set.offset_hours) {
              offset += parseFloat(set.offset_hours) * 60;
            }
            if(set.offset_minutes) {
              offset += parseFloat(set.offset_minutes);
            }
            if(set.offset_sign && set.offset_sign === '-') {
              offset *= -1;
            }
            set.minute -= offset;
          }
          if(set.unit) {
            relative = true;
            // SHORTEN ME
            var amt  = (parseFloat(set.num) || (DateLanguages[lang].numbers.indexOf(set.num) + 1) || 1);
            // SHORTEN ME
            set.engUnit = EnglishUnits[DateLanguages[lang].units.indexOf(set.unit) % 8];
            /* DEPRECATE ME!!
            var textNumIndex;
            if(amt === 'the' || amt === 'a') {
              amt = 1;
            ...
            if(set.modifier_sign === 'before' || set.modifier_sign === 'ago' || set.modifier_sign === 'last') {
              amt *= -1;
            } else if(set.modifier_sign === 'this' || set.modifier_sign === 'the') {
              amt = 0;
            }
            */
           // SHORTEN ME
           if(set.shift) {
             var tmp = DateLanguages[lang].modifiersByName[set.shift];
             var shift = tmp ? tmp.value : 0;
             amt *= shift;
             if(set.engUnit === 'month' && set.date !== undefined) {
               setAfter = { day: set.date };
               delete set.date;
             }
             if(set.engUnit === 'year' && set.month !== undefined) {
               setAfter = { month: set.month, day: set.date };
               delete set.month;
               delete set.date;
             }
           }
           if(set.sign) {
             amt *= DateLanguages[lang].modifiersByName[set.sign].value;
           }
           if(set.weekday !== undefined || set.month !== undefined) {
             d.set({ weekday: set.weekday, month: set.month }, true);
           }
            /* NEED THIS????
            if(set.unit === DateLanguages[lang].units[6] && !format.relative) {
              set.year = d.getFullYear();
            } else if(set.unit === DateLanguages[lang].units[5] && !format.relative) {
              set.month = d.getMonth();
            } else*/
           /* maybe not.>>???
           if(set.unit === DateLanguages[lang].units[4] && !format.relative) {
             set.day = d.getDate();
             unit = 'day';
             amt *= 7;
           }
           */
           if(set[set.engUnit] === undefined) {
             set[set.engUnit] = 0;
           }
           set[set.engUnit] += amt;
         }
          //if(set.edge) {
            //var edge = set.edge;
            //var amt  = (parseFloat(set.modifier_amount) || (DateLanguages[lang].numbers.find(set.modifier_amount) + 1) || 1) * (DateLanguages[lang].modifiersByName[set.sign].value || 1);
            /*
            if(DateLanguages[lang].modifiersByName[set.edge].value === 1) {
              if(engUnit === 'week') {
                //set.month = d.getMonth();
                set.weekday = 0;
              }
              //if(!edge.match(/day/)) {
                // ALWAYS reset times here.... no???
                //set.hour        = 0;
                //set.minute      = 0;
                //set.second      = 0;
                //set.millisecond = 0;
              //}
            } else if(DateLanguages[lang].modifiersByName[set.edge].value === -1) {
              if(engUnit === 'week') {
                set.month = d.getMonth();
                set.weekday = 6;
              } else if(engUnit === 'month' || set.month) {
                set.day = 32 - new Date(d.getFullYear(), set.month, 32).getDate();
              } else if(engUnit === 'year') {
                set.month = 11;
                set.day  = 31;
              }
              //if(!edge.match(/day/)) {
                // ALWAYS set times to the end here... no???
                set.hour        = 23;
                set.minute      = 59;
                set.second      = 59;
                set.millisecond = 999;
            ...
            */
          if(set.year_sign && set.year_sign === '-') {
            set.year *= -1;
          }
          /*
          if(format.today) {
      ...
      if(set && set.edge) {
        // TODO: CLEANUP
        var mod = DateLanguages[lang].modifiersByName[set.edge];
        var edgeUnit;
        arrayEach(dateUnits.slice(0,4).reverse(), function(u) {
          if(set[u.unit] !== undefined) edgeUnit = u.unit;
        });
        if(edgeUnit === 'year') set.specificity = 'month';
        else if(edgeUnit === 'month' || edgeUnit === 'week') set.specificity = 'day';
        var method = (mod.value === -1 ? 'endOf' : 'beginningOf') + edgeUnit.capitalize();
        d[method].call(d);
        if(mod.time) d.resetTime();
      }
      if(setAfter) {
        d.set(setAfter, true);
      }
  ...
  var compareDate = function(d, find, buffer, dir, edges) {
    var dateUnit, accuracy;
    var p = getExtendedDate(find);
    buffer = buffer > 0 ? buffer : 0;
    if(!p.date.isValid()) return false;
    if(p.set && p.set.specificity) {
      dateUnit = dateUnits.find(function(u) {
        return u.unit === p.set.specificity;
      });
      accuracy = dateUnit.multiplier(p.date, d - p.date) - 1;
    /*
    arrayEach(dateUnits, function(u) {
      console.info('ACCURACTY', u.unit, p.set['hour'], dateUnits);
    ...
    */
      if(p.format && p.format.to.find('shift')) {
        var beginning = p.date['beginningOf'+ dateUnit.unit.capitalize()];
        if(beginning) {
  ...
  var updateDate = function(date, params, reset, utc, advance) {
    ...
    // UPDATE: It makes no sense that I'm resetting HERE in the first place, no???
    //if(reset) {
      //date.setDate(15);
    //}
    ...
    if(!advance && params.hasOwnProperty('weekday')) {
    ...
    // or for years.
    arrayEach(EnglishUnits, function(u) {
      if(params[u] !== undefined || params[u + 's'] !== undefined) {
        params.specificity = u;
        return false;
      } else if(reset && u !== 'week' && u !== 'year') {
        params[u] = (u === 'day') ? 1 : 0;
      }
    });
  ...
  /* DEPRECATE??
  var getDateValue = function(params, unit, index, reset) {
    var value = params[unit] || params[unit + 's'];
    if(reset && index > 3) {
      return 0;
    } else if(params[unit] !== undefined) {
      return params[unit];
    } else {
      return params[unit + 's'];
    }
  } */
  ...
  var getMonth = function(s) {
    ...
    var index = EnglishMonths.indexOf(s) % 12;
    return index === -1 ? null : index;
  }

  var getWeekday = function(s) {
    if(/\w+\s+\w+/.test(s)) return null;
    var index = EnglishWeekdays.indexOf(s) % 7;
    return index === -1 ? null : index;
  }
  ...
  var createDate = function(args) {
    var f, lang = args[1] || 'en';
    if(Object.isString(args[0])) {
      // If a string was passed, then make sure the localization is set
      checkLocalizationInitialized(lang);
    }
  ...
  var buildDateInputFormats = function() {
    ...
    EnglishUnits    = DateLanguages['en']['units'];
    EnglishMonths   = DateLanguages['en']['months'];
    EnglishWeekdays = DateLanguages['en']['weekdays'];
    var monthReg = '\\d{1,2}|' + EnglishMonths.join('|');
  ...
  var buildRelativeAliases = function() {
    var weekdays = EnglishWeekdays.slice(0,7);
    var months = EnglishMonths.slice(0,12);
  ...
  extend(Date, false, false, {
    ...
    'registerLanguage': function(code, set) {
      //var f = 
//'({MONTHS})[\\s\\-.](\\d{4})'
//
      var setArray = function(name, def, lower, length) {
        var arr = set[name] || def;
        if(lower)  arr = arr.map('toLowerCase');
        if(length) arr = arr.concat(arr.map(function(s) { return s.to(length); }));
        return set[name] = arr;
      }

      var addFormat = function(src, format) {
        format.reg = new RegExp('^' + src + '$', 'i');
        format.lang = code;
        dateInputFormats.unshift(format);
      }

      var setModifiers = function(name) {
        var arr;
        arrayEach(set.modifiers, function(m) {
          set.modifiersByName[m.text] = m;
        });
        Object.each(set.modifiers.groupBy('name'), function(name, m) {
          m = m.map('text');
      ...
      setArray('months', [], true, set.abbreviatedMonthLength);
      setArray('weekdays', [], true, set.abbreviatedWeekdayLength);
      setArray('numbers', []);
      setArray('units', []);
      setArray('articles', []);
      set.modifiersByName = {};
      if(set.pluralSuffix) {
        set.units = set.units.concat(set.units.map(function(u){ return u + set.pluralSuffix; }));
      }

      /*
      arrayEach(set.modifiers, function(m) {
        set.modifiersByName[m.text.toLowerCase()] = m;
      });
      */
      ...
      var poop = {
        ...
        'year': '\\d{4}',
        //'space': '[-,. ]+',
        'num': set.numbers.concat(set.articles).concat('\\d+').join('|'),
      }

      setModifiers();

      console.info(set);
      addFormat('(' + set.months.join('|') + ')', { to: ['month'] });
      addFormat('(' + set.weekdays.concat(set.modifiers.filter(function(m){ return m.name === 'day'; }).map('text')).join('|') + ')', { to: ['day'] });

      arrayEach(set.formats, function(src) {
        var options = { to: [] }, modifiers;
        src = src.replace(/\s+/g, '[-,. ]*');
        src = src.replace(/\{(.+?)\}/g, function(all, k) {
          var opt = k.match(/\?$/), slice = k.match(/(\d)(?:-(\d))?/), nc = k.match(/^\d+$/), key = k.replace(/[^a-z]+$/, ''), value, len;
          if(key === 'time') {
            options.to = options.to.concat(timeArray);
            return optionalTime;
          }
          if(nc) {
          ...
          /*
          if(value === undefined) {
            modifiers = set.modifiers.filter(function(m){
              return m.type === key;
            });
            if(modifiers.length > 0) {
              if(key === 'sign') options.relative = true;
              if(modifiers.all(function(m){ return !m.hasOwnProperty('value'); })) optional = true;
              value = modifiers.map('text').join('|');
            }
          }
          if(value) {
            if(optional) {
        ...
        addFormat(src, options);
      });

      dateInputFormats.each(function(f) {
        console.info(f.reg.source, f.variant);
      });

      /*
      arrayEach(set.formatOrders, function(fo) {
        var tokens = [];
        var to = [];
        fo.each(function(s) {
          if(s === 'd') {
            tokens.push('(?:(\\d{1,2})' + (set.dateSuffix ? '(?:' + set.dateSuffix + ')?' : '') + ')?');
            to.push('day');
          } else if(s === 'm') {
            tokens.push('(' + set.months.join('|') + ')');
            to.push('month');
          } else if(s === 'y') {
            tokens.push('(?:(\\d{4})' + (set.yearSuffix ? '(?:' + set.yearSuffix + ')?' : '') + ')?');
            to.push('year');
          }
        });
        addFormat(tokens.join('[\\s,]*'), { to: to });
      });

      var setRelativeFormat = function(name, format, sign) {
        if(!format) return;
        var match = [];
        set[name] = format.replace(new RegExp('1|' + set.units[1], 'gi'), function(r) {
          if(r === '1') {
            match.push('modifier_amount');
            return '{num}';
          } else {
            match.push('modifier_unit');
            return '{unit}';
          }
        });
        var vars = { num: '(' + set.articles.join('|') + set.numbers.join('|') + '|\\d+)', unit: '(' + set.allUnits.join('|') + ')' };
        addFormat(set[name].assign(vars), { to: match, relative: true, sign: sign });
      }
      */

      //setRelativeFormat('relativePastFormat', set.oneSecondAgo, -1);
      //setRelativeFormat('relativeFutureFormat', set.oneSecondFromNow, 1);

      /*
      if(set.oneSecondAgo) {
        set.relativePastFormat = set.oneSecondAgo.replace(/1/, '{num}').replace(set.units[1], '{unit}');
        var match = set.relativePastFormat.match(/\{.+?\}/g,
        var r = supplant(s.relativePastFormat, [{
          num: '(' + set.articles.join('|') + set.numbers.join('|') + '|\\d+)',
          unit: '(' + set.allUnits.join('|') + ')'}]);
          console.info(p);
        dateInputFormats.unshift({ reg: new RegExp('^' + r + '$'), to: to });
      }
      set.relativeFutureFormat = set.oneSecondFromNow.replace(/1/, '{num}').replace(set.units[1], '{unit}');
      */

      //dateInputFormats.unshift({ reg: new RegExp('^' + '), to: to });
    ...
    //{ reg: '(a|{NUMBER}|\\d+) (millisecond|second|minute|hour|day|week|month|year)s? (from now|ago)', to: ['modifier_amount','modifier_unit','modifier_sign'], relative: true },
      ...
      arrayEach(TextBasedDateFormats, function(f) {
        var src = '^' + f.src;
        var to = f.to.concat();
        src = src.replace(/{WEEKDAYS}/, set.weekdays.join('|'));
        src = src.replace(/{MONTHS}/, set.months.join('|'));
        src = src.replace(/{NUMBER}/, set.numbers.join('|'));
        //src = src.replace(/{ORDINALS}/, set.ordinalSuffixes.join('|'));
        if(!f.timeImplied) {
          src = src + optionalTime;
          to = to.concat(['hour','minute','second','millisecond','meridian','utc','offset_sign','offset_hours','offset_minutes']);
        }
        dateInputFormats.unshift({ reg: new RegExp(src, 'i'), to: to });
      });
      */

      DateLanguages[code] = set;

      //var monthReg   = months.map(function(m) { }).join('|');
      //dateInputFormats.push({ reg: new RegExp('('+ anyMonth +')[\\s\\-.](\\d{4})', 'i'), to: ['month','year'] });
      //{ reg: '({MONTHS})[\\s\\-.](\\d{4})', to: ['month','year'] },
      //abbreviatedMonths   = months.map(function(m) { return m.to(3); });
      //abbreviatedWeekdays = weekdays.map(function(m) { return m.to(3); });
      //var monthReg   = months.map(function(m) { return m.to(3)+'(?:\\.|'+m.from(3)+')?'; }).join('|');
  ...
  extend(Date, true, false, {
    ...
    'isBetween': function(first, second, margin) {
      // TODO REFACTOR THIS TO TAKE SPECIFICITY INTO ACCOUNT.... SET TO THE END OF WHATEVER IF THERE IS SPECIFICITY
      /*
      var time = this.getTime();
      var one = getExtendedDate(first);
      var two = getExtendedDate(second);
      var t1 = one.getTime();
      var t2 = two.getTime();
      margin = margin || 0;
      if(
      console.info('WTAA??', this, Date.create(first), Date.create(second), time, one, two, margin);
      return (time > Math.min(t1, t2) - margin) && (time < Math.max(t1, t2) + margin);
      */
