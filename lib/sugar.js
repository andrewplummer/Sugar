(function() {
  ...
  function buildWatch() {
    extend(Object, false, true, {
       ...
       * @extra <fn> is passed three arguments: the property (<prop>), the old value, and the new value. The return value of [fn] will be set as the new value. This method is useful for things such as validating or cleaning the value when it is set. Warning: this method WILL NOT work in IE 8 and below (this is the only method in Sugar that is not fully compatible).
       * @example
       *
       ...
       *
       ***
       * @method watch(<prop>, <fn>)
       * @returns Nothing
       * @short Watches a property of the object and runs <fn> when it changes.
       * @extra <fn> is passed three arguments: the property (<prop>), the old value, and the new value. The return value of [fn] will be set as the new value. This method is useful for things such as validating or cleaning the value when it is set. Warning: this method WILL NOT work in IE 8 and below (this is the only method in Sugar that is not fully compatible). This method is only available on extended objects created with %Object.extended%.
       * @example
       *
       *   Object.extended({ foo: 'bar' }).watch('foo', function(prop, oldVal, newVal) {
       *     // Will be run when the property 'foo' is set on the object.
       *   });
       *
       ***/
  ...
  function buildTrim() {
    var support = /^\s+$/.test(getTrimmableCharacters());
  ...
  extend(String, true, false, {
    ...
    'truncate': function(length, append, split) {
      ...
      reg = new RegExp('[^' + getTrimmableCharacters() + a + '][' + getTrimmableCharacters() + a + ']');
  ...
  function checkLocaleFormatsAdded(code) {
  ...
  function initializeLocale(code, set, current) {
    if(!code) return null;
  ...
  function getLocalizationSet(code) {
    ...
    set.pastFormat = set.formats[0];
    set.futureFormat = set.formats[bool(3) ? 1 : 0];
    return set;
  }
  ...
  extend(Localization, true, false, {
    ...
    'relative': function(num, u, ms) {
      ...
      format = ms > 0 ? this.futureFormat : this.pastFormat;
      if(this.code == 'ru') {
  ...
  function getExtendedDate(f, locale) {
    var d = new Date(), relative = false, variant = getVariant(locale), format, set, unit, num, loc, tmp;
    if(Object.isDate(f)) {
      d = f;
      ...
      checkLocaleFormatsAdded(locale);
  ...
  function formatDate(date, f, relative, locale) {
    var adu, loc = getLocalization(locale), caps = /^[A-Z]/, value;
    if(!date.isValid()) {
      return 'Invalid Date';
    } else if(Date[f]) {
      f = Date[f];
  ...
  function buildDateMethods() {
    arrayEach(DateUnits, function(u, i) {
      ...
      defineProperty(Date.prototype, unit+'sSince', function(f) {
        return round((this.getTime() - Date.create(f).getTime()) / multiplier);
      });
      defineProperty(Date.prototype, unit+'sUntil', function(f) {
        return round((Date.create(f).getTime() - this.getTime()) / multiplier);
      });
  ...
  extend(Date, true, false, {
     ...
     * @extra [fn] can be passed to provide for more granular control over the resulting string. [fn] is passed 4 arguments: the adjusted value, unit, offset in milliseconds, and a localization object. As an alternate syntax, [locale] can also be passed as the first (and only) parameter instead. For more information, see @date_format.
     * @example
     *
  ...
  extend(Function, true, false, {
    ...
    'lazy': function(ms, limit) {
      ...
      function lazy() {
        ...
        setDelay(lazy, round(++run * ms), fn, this, arguments);
      }
      return lazy;
    },
