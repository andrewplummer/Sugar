...
(function(){
  ...
  var dateFormats = [
    ...
    { reg: /^(\d{4})[-\/.](\d{1,2})[-\/.](\d{1,2})$/, to: ['year','month','day'] }, // Standard ISO8601
    ...
    // Text formats
    { reg: new RegExp('^('+monthReg+')[\\s\\-.]?(\\d{4})$', 'i'), to: ['month','year'] }, // Month yyyy
    { reg: new RegExp('^('+monthReg+') (\\d{1,2})(?:st|nd|rd|th)?,? (\\d{4})$', 'i'), to: ['month','day','year'] }, // Month dd yyyy
    { reg: new RegExp('^(\\d{1,2}) ('+monthReg+'),? (\\d{4})$', 'i'), to: ['day','month','year'] } // dd Month yyyy
  ];
  ...
  var getDateFromString = function(str, variant){
    ...
    dateFormats.each(function(format){
      ...
      if(match){
        console.info('woohoo', match, format);
        format.to.each(function(t, i){
          ...
          if(t === 'month'){
            if(isNaN(value)){
              ...
              value = abbreviatedMonths.indexOf(match[i + 1].capitalize().to(2));
            } else {
              // Otherwise decrement by 1.
              value--;
            }
          }
          if(t === 'year' && match[i + 1].length === 2){
            // If the year is two digits, add the most appropriate century prefix.
            value = (new Date).getFullYear().round(-2) - value.round(-2) + value;
          }
          result[t] = value;
        });
      }
