(function() {
  ...
  var buildWidthConversionTables = function() {
    ...
    widthConversionRanges.each(function(r) {
      ...
      hr += String.fromCharCode(r.start) + '-' + String.fromCharCode(r.end);
      zr += String.fromCharCode(r.start + r.shift) + '-' + String.fromCharCode(r.end + r.shift);
    });
    zenkakuKatakana.each(function(c, i) {
      hankaku = hankakuKatakana.charAt(i);
      type = katakanaPunctuation.test(c) ? 'p' : 'k';
      setWidthConversion(type, hankaku, c);
    ...
    allHankaku = new RegExp('[' + hr.escapeRegExp() + '][ﾞﾟ]?', 'g');
    allZenkaku = new RegExp('[' + zr.escapeRegExp() + ']', 'g');
  }
