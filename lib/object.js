  ...
  function objectMerge(target, source, deep, resolve, hidden, descriptor) {
    ...
    if (isUndefined(target) || resolveConflicts && isRegExp(source)) {
      // Regexes cannot be updated in place, effectively requiring them to be
      // reassigned to be properly merged, so do this here as long as resolving
      // conflicts is allowed.
