  ...
  function parseQueryComponent(obj, key, value, deep, auto, separator, transform) {
    ...
    obj[key] = getTransformedValue(obj, key, value, auto, transform);
  }
  ...
  function getTransformedValue(obj, key, value, auto, transform) {
    var fnValue;
    if (transform) {
  ...
  function mergeAll(target, sources, opts) {
    ...
    forEach(sources, function(source) {
      return objectMergeWithOptions(target, source, opts);
    });
    return target;
  }
  ...
  function objectMergeWithOptions(target, source, opts) {
    opts = opts || {};
  ...
  defineInstance(sugarObject, {
     ...
     *   Object.merge({a:1},{a:2},{resolve:false}) -> { a:1 }
     +   Object.merge({a:1},{a:2},{resolve:function(key, a, b) {
     *     return a + b;
     *   }}); -> { a:3 }
     *
     ***/
     ...
     * @returns Merged object
     * @short Merges properties from <sources>, which should be an array, into <target>.
     * @extra Options are identical to %Object.merge%: %deep% performs deep merging (default is %false%), %hidden% merges non-enumerable properties (default is %false%), %descriptor% merges by property descriptor (default is %false%), and %resolve% determines which property wins in the case of a conflict (default is %true%, meaning source wins). If a function is passed for %resolve%, the return value will become the final value. Supports %extended objects%. For more, see %object_merging%.
     *
     * @example
     *
     *   Object.mergeAll({a:1},[{b:2},{c:3}]) -> {a:1,b:2,c:3}
     +   Object.mergeAll({a:1},[{a:2},{b:5}], {resolve:false}); -> {a:1,b:5}
     *
     ***/
